...
class PrepareSingleImage(object):
    ...
    def _search_config_keys_for_flash_config_file(self, config_keys):
        ...
        """
        if config_keys.get("DBG_CORE") == "app/p1":
            return config_keys.get("FLASH_CONFIG", '')
        return None
    ...
    def create_flash_image(self, devkit, output_path, encrypted = False,
        ...
        if encrypted is True and "audio" in map:
    ...
    def get_prebuilt_image(self, name):
        retval = None
        if name in self._projects:
            retval = self._projects[name]
        return retval
