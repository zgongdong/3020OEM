...
/*!
  Report that the (background) erase operation for sector within
  RA Partition and containing the offset, is completed.
  The message content is a #MessageRaPartitionBgEraseStatus.
...
/*!
  Message type received when (background) erase operation for sector within
  RA Partition and containing the offset, is completed.
*/
typedef struct
{
    raPartition_result  status; /*!< Erase operation status on completion.  */
    uint32              offset; /*!< Offset within sector requested to be
                                     erased. */
