...
static void roleSelectionNextAfterDiscoverCharacteristics(GATT_ROLE_SELECTION_CLIENT *instance) 
{
    ...
    {
        if (   !instance->handle_state
            || !instance->handle_figure_of_merit
            || !instance->handle_role_control)
        {
                                               ...
                                               const GATT_MANAGER_DISCOVER_ALL_CHARACTERISTIC_DESCRIPTORS_CFM_T *cfm)
{
        ...
        && GATT_CLIENT_CHARACTERISTIC_CONFIGURATION_UUID == cfm->uuid[0])
    {
        ...
        {
            instance->handle_state_config = cfm->handle;
        }
        else if (role_selection_client_finding_notification_handle_fom == state)
        {
            instance->handle_figure_of_merit_config = cfm->handle;
        }
        else
        {
...
static const struct {
    ...
    { {UUID_128_FORMAT_gatt_uuid_t(UUID128_ROLE_SEL_MIRRORING_STATE)},     offsetof(GATT_ROLE_SELECTION_CLIENT, handle_state) },
                                                       ...
                                                       const GATT_MANAGER_DISCOVER_ALL_CHARACTERISTICS_CFM_T *cfm)
{
    ...
    if (    instance->handle_state
        && (instance->handle_state < cfm->handle) 
        && (cfm->handle <= instance->handle_state_end))
    {
        instance->handle_state_end = cfm->handle - 1;
    }

    if (    instance->handle_figure_of_merit
        && (instance->handle_figure_of_merit < cfm->handle) 
        && (cfm->handle <= instance->handle_figure_of_merit_end))
    {
        instance->handle_figure_of_merit_end = cfm->handle - 1;
    }
    ...
    if (!cfm->more_to_come)
    {
        ...
        switch (state)
        {
            case role_selection_client_finding_notification_handle:
                if (instance->handle_state_config)
                {
            ...
            case role_selection_client_finding_notification_handle_fom:
                if (instance->handle_figure_of_merit_config)
                {
