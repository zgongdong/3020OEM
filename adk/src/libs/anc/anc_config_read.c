...
#define ANC_TUNING_TOOL_MODE1_KEY (CONFIG_DSP(1))
#define ANC_TUNING_TOOL_MODE2_KEY (CONFIG_DSP(2))
...
{
    return ((PsRetrieve(ANC_TUNING_TOOL_MODE1_KEY, NULL, 0) == ANC_TUNING_TOOL_CONFIG_SIZE) &&
            (PsRetrieve(ANC_TUNING_TOOL_MODE2_KEY, NULL, 0) == ANC_TUNING_TOOL_CONFIG_SIZE));
}
...
bool ancConfigReadPopulateAncData(anc_config_t * config_data, anc_mode_t set_mode)
{
    ...
    readAncTuningToolModeConfig(ANC_TUNING_TOOL_MODE1_KEY, &config_data->mode1);
    readAncTuningToolModeConfig(ANC_TUNING_TOOL_MODE2_KEY, &config_data->mode2);
    return TRUE;
}
