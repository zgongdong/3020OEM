...
/****************************************************************************
NAME
    send_sm_add_device_at_tdl_pos_req

FUNCTION
    Send an internal message to Add the device and link keys at the specified
    position in the trusted device list. In this way messages can be queued
    until the previous DM_SM_ADD_DEVICE_CFM has been received.

    When this message is handled, it will call
    connectionAddDeviceAtTdlPosition(...).

RETURNS
    Nothing.
*/
static void send_sm_add_device_at_tdl_pos_req(uint8 position)
{
...
uint16 connectionInitTrustedDeviceList(void)
{
         ...
         i++)
    {
        if (PsRetrieve(TRUSTED_DEVICE_LIST + pos, td, PS_SIZE_ADJ(SIZE_TD_DATA_T)))
        {
            ...
            send_sm_add_device_at_tdl_pos_req(pos);
...
FUNCTION
    This function is called to replace the IRK of a device in the TDL
*/
            ...
            )
{
    ...
    CL_DEBUG(("connectionAuthReplaceIrk. x%04x td:%p",peer_bd_addr->lap, td));

    if (td != NULL)
    {
        DM_SM_KEY_ID_T old_irk;
        uint16 trusted;
        bool result;

        /* unpack keys */
        unpack_td_key((uint16 *)&old_irk, td, TDL_KEY_ID);

        trusted = td->content.trusted;
        
        /* Pack new IRK */
        td = pack_td_key(&td, (uint16 *)new_irk ,DM_SM_KEY_ID);
        td->content.trusted = trusted;

        /* store device back in TDL */
        if ( !PsStore(TRUSTED_DEVICE_LIST + pos, td, PS_SIZE_ADJ(CALC_TD_SIZE(td))) )
        {
            result = FALSE;
        }
        ...
        /* free TDL entry allocated in tdl_find_device */
        free(td);
        return result;
    }
...
{
    td_data_t *td = PanicUnlessMalloc(SIZE_TD_DATA_T);

    PanicZero(PsRetrieve(TRUSTED_DEVICE_LIST + pos, td, PS_SIZE_ADJ(SIZE_TD_DATA_T)));
    dm_sm_add_device_req(td);
    free(td);
}
...
/****************************************************************************
NAME
    ConnectionTrustedDeviceListDuplicate

FUNCTION
    This function duplicates an entry in the TDL, stores the new
...
{
    uint16    pos_existing, pos_new;    /* TRUSTED_DEVICE_LIST + pos */
    uint16    idx_existing, idx_new;    /* tdi.order[idx] */
    td_data_t *td_existing, *td_new;
    bool result = FALSE;
    
    if(BdaddrIsZero(existing_addr) || BdaddrIsZero(new_addr))
        return result;
    
    /* find existing device in TDL */
    td_existing = tdl_find_device(TYPED_BDADDR_PUBLIC, existing_addr, &pos_existing, &idx_existing);
    if(td_existing == NULL)
        return result;  /* no existing device found, nothing to duplicate */

    /* if the new address already exists then delete it */
    td_new = tdl_find_device(TYPED_BDADDR_PUBLIC, new_addr, &pos_new, &idx_new);
    if(td_new != NULL)
    {
        delete_from_trusted_device_list(pos_new, idx_new);
        free(td_new); /* free TDL entry allocated in tdl_find_device */
    }

    /* Update the bdaddr in the copy of the existing device */      
    pack_td_bdaddr(td_existing, TYPED_BDADDR_PUBLIC, new_addr);
    
    /* Update the trusted device list in the new position */
    if ( PsStore(TRUSTED_DEVICE_LIST + pos_new, td_existing, PS_SIZE_ADJ(CALC_TD_SIZE(td_existing))) )
    {
        /* Set hash of new device for more efficient list searches */
        GET_TDI_CACHE.element[idx_new].hash = TDI_HASH(*new_addr);

        /* Now the hash is updated, moved to top of list */
        update_trusted_device_index(pos_new, idx_new);

        /* Update the device in the Bluestack Security Manager database */
        dm_sm_add_device_req(td_existing);
    
        result = TRUE;
    }

    /* free TDL entry allocated in tdl_find_device for the existing device */
    free(td_existing);
    return result;
