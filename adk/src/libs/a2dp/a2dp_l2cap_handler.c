...
static a2dp_status_code convertDisconnectStatusCode(l2cap_disconnect_status l2cap_status)
{
    ...
    case l2cap_disconnect_transferred:
        return a2dp_disconnect_transferred;
...
/*****************************************************************************/
remote_device* a2dpAddDevice (const bdaddr *addr)
{
    uint8 i;
...
static bool completeMedia (media_channel *media, uint16 connection_id, Sink sink, uint16 mtu)
{
    ...
    if (media != NULL)
    {
        ...
        {   /* Finalise channel information */
            ...
            media->connection.active.sink = sink;
            media->connection.active.bitfields.mtu = MIN(DEFAULT_L2CAP_LOCAL_MTU_MAXIMUM, mtu);
        ...
        if (media->status.conn_info.connection_state == avdtp_connection_connected)
        {   /* Update the stored MTU due to an L2CAP parameter renegotiation */
            media->connection.active.bitfields.mtu = mtu;
            return TRUE;
        }
    }
...
void a2dpHandleSignallingConnectReq(const A2DP_INTERNAL_SIGNALLING_CONNECT_REQ_T *req)
{
    ...
    {   /* Outgoing request to a new device */
        if ( (device=a2dpAddDevice(&req->addr))!=NULL )
        {
            device->clientTask = req->clientTask;
...
void a2dpHandleL2capConnectInd(const CL_L2CAP_CONNECT_IND_T *ind)
{
    ...
    if(ind->psm == AVDTP_PSM)
    {
        ...
        {   /* Incoming request from a new device */
            if ( (device=a2dpAddDevice(&ind->bd_addr))!=NULL )
...
void a2dpHandleL2capConnectCfm(const CL_L2CAP_CONNECT_CFM_T *cfm)
{
    ...
    if (device != NULL)
    {
        ...
        {
        case avdtp_connection_idle:
            /* Only valid reason to get here is if the ACL to the A2DP source is closed whilst the media
             * channel is being opened. Check if this is the case.
             */
            if ( (media = findMediaFromConnectionId(device, cfm->connection_id)) != NULL)
            {
                if (media->status.conn_info.connection_state == avdtp_connection_disconnect_pending)
                {
                    /* Let the application know the media channel opening failed. */
                    a2dpMediaOpenCfm(device, media, a2dp_l2cap_fail);

                    /* Proceed to tear down any signalling channel and device structs. */
                    a2dpSignallingDisconnectInd(device, convertDisconnectStatusCode(l2cap_disconnect_successful));
