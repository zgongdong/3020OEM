                                        ...
                                        Message goal_data)
{
    ...
    if (appDeviceGetPrimaryBdAddr(&peer_addr))
    {
        ...
        proc_start_cfm_fn(tws_topology_procedure_sec_connect_peer, proc_result_success);
        ...
        if (!td->connect_wait_lock)
        {
            ...
            TwsTopology_DelayedCompleteCfmCallback(proc_complete_fn, tws_topology_procedure_sec_connect_peer, proc_result_success);
        }
        else
        {
...
void TwsTopology_ProcedureSecConnectPeerCancel(twstop_proc_cancel_cfm_func_t proc_cancel_cfm_fn)
{
    ...
    TwsTopology_DelayedCancelCfmCallback(proc_cancel_cfm_fn, tws_topology_procedure_sec_connect_peer, proc_result_success);
}
...
static void twsTopology_ProcSecConnectPeerCheckAclStatus(bool timeout)
{
    ...
    if (appDeviceGetPrimaryBdAddr(&peer_addr))
    {
        ...
        {
            td->complete_fn(tws_topology_procedure_sec_connect_peer, proc_result_success);
        }
        else
        {
            td->complete_fn(tws_topology_procedure_sec_connect_peer,
                            timeout ? proc_result_timeout : proc_result_failed);
        }
    }
    else
    {
