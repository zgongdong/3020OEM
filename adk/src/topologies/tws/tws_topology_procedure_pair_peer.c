...
void TwsTopology_ProcedurePairPeerStart(Task result_task,
                                        ...
                                        Message goal_data);
void TwsTopology_ProcedurePairPeerCancel(twstop_proc_cancel_cfm_func_t proc_cancel_cfm_fn);

const tws_topology_procedure_fns_t proc_pair_peer_fns = {
                                        ...
                                        Message goal_data)
{
    ...
    DEBUG_LOG("TwsTopology_ProcedurePairPeerStart");
    ...
    proc_start_cfm_fn(tws_topology_procedure_pair_peer, proc_result_success);
}
...
void TwsTopology_ProcedurePairPeerCancel(twstop_proc_cancel_cfm_func_t proc_cancel_cfm_fn)
{
    ...
    TwsTopology_DelayedCancelCfmCallback(proc_cancel_cfm_fn, tws_topology_procedure_pair_peer, proc_result_failed);
}
...
static void twsTopology_ProcPairPeerHandlePeerPairLePairCfm(const PEER_PAIR_LE_PAIR_CFM_T *cfm)
{
    ...
    DEBUG_LOG("twsTopology_ProcPairPeerHandlePeerPairLePairCfm %u", cfm->status);
    ...
    TwsTopProcPairPeerGetTaskData()->complete_fn(tws_topology_procedure_pair_peer, proc_result_success);
}
