                                        ...
                                        Message goal_data)
{
    ...
    proc_start_cfm_fn(tws_topology_procedure_disconnect_le_connections, proc_result_success);
...
void TwsTopology_ProcedureDisconnectLeConnectionsCancel(twstop_proc_cancel_cfm_func_t proc_cancel_cfm_fn)
{
    ...
    td->active = FALSE;
    TwsTopology_DelayedCancelCfmCallback(proc_cancel_cfm_fn, tws_topology_procedure_disconnect_le_connections, proc_result_success);
}
...
static void twsTopology_ProcDisconnectLeConnectionsHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        case CON_MANAGER_DISCONNECT_ALL_LE_CONNECTIONS_CFM:
            td->active = FALSE;
            TwsTopology_DelayedCompleteCfmCallback(td->complete_fn, tws_topology_procedure_disconnect_le_connections, proc_result_success);
            break;
