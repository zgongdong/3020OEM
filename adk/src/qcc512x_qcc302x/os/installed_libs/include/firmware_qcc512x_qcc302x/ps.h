...
PSKEY_UPGRADE      Storage reserved for upgrade library on P1
...
and defragmentation.
New applications should consider use the PsDefragBlocking() trap instead, but
for compatibility with legacy code PsFlood() is still defined and points at
PsDefrag().
*/
 ...
 *   the store content, system load and flash chip performance. See
 *   PsDefragBlocking() for a version of this trap that returns once the
 *   defragmentation operation is complete.
 * 
...
/**
 *  \brief  Give the next Apps key stored in PsStore 
 *  \param value  0 for start from the begining of the store, 1 for the
 *               next key.
 *             
 *  \param key             This is the location where to save the next appkey found in the
 *             store (or 0 if all Appkeys have been read.)
 *             The first appkey stored is returned when the reset of the
 *             function is called: PsNextKey(0, key)
 *             
 *  \return             Return True if there are still Appkeys in PsStore.
 *             Return False if the Appkey previously read was the last one.
 *           
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
 * \ingroup trapset_core
 */
bool PsNextKey(uint16 value, uint16* key);

/**
 *  \brief  Give the next Audio key stored in the PsStore. 
 *  \param value  0 for start the search from the begining of the store
 *               1 to have the next key.
 *             
 *  \param key             This is the location where to save the next Audiokey found in the
 *             store (or 0 if all Audiokeys have been read.)
 *             The first Audiokey stored is returned when the reset of the
 *             function is called: PsNextAudioKey(0, key)
 *             
 *  \return             Return True if there are still Audiokeys in PsStore.
 *             Return False if the Audiokey previously read was the last one.
 *           
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
 * \ingroup trapset_core
 */
bool PsNextAudioKey(uint16 value, uint32* key);

/**
 *  \brief Defragment the PS store
 *   Changing a key with PsStore() trap adds a new instance of the key to PS store.
 *   After some time the store becomes filled with multiple instances of the same
 *   keys that consume space. To remove the old data and recover space in
 *   the store, defragmentation needs to be performed from time to time.
 *   Defragmentation is started automatically if after a PsStore request the store
 *   is more than 90% full or can be started manually at any time using this trap
 *   or the PsDefrag() trap.
 *   For the reference with 64KB store, PsFreeCount(16) returns 142 when
 *   store is 90% full and 1055 when store is 90% empty.
 *   This trap will run defragmentation in a blocking manner, i.e. upon calling
 *  this
 *   trap, control will not be returned to the application until defragmentation is
 *   complete. Defragmentation is a time and memory intensive operation, it is
 *   recommended that this trap only be called if it's acceptable for the
 *   application to be blocked for several seconds.
 *   See PsDefrag() for the non-blocking version of this trap.
 *         
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
