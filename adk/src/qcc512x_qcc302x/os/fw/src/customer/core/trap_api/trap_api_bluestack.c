...
uint8 VmGetAclMode(const tp_bdaddr *acl_address)
{
    ...
    ipc_send(IPC_SIGNAL_ID_VM_GET_ACL_MODE, &send, sizeof(send));
    (void)ipc_recv(IPC_SIGNAL_ID_VM_GET_ACL_MODE_RSP, &rsp);
    return rsp.ret;
}
...
uint8 VmGetAclRole(const tp_bdaddr *acl_address)
{
    ...
    IPC_UINT8_RSP rsp;
    send.acl_address = (const TP_BD_ADDR_T *)acl_address;
    ipc_send(IPC_SIGNAL_ID_VM_GET_ACL_ROLE, &send, sizeof(send));
    (void)ipc_recv(IPC_SIGNAL_ID_VM_GET_ACL_ROLE_RSP, &rsp);
    return rsp.ret;
}
