...
struct rule_set_tag
{
    ...
    task_list_with_data_t event_tasks;
    ...
    for (rule_index = 0; rule_index < rule_set->rules_count; rule_index++)
    {
             ...
             rule->flags == rule_flag_always_evaluate)
        {
                ...
                (action == rule_action_run_with_param))
            {
                ...
                while (TaskList_IterateWithData(TaskList_GetBaseTaskList(&rule_set->event_tasks), &task, &data))
                {
...
rule_set_t RulesEngine_CreateRuleSet(const rule_set_init_params_t *params)
{
    ...
    TaskList_WithDataInitialise(&rule_set->event_tasks);
...
{
    Task next_client = (Task)0;
    task_list_data_t data = {0};
    task_list_t *event_list = TaskList_GetBaseTaskList(&rule_set->event_tasks);

    while (TaskList_IterateWithData(event_list, &next_client, &data))
    {
        TaskList_RemoveTask(event_list, next_client);
        next_client = 0;
    }
...
void RulesEngine_SetEvent(rule_set_t rule_set, Task client_task, rule_events_t event_mask)
{
    ...
    if (TaskList_GetDataForTask(event_list, client_task, &data))
    {
        data.u64 |= event_mask;
        TaskList_SetDataForTask(event_list, client_task, &data);
    }
    else
    {
        data.u64 |= event_mask;
        TaskList_AddTaskWithData(event_list, client_task, &data);
    }
...
void RulesEngine_ResetEvent(rule_set_t rule_set, rule_events_t event)
{
    ...
    while (TaskList_IterateWithData(TaskList_GetBaseTaskList(&rule_set->event_tasks), &iter_task, &data))
    {
        if ((data.u64 & event) == event)
        {
            ...
            if (data.u64)
            {
                TaskList_SetDataForTask(event_list, iter_task, &data);
            }
            else
            {
                TaskList_RemoveTask(event_list, iter_task);
            }
        }
    }
}
