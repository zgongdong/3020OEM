class CMockGeneratorPluginReturnThruPtr
  ...
  def mock_function_declarations(function)
    ...
    function[:args].each do |arg|
      if (@utils.ptr_or_str?(arg[:type]) and not arg[:const?])
        ...
        lines << "void #{function[:name]}_CMockReturnMemThruPtr_#{arg[:name]}(UNITY_LINE_TYPE cmock_line, const char* cmock_file, #{arg[:type]} #{arg[:name]}, int cmock_size);\n"
      end
    end
    lines
  end
  ...
  def mock_interfaces(function)
    ...
    function[:args].each do |arg|
      arg_name = arg[:name]
      if (@utils.ptr_or_str?(arg[:type]) and not arg[:const?])
        lines << "void #{func_name}_CMockReturnMemThruPtr_#{arg_name}(UNITY_LINE_TYPE cmock_line, const char* cmock_file, #{arg[:type]} #{arg_name}, int cmock_size)\n"
        lines << "{\n"
        ...
        lines << "  UNITY_TEST_ASSERT_NOT_NULL(cmock_call_instance, cmock_line, cmock_file, CMockStringPtrPreExp);\n"
  ...
  def mock_implementation(function)
    ...
    function[:args].each do |arg|
      ...
      if (@utils.ptr_or_str?(arg[:type]) and not arg[:const?])
        ...
        lines << "  {\n"
        lines << "    UNITY_TEST_ASSERT_NOT_NULL(#{arg_name}, cmock_line, cmock_file, CMockStringPtrIsNULL);\n"
