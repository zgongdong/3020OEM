...
bool A2dpDeviceFromBdaddr (const bdaddr *addr, uint8 *device_id)
{
    ...
    if (!addr || !device_id || !a2dp)
    {
        return FALSE;
    }

    for (dev = 0; dev < A2DP_MAX_REMOTE_DEVICES; dev++)
    {
        if (BdaddrIsSame(addr, &a2dp->remote_conn[dev].bd_addr) &&
            a2dp->remote_conn[dev].signal_conn.status.connection_state != avdtp_connection_idle)
        {
            *device_id = dev;
            return TRUE;
        }
