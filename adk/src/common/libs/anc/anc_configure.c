...
/******************************************************************************/
static void updateMicGain(const audio_mic_params mic_params)
{
    Source anc_mic = AudioPluginGetMicSource(mic_params, mic_params.channel);

    if(mic_params.is_digital)
    {
        ANC_ASSERT(SourceConfigure(anc_mic, STREAM_DIGITAL_MIC_INPUT_GAIN, ancDataGetAdcGain(mic_params.channel)));
    }
    else
    {
        ANC_ASSERT(SourceConfigure(anc_mic, STREAM_CODEC_RAW_INPUT_GAIN, calculateCodecRawInputGain(mic_params.channel)));
    }
}
...
DESCRIPTION
    ...
    microphone channel.
*/
static void updateSidetoneGain(const audio_mic_params mic_params)
{
    Source anc_mic = AudioPluginGetMicSource(mic_params, mic_params.channel);
...
/******************************************************************************/
static void configureMicChannel(audio_mic_params path)
{
	Source mic_src = AudioPluginMicSetup(path.channel, path, ancDataGetMicSampleRate());
	updateMicGain(path);
...
static void configureMics(void)
{
    ...
    {
        configureMicChannel(mic_params->feed_forward_left);
    }

    if (mic_params->enabled_mics & feed_forward_right)
   	{
        configureMicChannel(mic_params->feed_forward_right);
   	}
...
static void closeMics(void)
{
    ...
    if (mic_params->enabled_mics & feed_forward_left)
    {
        AudioPluginMicShutdown(mic_params->feed_forward_left.channel, &mic_params->feed_forward_left, TRUE);
    }
    if (mic_params->enabled_mics & feed_forward_right)
    {
        AudioPluginMicShutdown(mic_params->feed_forward_right.channel, &mic_params->feed_forward_right, TRUE);
    }
}
...
static bool enableFilter(bool enable)
{
    ...
    if (mic_params->enabled_mics & feed_forward_left)
    {
   	    success = CodecSetIirFilter16Bit(getChannelMask(mic_params->feed_forward_left.channel, mic_params->feed_forward_left),
    	                                      enable, ancDataGetChannelCoefficients(mic_params->feed_forward_left.channel));
    }

    if (mic_params->enabled_mics & feed_forward_right)
    {
        success = CodecSetIirFilter16Bit(getChannelMask(mic_params->feed_forward_right.channel, mic_params->feed_forward_right),
                                      enable, ancDataGetChannelCoefficients(mic_params->feed_forward_right.channel));
    }
...
void ancConfigureFinetuneGains(void)
{
    ...
    if (mic_params->enabled_mics & feed_forward_left)
    {
        ...
        {
            updateMicGain(mic_params->feed_forward_left);
	    }
    }
    ...
    if (mic_params->enabled_mics & feed_forward_right)
    {
        ...
        {
	        updateMicGain(mic_params->feed_forward_right);
        }
    }
}
...
void ancConfigureSidetoneGains(void)
{
    ...
    if (mic_params->enabled_mics & feed_forward_left)
    {
        updateSidetoneGain(mic_params->feed_forward_left);
    }

    if (mic_params->enabled_mics & feed_forward_right)
    {
        updateSidetoneGain(mic_params->feed_forward_right);
    }
}
