...
{
    if(ancDataGetMode() == anc_mode_1)
    {
        return &ancDataGetConfigData()->mode1;
    }
    else if(ancDataGetMode() == anc_mode_2)
    {
        return &ancDataGetConfigData()->mode2;
    }
    ANC_PANIC();
    return NULL;
}

void ancDataRetrieveAndPopulateTuningData(anc_mode_t set_mode)
{
    ancConfigReadPopulateAncData(ancDataGetConfigData(), set_mode);
}
