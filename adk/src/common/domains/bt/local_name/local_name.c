...
static void localName_StoreName(CL_DM_LOCAL_NAME_COMPLETE_T *message)
{
    ...
    {
        uint16 size_name = message->size_local_name + LOCAL_NAME_SIZE_LE_PREFIX;

        free(local_name_task_data.name);
        local_name_task_data.name = PanicUnlessMalloc(size_name + 1);
        ...
        local_name_task_data.name[size_name] = '\0';
...
uint8 *LocalName_GetName(void)
{    
    return LocalName_GetPrefixedName() + LOCAL_NAME_SIZE_LE_PREFIX;
}


uint8 *LocalName_GetPrefixedName(void)
{
