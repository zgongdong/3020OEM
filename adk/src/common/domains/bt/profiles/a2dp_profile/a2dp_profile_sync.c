...
#define A2DP_SYNC_LOG(x)        DEBUG_LOG(x)
#define A2DP_SYNC_LOGF(x, ...)  DEBUG_LOGF(x, __VA_ARGS__)
/*! \} */
                                                       ...
                                                       const AUDIO_SYNC_CONNECT_IND_T *ind)
{
    ...
    A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncConnectIndication(%p) state(0x%x)", theInst, local_state);
                                                     ...
                                                     const AUDIO_SYNC_CONNECT_RES_T *res)
{
    ...
    if (((res->sync_id + 1) & 0xff) != theInst->a2dp.sync_counter)
    {
        ...
        A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncConnectResponse(%p) late state(0x%x) sync_id(%d) sync_count(%d)",
            theInst, local_state, res->sync_id, theInst->a2dp.sync_counter);
        return;
    }

    A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncConnectResponse(%p) state(0x%x) sync_id(%d)",
                                                        ...
                                                        const AUDIO_SYNC_ACTIVATE_IND_T *ind)
{
    ...
    A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncActivateIndication(%p) state(0x%x)", theInst, local_state);
    ...
    default:
        A2DP_SYNC_LOGF("appA2dpHandleInternalA2dpInstSyncIndication(%p) unhandled state 0x%x",
                       theInst, local_state);
        break;
    }
                                                      ...
                                                      const AUDIO_SYNC_ACTIVATE_RES_T *res)
{
    ...
    if (((res->sync_id + 1) & 0xff) != theInst->a2dp.sync_counter)
    {
        ...
        something else triggered the instance to exit the _SYNC state. So this sync
        response is late, and now irrelevant. */
        A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncActivateResponse(%p) late state(0x%x) sync_id(%d) sync_count(%d)",
                       theInst, local_state, res->sync_id, theInst->a2dp.sync_counter);
        return;
    }

    A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncActivateResponse(%p) state(0x%x) sync_id(%d)",
                   theInst, local_state, res->sync_id);
                                                     ...
                                                     const AUDIO_SYNC_STATE_IND_T *ind)
{
    ...
    A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncStateIndication(%p) local_state(0x%x) state(0x%x)", theInst, local_state, ind->state);
    ...
    case AUDIO_SYNC_STATE_CONNECTED:
        ...
        default:
            A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncStateIndication(%p) unhandled state 0x%x",
                           theInst, local_state);
            break;
        }
        break;
                                                                 ...
                                                                 const AUDIO_SYNC_CODEC_RECONFIGURED_IND_T *ind)
{
    ...
    A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncCodecReconfiguredIndication(%p) state(0x%x)", theInst, appA2dpGetState(theInst));
    ...
    default:
        A2DP_SYNC_LOGF("appA2dpSyncHandleA2dpSyncCodecReconfiguredIndication(%p) unhandled state 0x%x",
                       theInst, local_state);
        break;
    }
...
{
    avA2dpState local_state = appA2dpGetState(theInst);
    DEBUG_LOGF("appA2dpSyncRegister(%p) sync_if(%p) state(0x%x)",
...
void appA2dpSyncHandleMessage(avInstanceTaskData *theInst, MessageId id, Message message)
{
    ...
    default:
        A2DP_SYNC_LOGF("appA2dpSyncHandleMessage unhandled msg id 0x%x", id);
        break;
    }
}
