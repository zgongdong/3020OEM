...
static void ScoFwdPlayRingAtWcTime(rtime_t sync_time)
{
    ...
    DEBUG_LOG("ScoFwdPlayRingAtWcTime");
    ...
    if (RtimeWallClockGetStateForSink(&wc_state, theScoFwd->wallclock_sink))
    {
        ...
        if (RtimeWallClock24ToLocal(&wc_state, sync_time, &local_rtime))
        {
            /* Convert to milliseconds in the future */
            int32_t delay = rtime_sub(local_rtime, VmGetTimerTime()) / 1000;

            /* If value is negative, do not ring */
            if(delay < 0)
            {
                DEBUG_LOG("ScoFwdHandleOTARing delay is negative, don't ring.");
                return;
            }

            DEBUG_LOGF("ScoFwdHandleOTARing sync_time: %u, delay: %u", sync_time, delay);
...
static void scoFwdProfile_SetScoFwdWasConnected(const bdaddr *bd_addr, bool connected)
{
    ...
    if (device)
    {
        ...
        DEBUG_LOGF("scoFwdProfile_SetScoFwdWasConnected, connected %02x", connected_profiles);
...
{
    MessageSend(appInitGetInitTask(), SFWD_INIT_CFM, NULL);
}
...
static void ScoFwdSetState(scoFwdState new_state)
{
    ...
    DEBUG_LOGF("ScoFwdSetState(%d) from %d", new_state, old_state);
...
{
    DEBUG_LOGF("ScoFwdHandleL2capRegisterCfm, status %u, psm %u", cfm->status, cfm->psm);
...
{
    DEBUG_LOGF("ScoFwdHandleClSdpRegisterCfm, status %d", cfm->status);
    PanicFalse(ScoFwdGetState() == SFWD_STATE_INITIALISING);
...
{
    scoFwdTaskData *theScoFwd = GetScoFwd();
    DEBUG_LOGF("ScoFwdHandleClSdpServiceSearchAttributeCfm, status %d", cfm->status);
...
{
    scoFwdTaskData *theScoFwd = GetScoFwd();
    DEBUG_LOGF("ScoFwdHandleL2capConnectInd, state %u, psm %u", ScoFwdGetState(), ind->psm);
    ...
    if (appDeviceIsPeer(&ind->bd_addr))
    {
        switch (ScoFwdGetState())
        {
            ...
            case SFWD_STATE_CONNECTING:
                ...
                DEBUG_LOGF("ScoFwdHandleL2capConnectInd, crossover, accept %u", accept);
                break;
...
{
    scoFwdTaskData *theScoFwd = GetScoFwd();
    DEBUG_LOGF("ScoFwdHandleL2capConnectCfm, status %u, pending %u", cfm->status, theScoFwd->pending_connects);
    ...
    switch (ScoFwdGetState())
    {
        case SFWD_STATE_CONNECTING:
            ...
            {
                DEBUG_LOGF("ScoFwdHandleL2capConnectCfm, connected, conn ID %u, flush remote %u", cfm->connection_id, cfm->flush_timeout_remote);
        ...
        default:
            ...
            DEBUG_LOGF("ScoFwdHandleL2capConnectCfm, failed, pending %u", theScoFwd->pending_connects);
            break;
    }
}
...
{
    scoFwdTaskData *theScoFwd = GetScoFwd();
    DEBUG_LOGF("ScoFwdHandleL2capDisconnectInd, status %u", ind->status);
...
{
    UNUSED(cfm);
    DEBUG_LOGF("ScoFwdHandleL2capDisconnectCfm, status %u", cfm->status);
...
static void ScoFwdHandleStopReceiveChain(void)
{
    ...
    {
        DEBUG_LOGF("ScoFwdHandleStopChain Asked to stop when not active. State %d",ScoFwdGetState());
    }
}
...
{
    DEBUG_LOGF("ScoFwdHandleEnableForwarding SFWD state %u", ScoFwdGetState());
...
static void ScoFwdHandleDisableForwarding(void)
{
    ...
    DEBUG_LOGF("ScoFwdHandleDisableForwarding SFWD state %u", ScoFwdGetState());
...
{
    DEBUG_LOGF("ScoFwdHandlePhyStateChangedInd new state %u", ind->new_state);
...
static void ScoFwdHandleHfpVolumeInd(const APP_HFP_VOLUME_IND_T *ind)
{
    ...
    if (theScoFwd->forward_volume)
    {
        if (ScoFwdIsStreaming())
        {
            DEBUG_LOGF("ScoFwdHandleHfpVolumeInd volume %u", ind->volume);
...
{
    scoFwdTaskData *theScoFwd = GetScoFwd();
    DEBUG_LOGF("ScoFwdHandlePeerSignallingConnectionInd, status %u", ind->status);
...
static void ScoFwdHandleScoConnected(const APP_HFP_CONNECTED_IND_T *connected)
{
    ...
    DEBUG_LOGF("ScoFwdHandleScoConnected: SCO Fwd handling volume %d",!tws_plus_device);
...
static void ScoFwdHandleMessage(Task task, MessageId id, Message message)
{
    ...
    {
        DEBUG_LOGF("**** ScoFwdHandleMessage: 0x%x (%d)",id,id);
    }
#endif
    ...
    switch (id)
    {
        ...
        default:
            DEBUG_LOGF("ScoFwdHandleMessage. UNHANDLED Message id=x%x (%d). State %d", id, id, ScoFwdGetState());
            break;
    }
}
...
bool ScoFwdIsCallIncoming(void)
{
    ...
    DEBUG_LOGF("ScoFwdIsCallIncoming: Checking for incoming call (%d)",theScoFwd->peer_incoming_call);
...
void ScoFwdRing(void)
{
    ...
    {
        DEBUG_LOGF("ScoFwdRing, wc_state %u", wc_state);
...
void ScoFwdRingCancel(void)
{
    scoFwdTaskData *theScoFwd = GetScoFwd();
    if(MessageCancelAll(&theScoFwd->task, SFWD_INTERNAL_PLAY_RING))
    {
        DEBUG_LOG("ScoFwdRingCancel, Pending message to RING cancelled");
    }
}
...
*/
void ScoFwdHandleHfpAudioConnectConfirmation(const HFP_AUDIO_CONNECT_CFM_T *cfm)
{
    DEBUG_LOGF("ScoFwdHandleHfpAudioConnectConfirmation %u", cfm->status);

    HfpProfile_StoreConnectParams(cfm);
    Telephony_NotifyCallAudioConnected(HfpProfile_VoiceSourceDeviceMappingGetSourceForIndex(cfm->priority));
...
{
    UNUSED(ind);
    DEBUG_LOGF("ScoFwdHandleHfpAudioDisconnectIndication %u", ind->status);
    ...
    if (appConfigScoForwardingEnabled())
    {
        switch (ScoFwdGetState())
        {
            ...
            default:
                DEBUG_LOGF("ScoFwdHandleHfpAudioDisconnectIndication unexpected SFWD state %u", ScoFwdGetState());
                break;
        }
    }
