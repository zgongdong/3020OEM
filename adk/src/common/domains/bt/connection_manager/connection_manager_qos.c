...
/******************************************************************************/
static const ble_connection_params* conManagerGetParamsToUse(cm_qos_t qos)
{
    return cm_qos_params[qos];
}

/******************************************************************************/
static void conManagerSendParameterUpdate(const tp_bdaddr* tpaddr, cm_qos_t qos)
{
    DEBUG_LOG("conManagerSendParameterUpdate %d", qos);
    ...
    if(tpaddr && tpaddr->transport == TRANSPORT_BLE_ACL)
    {
        ...
        {
            const ble_connection_params* params = conManagerGetParamsToUse(qos);
