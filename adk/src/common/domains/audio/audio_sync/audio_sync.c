...
#define AUDIO_SYNC_LOG(x)         DEBUG_LOG(x)
#define AUDIO_SYNC_LOGF(x, ...)   DEBUG_LOGF(x, __VA_ARGS__)
/*! \} */
                                 ...
                                 uint8 seid, uint8 sync_id)
{
    ...
    AUDIO_SYNC_LOGF("AudioSync_ConnectIndication sync_inst %p replay_task %p",
    ...
    if (sync_inst)
    {
        ...
        /* Allow 500ms for sync, after that the calling task will carry on */
        delay = 500;
    }
                                  ...
                                  uint8 seid, uint8 sync_id)
{
    ...
    AUDIO_SYNC_LOGF("AudioSync_ActivateIndication sync_inst %p replay_task %p",
                    sync_inst, reply_task);
                               ...
                               audio_sync_state_t state, uint8 seid)
{
    if (sync_inst)
    {
        AUDIO_SYNC_LOGF("AudioSync_StateIndication sync_inst %p state 0x%x",
                        sync_inst, state);
                                           ...
                                           uint8 seid, uint8 device_id, uint8 stream_id)
{
    if (sync_inst)
    {
        AUDIO_SYNC_LOGF("AudioSync_CodecReconfiguredIndication sync_inst %p seid 0x%x d_id %u s_id %u",
