...
static void stateProxy_UpdateActiveHandsetAddr(void)
{
    ...
    bdaddr peer_addr;
    uint16 handset_tws = DEVICE_TWS_UNKNOWN;
    ...
    proxy->local_state->handset_tws = handset_tws;
        ...
        appDeviceGetPeerBdAddr(&peer_addr))
    {
        ...
        msg->handset_tws_version = handset_tws;
...
static void stateProxy_FlagChangesInState(state_proxy_data_t * new_state, state_proxy_data_t * previous_state)
{
    ...
    if(new_state->flags.in_ear != previous_state->flags.in_ear)
    {
        ...
        message->new_state = PHY_STATE_IN_EAR;
        message->event = phy_state_event_in_ear;
        stateProxy_HandleRemotePhyStateChangedInd(message);
    }
}
 ...
 *******************************************/
/*! \brief Send state proxy version to peer.
*/
static void stateProxy_SendVersion(uint16 version)
{
    bdaddr peer_addr;
    if (appDeviceGetPeerBdAddr(&peer_addr))
    {
        state_proxy_version_t* msg = PanicUnlessMalloc(sizeof(state_proxy_version_t));

        msg->version = version;

        appPeerSigMarshalledMsgChannelTx(stateProxy_GetTask(),
                PEER_SIG_MSG_CHANNEL_STATE_PROXY,
                msg, MARSHAL_TYPE_state_proxy_version_t);
    }
    else
    {
        DEBUG_LOG("stateProxy_SendVersion no peer to send to");
    }
}
...
static void stateProxy_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CL_DM_RSSI_BDADDR_CFM:
            stateProxy_HandleClDmRssiBdaddrCfm((const CL_DM_RSSI_BDADDR_CFM_T *)message);
            break;
...
{
    return stateProxy_GetRemoteData()->handset_tws == DEVICE_TWS_VERSION;
}
