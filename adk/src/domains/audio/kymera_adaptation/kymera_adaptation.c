...
static void kymeraAdaptation_ConnectVoice(connect_parameters_t * params)
{
    if(params->source_params.data_length == sizeof(voice_connect_parameters_t))
    {
        ...
        {
            DEBUG_LOGF("kymeraAdaptation_ConnectVoice: forwarding requested but no chain to support forwarding exists");
            Panic();
        }
    }
    else
...
static void kymeraAdaptation_ConnectAudio(connect_parameters_t * params)
{
    if(params->source_params.data_length == sizeof(audio_connect_parameters_t))
    {
        ...
        a2dp_codec_settings codec_settings = { .rate = connect_params->rate, .channel_mode = connect_params->channel_mode,
                                                .seid = connect_params->seid, .sink = connect_params->sink,
                                                .codecData = { .content_protection = connect_params->content_protection,
                                                        .bitpool = connect_params->bitpool, .format = connect_params->format,
                                                        .packet_size = connect_params->packet_size } };
        ...
        appKymeraA2dpStart(connect_params->client_lock, connect_params->client_lock_mask,
                            &codec_settings, volume_in_db, connect_params->master_pre_start_delay);
    }
    else
    {
        Panic();
