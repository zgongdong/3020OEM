...
void Usb_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        default:
            DEBUG_LOGF("USB_COMMON Message handler, id %x", id);
            break;
    }
}
...
void Usb_TimeCriticalInit(void)
{
    ...
    if (usbConfigAncTuningEnabled())
    {
        ...
        DEBUG_LOGF("Usb_TimeCriticalInit configuration status is %d",status);
        ...
        DEBUG_LOGF("Usb_TimeCriticalInit Enumeration status is %d",status);
        if (status != usb_device_class_status_success)
        {
            Panic();
        }
...
void Usb_AttachtoHub(void)
{
    if(!UsbHubAttach())
    {
        DEBUG_LOGF("USB_AttachtoHub failed");
        Panic();
    }
}
...
void Usb_DetachFromHub(void)
{
    if(!UsbHubDetach())
    {
        DEBUG_LOGF("Usb_DetachFromHub failed");
        Panic();
    }
}
