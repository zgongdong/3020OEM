...
static void appHfpExitConnectedIncoming(void)
{
     ...
     *  This can now be indicated via the APP_HFP_SCO_INCOMING_ENDED_IND */
...
static void appHfpSetState(hfpState state)
{
    ...
    Ui_InformContextChange(ui_provider_hfp, state);
    ...
    switch (appHfpGetState())
    {
        case HFP_STATE_INITIALISING_HFP:
        {
            ...
            if (cfm->status == hfp_init_success)
            {
                ...
                MessageSend(appInitGetInitTask(), APP_HFP_INIT_CFM, 0);
            }
            else
                Panic();
        }
    ...
    switch (appHfpGetState())
    {
        ...
        case HFP_STATE_DISCONNECTING:
        {
            ...
            if (cfm->status == hfp_audio_connect_success)
            {
                ...
                ScoFwdHandleHfpAudioConnectConfirmation(cfm);
...
static bool hfpProfile_IsPeerRingForwardSupported(void)
{
    ...
    if (appConfigScoForwardingEnabled())
    {
        ...
        if (device)
        {
            ...
            return sco_fwd_features & SFWD_FEATURE_RING_FWD;
        }
        else
            return FALSE;
    }
...
static void appHfpHandleHfpRingIndication(const HFP_RING_IND_T *ind)
{
     ...
     * - the peer is in ear
    */
    bool ring_fwd = appConfigScoForwardingEnabled() && hfpProfile_IsPeerRingForwardSupported() && StateProxy_IsPeerInEar();
