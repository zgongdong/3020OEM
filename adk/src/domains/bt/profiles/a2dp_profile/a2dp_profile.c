...
a2dpTaskData * A2dpProfile_GetHandsetData(void)
{
    ...
    if(appDeviceGetHandsetBdAddr(&address))
    {
        DEBUG_LOG("A2dpProfile_GetHandsetData handset exists");
        avInstanceTaskData *appAvInstance = appAvInstanceFindFromBdAddr(&address);
        if (appAvInstance)
            a2dp = &(appAvInstance->a2dp);
    }
    else
    {
        DEBUG_LOG("A2dpProfile_GetHandsetData handset doesn't exist");
    }
    DEBUG_LOG("A2dpProfile_GetHandsetData =%p", a2dp);
    return a2dp;
}
...
static void disconnectPeerSource(avInstanceTaskData *theInst)
{
    ...
    {
        DEBUG_LOGF("appA2dpStopAudio(%p) no codec_settings not starting kymera", (void *)theInst);
    }
}
...
{
    DEBUG_LOGF("appA2dpStartAudio(%p), device_id=%d, stream_id=%d", (void *)theInst, theInst->a2dp.device_id, theInst->a2dp.stream_id);
    ...
    {
        /* Send message with audio_source_a2dp as a parameter and let the service do the connection */
        MESSAGE_MAKE(media, AV_A2DP_AUDIO_CONNECT_MESSAGE_T);
        media->audio_source = audio_source_a2dp_1;
        appAvSendStatusMessage(AV_A2DP_AUDIO_CONNECTED, (void *)media, sizeof(audio_source_t));
    }
}
...
{
    DEBUG_LOGF("appA2dpStopAudio(%p), device_id=%d, stream_id=%d", (void *)theInst, theInst->a2dp.device_id, theInst->a2dp.stream_id);
    if (theInst == appAvGetA2dpSource())
    {
    ...
    else
    {
        ...
        if (a2dp && a2dp == &theInst->a2dp)
        {
            /* Use the lock to prevent a call to appAvInstanceDestroy until cleared */
            appA2dpSetAudioStopLockBit(theInst);
            DEBUG_LOGF("appA2dpStopAudio(%p) %p %u", (void *)theInst, &appA2dpGetLock(theInst), appA2dpGetLock(theInst));
        }
        /* send message with audio_source_a2dp as a parameter and let the service do the disconnection */
        appAvSendStatusMessage(AV_A2DP_AUDIO_DISCONNECTED, (void *)media, sizeof(audio_source_t));
    }
}
...
{
    DEBUG_LOGF("appA2dpEnterDisconnected(%p) %p %u", (void *)theInst, &appA2dpGetLock(theInst), appA2dpGetLock(theInst));
...
static void appA2dpSetState(avInstanceTaskData *theInst, avA2dpState a2dp_state)
{
    ...
    Ui_InformContextChange(ui_provider_media_player, a2dp_state);
