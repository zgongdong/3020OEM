...
static void appAvrcpEnterConnected(avInstanceTaskData *theInst)
{
    ...
    BtDevice_SetSupportedProfile(&theInst->bd_addr, DEVICE_PROFILE_AVRCP);
}
...
{
    return TaskList_AddTask(theInst->avrcp.client_list, client);
}
    ...
    switch (appAvrcpGetState(theInst))
    {
        case AVRCP_STATE_DISCONNECTED:
        {
            ...
            if (ConManagerIsConnected(&theInst->bd_addr))
            {
                ...
                while (TaskList_Iterate(theInst->avrcp.client_list, &client_task))
                {
                                                         ...
                                                         const AV_INTERNAL_AVRCP_CONNECT_IND_T *ind)
{
    ...
    switch (appAvrcpGetState(theInst))
    {
        case AVRCP_STATE_DISCONNECTED:
        {
            ...
            if (TaskList_Size(theInst->avrcp.client_list))
            {
                ...
                message->bd_addr = theInst->bd_addr;
                TaskList_MessageSend(theInst->avrcp.client_list, AV_AVRCP_CONNECT_IND, message);

                /* Set counter for the number of responses we're expecting */
                theInst->avrcp.client_responses = TaskList_Size(theInst->avrcp.client_list);
        ...
        case AVRCP_STATE_CONNECTING_REMOTE_WAIT_RESPONSE:
        {
            ...
            if (theInst->avrcp.client_responses == 0)
            {
                ...
                AvrcpConnectResponse(&theInst->av_task,
                                     ...
                                     TaskList_Size(theInst->avrcp.client_list) ? TRUE : FALSE,
                                     &theInst->bd_addr);
                                              ...
                                              const AVRCP_CONNECT_CFM_T *cfm)
{
    ...
    switch (appAvrcpGetState(theInst))
    {
        ...
        case AVRCP_STATE_CONNECTING_REMOTE:
        {
            ...
            TaskList_MessageSend(theInst->avrcp.client_list, AV_AVRCP_CONNECT_CFM, message);
        ...
        case AVRCP_STATE_CONNECTED:
        {
            ...
            switch (ind->opid)
            {
                ...
                case opid_volume_down:
                    if (ind->state == 0)
                    {
                        int new_volume = AudioSources_GetVolume(audio_source_a2dp_1).value + ((ind->opid == opid_volume_up) ? 8 : -8);
...
static void appAvrcpHandleEventVolumeChangedInd(avInstanceTaskData *theInst, AVRCP_EVENT_VOLUME_CHANGED_IND_T *ind)
{
    ...
    switch (ind->response)
    {
        case avctp_response_changed:
        {
            ...
            message->bd_addr = theInst->bd_addr;
            message->volume = ind->volume;
            TaskList_MessageSend(theInst->avrcp.client_list, AV_AVRCP_VOLUME_CHANGED_IND, message);
        }
        break;
...
static void appAvrcpHandleEventPlaybackStatusChangedInd(avInstanceTaskData *theInst, AVRCP_EVENT_PLAYBACK_STATUS_CHANGED_IND_T *ind)
{
    ...
    switch (ind->response)
    {
        ...
        case avctp_response_interim:
        {
            ...
            TaskList_MessageSend(theInst->avrcp.client_list, AV_AVRCP_PLAY_STATUS_CHANGED_IND, message);
...
bool appAvrcpIsValidClient(avInstanceTaskData *theInst, Task client_task)
{
    return theInst->avrcp.client_list ?
                TaskList_IsTaskOnList(theInst->avrcp.client_list, client_task) :
                FALSE;
}
