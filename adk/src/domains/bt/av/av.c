...
static bool av_SetVolume(const bdaddr *bd_addr, uint8 volume)
{
    ...
    if (device)
    {
        ...
        if (!volume_set || (volume != a2dp_volume))
        {
            ...
            DEBUG_LOGF("av_SetVolume %x %d", bd_addr->lap, volume);
        }
    }
    return volume_set;
}
        ...
        if (appDeviceIsHandsetA2dpConnected())
        {
            ...
            if  (appDeviceGetHandsetBdAddr(&handset_bd_addr) && !appDeviceIsTwsPlusHandset(&handset_bd_addr))
            {
                ...
                if (handsetInst)
                {
                    ...
                    DEBUG_LOGF("appAvVolumeHandleAvrcpConnect(%p), peer connected, handset already connected, volume=%u", theInst, volume);
...
unsigned AvGetCurrentContext(void)
{
    ...
    else
    {
        ...
        else if (appAvIsConnectedSlave() || appAvIsConnectedMaster())
        {
...
static bool appAvA2dpLockPending(void)
{
    ...
    for (instance = 0; instance < AV_MAX_NUM_INSTANCES; instance++)
    {
        ...
        {
            DEBUG_LOGF("appAvA2dpLockPending %i %p %d",
...
void appAvInstanceDestroy(avInstanceTaskData *theInst)
{
    ...
    {
        DEBUG_LOGF("appAvInstanceDestroy(%p) permitted", theInst);
    ...
    {
        DEBUG_LOGF("appAvInstanceDestroy(%p) A2DP (%d) or AVRCP (%d) not disconnected, or A2DP Lock Pending",
...
static void appAvHandleAvrcpInitConfirm(avTaskData *theAv, const AVRCP_INIT_CFM_T *cfm)
{
    ...
    if (cfm->status == avrcp_success)
    {
        ...
        MessageSend(appInitGetInitTask(), AV_INIT_CFM, NULL);
...
bool appAvInit(Task init_task)
{
    ...
    Ui_RegisterUiProvider(ui_provider_av,AvGetCurrentContext);
