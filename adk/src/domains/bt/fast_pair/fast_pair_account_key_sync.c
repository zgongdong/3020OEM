...
static void fastPair_AccountKeySync_SendMarshalledData(fast_pair_account_key_sync_req_t *sync_data)
{
    ...
    {
        DEBUG_LOG("fastPair_AccountKeySync_SendMarshalledData. Account Key Index Info");
        for(uint8 index = 0; index < MAX_FAST_PAIR_ACCOUNT_KEYS; index++)
        {
            uint16 temp = sync_data->account_key_index[index];
            DEBUG_LOG("%02x", temp);
        }
        DEBUG_LOG("fastPair_AccountKeySync_SendMarshalledData : Account Keys Info");
        ...
        DEBUG_LOG("fastPair_AccountKeySync_SendMarshalledData. Send Marshalled Data to the peer.");
    ...
    {
        DEBUG_LOG("fastPair_AccountKeySync_SendMarshalledData. No Peer to send to.");
    }
}
