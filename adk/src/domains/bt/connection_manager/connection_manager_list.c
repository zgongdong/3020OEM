...
void ConManagerDebugAddress(const tp_bdaddr *tpaddr)
{
    ...
    DEBUG_LOG("ConManagerDebugAddress, transport %02x, type %02x, addr %04x,%02x,%06lx",
              tpaddr->transport, 
              tpaddr->taddr.type,
    ...
    if (tpaddr->taddr.type == TYPED_BDADDR_RANDOM)
    {
        if (VmGetPublicAddress(tpaddr, &resolved_tpaddr))
        {
            DEBUG_LOG("- resolved %04x %02x %06x",
...
{
    DEBUG_LOG("ConManagerDebugConnection, state %u, lock %u, users %u",
...
void ConManagerSetConnectionState(cm_connection_t *connection, cm_connection_state_t state)
{
    ...
    DEBUG_LOG("ConManagerSetConnectionState");
