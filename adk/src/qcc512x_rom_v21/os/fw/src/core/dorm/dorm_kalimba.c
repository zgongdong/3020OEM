...
#ifdef CHIP_HAS_SCALED_CLK_CONTROL
    /*
     * On AuraPlus, when App goes to shallow sleep, the scaled clock
     * mode should be enabled.
     *
     * The Scaled clock allow generated by APP HW to curator is
     * ANDed with the deep sleep enabled signals (PROC_DEEP_SLEEP_EN)
     * from both P0 and P1, the flag of No DMAC activity, the flag of
     * No Tbus Activity and the App P0 SW scaled clock allow
     * (APPS_SCALED_CLOCK_CONTROLS.SCALED_CLOCK_ALLOW).
     * Once all the necessary Scaled Clock Allows from all the subsystems
     * (i.e BT, Audio, App, Curator HW, Host etc) are assert, a hysteresis timer starts.
     * This is controlled by curator (CURATOR_SCALED_CLK_DELAY) which
     * is typically set to 8us. As a consequence, scaled clocking
     * might start after 8us have elapsed after all necessary gates allow.
     * This means processor has a window of full speed
     * (subject to its own clock divider setting as well)
     * during which it can get to its shallow sleep instruction.
     *
     * In order to enable scaled clock mode:
     *
     * For P1, PROC_DEEP_SLEEP_EN is set when entering shallow sleep.
     *
     * For P0, PROC_DEEP_SLEEP_EN is set when entering shallow sleep. And
     * when calling the function enter_shallow_sleep(), the P0 SW scaled
     * clock allow (SCALED_CLOCK_ALLOW) is set as close as possible when
     * shallow sleep instruction is executed.
     */
    hal_set_reg_proc_deep_sleep_en(1);
#endif /* CHIP_HAS_SCALED_CLK_CONTROL */
    enter_shallow_sleep();
#ifdef CHIP_HAS_SCALED_CLK_CONTROL
    /* Disable PROC_DEEP_SLEEP_EN when exiting from shallow sleep. */
...
void dorm_shallow_sleep(TIME latest)
{
    ...
    {
#ifdef CHIP_HAS_SCALED_CLK_CONTROL
        /* Enable PROC_DEEP_SLEEP_EN in order to enable scaled clock mode */
        hal_set_reg_proc_deep_sleep_en(1);
#endif /* CHIP_HAS_SCALED_CLK_CONTROL */
        enter_shallow_sleep();
#ifdef CHIP_HAS_SCALED_CLK_CONTROL
        /* Disable PROC_DEEP_SLEEP_EN */
        hal_set_reg_proc_deep_sleep_en(0);
#endif /* CHIP_HAS_SCALED_CLK_CONTROL */
    }
}
