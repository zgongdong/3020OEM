...
void stateProxy_GetInitialFlags(void)
{
    ...
    /*! \todo shouldn't be accessing the application FSM from here */
    proxy->local_state->flags.dfu_in_progress = appSmIsInDfuMode();
    proxy->local_state->flags.advertising = appSmIsBleAdvertising();
