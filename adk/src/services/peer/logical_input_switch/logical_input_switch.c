...
{
    /* The values of the logical input message bases are used in the
     * ButtonParseXML script to generate the message id defines and therefore if
     * these change in the code then they must also be updated in the script. */
    PanicFalse(LOGICAL_INPUT_MESSAGE_BASE==0x2900);
    PanicFalse(DEVICE_SPECIFIC_LOGICAL_INPUT_MESSAGE_BASE==0x2800);
