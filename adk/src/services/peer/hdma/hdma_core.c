...
void Hdma_CoreHandleEvent( hdma_timestamp timestamp, hdma_core_event_t event)
{
    ...
    switch(event)
    {
        ...
        case HDMA_CORE_PEER_OUT_OF_EAR:
            ...
            break;
        case HDMA_CORE_HFP_CONN:
            hdma_core_data->inCall = TRUE;
            break;
        case HDMA_CORE_HFP_DISCON:
            hdma_core_data->inCall = FALSE;
            break;
    }
...
static void hdma_SetHandoverEvent(hdma_core_handover_result_t newResult)
{
    ...
    if(!newResult.handover)
    {
        if(DEBUG)
        {
            HDMA_DEBUG_LOG("hdma_SetHandoverEvent: Handover is false - %d", newResult.handover);
        }
        if(hdma_core_data->hdma_result.handover != newResult.handover)
        {
            HDMA_DEBUG_LOG("hdma_SetHandoverEvent: Old Handover - %d, New handover - %d ", hdma_core_data->hdma_result.handover, newResult.handover);
            #ifndef DEBUG_HDMA_UT
    ...
    if (newResult.handover && (hdma_core_data->lastHandoverAttempt != INVALID_TIMESTAMP) && ((hdma_core_data->timestamp - hdma_core_data->lastHandoverAttempt) < suppressionPeriod))
    {
        if(DEBUG)
        {
            HDMA_DEBUG_LOG("hdma_SetHandoverEvent: Suppress Handover: reason = %d due to recently generated handover event, urgency = %d. Timestamp = %u", hdma_core_data->hdma_result.reason, hdma_core_data->hdma_result.urgency, hdma_core_data->timestamp);
        }
        return ;
    }
    ...
    if(DEBUG)
    {
        HDMA_DEBUG_LOG("hdma_SetHandoverEvent: Handover: reason = %d , urgency = %d. Timestamp = %u", hdma_core_data->hdma_result.reason, hdma_core_data->hdma_result.urgency, hdma_core_data->timestamp);
    }
    #ifndef DEBUG_HDMA_UT
...
{
    int16 thisVQ = hdma_Filter(timestamp, &(hdma_core_data->local_bud.voiceQuality), vqHalfLife, vqMaxAge, HDMA_QUEUE_MIC);
    int16 otherVQ = hdma_Filter(timestamp, &(hdma_core_data->remote_bud.voiceQuality), vqHalfLife, vqMaxAge, HDMA_QUEUE_MIC);
...
static void hdma_ValidateVoiceQuality(hdma_timestamp timestamp,hdma_core_handover_urgency_t *urgency, hdma_core_handover_urgency_t *suppressUrgency )
{
    ...
    hdma_CheckVoiceQuality(timestamp, vqHalfLife_ms.critical, vqMaxAge_ms.critical,
                                absVQThreshold.critical, relVQThreshold.critical, &otherIsBetter,&thisIsBetter);
    if (otherIsBetter)
    {
    ...
    hdma_CheckVoiceQuality(timestamp, vqHalfLife_ms.high, vqMaxAge_ms.high,
                                absVQThreshold.high, relVQThreshold.high, &otherIsBetter,&thisIsBetter);
    if (otherIsBetter)
    {
    ...
    hdma_CheckVoiceQuality(timestamp,
                                vqHalfLife_ms.low, vqMaxAge_ms.low,
                                absVQThreshold.low, relVQThreshold.low, &otherIsBetter,&thisIsBetter);
...
{
    int16 thisRSSI = hdma_Filter(timestamp, &(hdma_core_data->local_bud.phoneRSSI), rssiHalfLife_ms, rssiMaxAge_ms, HDMA_QUEUE_RSSI);
    int16 otherRSSI = hdma_Filter(timestamp, &(hdma_core_data->remote_bud.phoneRSSI), rssiHalfLife_ms, rssiMaxAge_ms, HDMA_QUEUE_RSSI);
    if(DEBUG)
    {
...
static hdma_core_handover_urgency_t hdma_ValidateLink(hdma_timestamp timestamp)
{
    ...
    if (hdma_CheckRSSILevel(timestamp,
                                rssiHalfLife_ms.critical, rssiMaxAge_ms.critical,
                                absRSSIThreshold.critical, relRSSIThreshold.critical))
    {
        urgency = HDMA_CORE_HANDOVER_URGENCY_CRITICAL;
    }
    else if (hdma_CheckRSSILevel(timestamp,
                                 rssiHalfLife_ms.high, rssiMaxAge_ms.high,
                                 absRSSIThreshold.high, relRSSIThreshold.high))
    {
        urgency = HDMA_CORE_HANDOVER_URGENCY_HIGH;
    }
    else if (hdma_CheckRSSILevel(timestamp,
                                    rssiHalfLife_ms.low, rssiMaxAge_ms.low,
                                    absRSSIThreshold.low, relRSSIThreshold.low))
    {
        urgency = HDMA_CORE_HANDOVER_URGENCY_LOW;
    }
    return urgency;
