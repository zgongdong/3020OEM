...
/*! \brief Initialise the HDMA component.
    \param[in] client_task Task to register for #hdma_messages messages.
...
{
    DEBUG_LOG("hdma_HandleCallEvent: Timestamp= %u isconnect %d", timestamp, isconnect);
    if(isconnect)
    {
        Hdma_CoreHandleEvent( timestamp, HDMA_CORE_HFP_CONN);
    }
    else
    {
        Hdma_CoreHandleEvent( timestamp, HDMA_CORE_HFP_DISCON);
    }
}
