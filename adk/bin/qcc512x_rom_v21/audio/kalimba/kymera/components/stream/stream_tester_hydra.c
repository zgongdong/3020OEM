...
static bool tester_close (ENDPOINT *ep, bool *pending);
...
{
    endpoint_tester_state *tester = &ep->state.tester;
    unsigned copied, copy_samples, data_available, meta_available, space;
    /**
    ...
    if (BUFF_METADATA(tester->source_data_buf))
    {
        meta_available = buff_metadata_available_octets(tester->source_data_buf) / OCTETS_PER_SAMPLE;
        data_available = MIN(data_available, meta_available);
    }
    ...
    if (copy_samples != 0)
    {
        ...
        if (SINK == ep->direction)
        {
            ...
            {
                audio_data_service_dequeue_meta(tester->service_priv, copy_samples * OCTETS_PER_SAMPLE, TRUE);
            }
        }
