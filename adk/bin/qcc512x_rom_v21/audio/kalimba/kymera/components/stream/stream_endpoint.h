...
typedef enum ENDPOINT_INT_CONFIGURE_KEYS
{
    ...
    /* Shadow endpoint shadowed operator's 'blocksize' */
    EP_SET_SHADOW_TINFO_BLOCKSIZE,

    /* Shadow endpoint shadowed operator's 'period' */
    EP_SET_SHADOW_TINFO_PERIOD,
...
typedef struct endpoint_a2dp_state
{
    ...
    /** private audio data service handle */
    void *bulk_data_if;
    /** maximum transfer unit = packet size */
    unsigned mtu_size;
    /** number of bytes written to determine when mtu size is reached */
    unsigned bytes_written;
    /** last packet size, used to deside as early as possibel if a packet must be padded to reach mtu_size */
    unsigned packet_size;
    /** 12 bytes RTP header, will be type-casted to sRtpHeader_t */
    unsigned rtp_header[3];
    /** rtp_header is followed by media_payload_header */
    uint8 media_payload_header;
    /** capability ID of the connected operator */
    uint16 operator_cap_id;
    /** backup, bulk data interface producer offset is only written when the complet packet was written to memory */
    uint16 producer_offset;
#endif /* CHIP_BASE_NAPIER */
...
typedef struct endpoint_shadow_state
{
    ...
    /* shadowed operator's timing info, tranferred in from other processor */
    unsigned operator_block_size;
    unsigned operator_period;
 ...
 * \param *pending  used to return if there is a pending operation (TRUE) to avoid
 *                  executing any subsequent callback.
 *
 */
 ...
 *
 */
ENDPOINT *stream_iso_get_endpoint(unsigned con_id, ENDPOINT_DIRECTION dir,
                                    unsigned num_params, unsigned *params);
#endif /* INSTALL_ISO_CHANNELS */
...
/**
 * \brief Function to close the endpoint by releasing the bulk data interface
 *
 * \param ep pointer to the endpoint that is being closed
 */
void stream_a2dp_close_endpoint(ENDPOINT* endpoint);
