 ...
 */
static void audio_generate_metadata(ENDPOINT *endpoint, unsigned new_words_written,
...
static bool audio_close (ENDPOINT *endpoint, bool *pending);
...
static bool audio_connect(ENDPOINT *endpoint, tCbuffer *Cbuffer_ptr, ENDPOINT *ep_to_kick, bool* start_on_connect)
{
    ...
    patch_fn_shared(stream_audio_hydra);
    hw_buf_size = stream_audio_get_buffer_length(audio_vsm_get_sample_rate_from_sid(stream_external_id_from_endpoint(endpoint)), endpoint->direction, TRUE);
    ...
    else
    {
        switch (key)
        {
            ...
            case STREAM_CONFIG_KEY_STREAM_ANC_CONTROL:
            {
                ...
                if (stream_direction_from_endpoint(endpoint) == SOURCE)
                {
                    status = stream_anc_source_configure(endpoint, (STREAM_CONFIG_KEY)key, value);
                }
                else
                {
                    status = stream_anc_sink_configure(endpoint, (STREAM_CONFIG_KEY)key, value);
                }
            ...
            case STREAM_CONFIG_KEY_STREAM_RM_USE_RATE_ADJUST_OPERATOR:
            {
                ...
                if(0 != opid)
                {
                    /* see if the operator is existing */
...
static bool is_hw_rate_adjustment_supported(ENDPOINT *endpoint)
{
    if (endpoint->state.audio.rm_enable_hw_rate_adjust)
    {
        ...
        return audio_vsm_get_hw_rate_adjustment_supported(id);
    }
...
{
#if defined(AUDIO_SOURCE_GENERATE_METADATA)
    endpoint_audio_state* ep_audio = &endpoint->state.audio;
