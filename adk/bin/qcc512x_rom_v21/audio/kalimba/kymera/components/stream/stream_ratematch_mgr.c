...
/****************************************************************************
Private Macro / Constant Declarations
*/

/* Define this to print out intermediate and final results
 * of rate match reconfiguration.
 */
/* #define VERBOSE_GRAPH_TRAVERSAL */

/* Define this to obtain trace printouts of the time taken for
 * find_graph_real_sources_and_sinks and ratematch_real_eps.
 * This only makes sense on hardware/kalsim, not in host unit tests.
 */
#ifdef __KCC__
/* #define PROFILE_RM_SETUP */
#endif /* __KCC__ */

/* Implement the preference for enacting at sinks,
 * among endpoints with otherwise equal ratematch preference rank.
 * This is legacy behaviour though its usefulness can be argued. It
 * seems to improve coexistence with TTP, as it leads to the same sink
 * endpoints being told to enact which then also perform TTP.
 * Without this, a source endpoint may rate match to sinks, and then
 * the sinks additionally perform TTP to adjust latency.
 */
#define RATEMATCH_ENACT_SINK_PREFERENCE
...
/* Verbose trace wrappers */
#ifdef VERBOSE_GRAPH_TRAVERSAL
#define GRAPH_MSG(F)            L2_DBG_MSG(F)
#define GRAPH_MSG1(F,A)         L2_DBG_MSG1(F,A)
#define GRAPH_MSG2(F,A,B)       L2_DBG_MSG2(F,A,B)
#define GRAPH_MSG3(F,A,B,C)     L2_DBG_MSG3(F,A,B,C)
#define GRAPH_MSG4(F,A,B,C,D)   L2_DBG_MSG4(F,A,B,C,D)
#define GRAPH_MSG5(F,A,B,C,D,E) L2_DBG_MSG5(F,A,B,C,D,E)
#else
#define GRAPH_MSG(F)
#define GRAPH_MSG1(F,A)
#define GRAPH_MSG2(F,A,B)
#define GRAPH_MSG3(F,A,B,C)
#define GRAPH_MSG4(F,A,B,C,D)
#define GRAPH_MSG5(F,A,B,C,D,E)
#endif /* VERBOSE_GRAPH_TRAVERSAL */
...
#ifdef RATEMATCH_ENACT_SINK_PREFERENCE
    /** Number of endpoints with the lowest rank */
    unsigned                num_lowest_rank;
#endif /* RATEMATCH_ENACT_SINK_PREFERENCE */

    /** Variable length array containing real endpoints either sources or sinks */
    ENDPOINT                *ep[];
} ENDPOINT_LIST;

/** A structure for managing a list of operators */
typedef struct {
    /** Vector of operator IDs */
    unsigned                *op_ids;

    /** Number of entries in op_ids[] */
    unsigned                next_new_op;
}
OPERATOR_LIST;
...
*/
static ENDPOINT_LIST* new_ep_list(unsigned capacity);
...
/**
 * \brief Allocate an ENDPOINT_LIST; it can be released with pfree/pdelete
 */
static ENDPOINT_LIST* new_ep_list(unsigned capacity)
{
    return (ENDPOINT_LIST *)xzpmalloc(sizeof(ENDPOINT_LIST) + capacity * sizeof(ENDPOINT*));
}
...
/**
 * \brief Allocate the array inside an OPERATOR_LIST
 */
static bool init_operator_list(OPERATOR_LIST* ops)
{
    unsigned* op_ids = xzpnewn(max_operators, unsigned);
...
/**
 * \brief Free the array inside an OPERATOR_LIST
 */
static void fini_operator_list(OPERATOR_LIST* ops)
{
    pdelete(ops->op_ids);
}

static void add_opid_to_graph_list(unsigned opid, OPERATOR_LIST *ops)
{
    unsigned j;
    bool already_on_list = FALSE;
    patch_fn_shared(stream_ratematch);

    for (j = 0; j < ops->next_new_op; j++)
    {
        if (ops->op_ids[j] == opid)
        {
    ...
    if (!already_on_list)
    {
        if (ops->next_new_op >= max_operators)
        {
            fault_diatribe(FAULT_AUDIO_RM_GRAPH_TOO_BIG, opid);
        }
        else
        {
            GRAPH_MSG2("            add_opid_to_graph_list 0x%04x at %d", opid, ops->next_new_op);
            ops->op_ids[ops->next_new_op] = opid;
            ops->next_new_op += 1;
        }
    }
}
 ...
 * \param ops Array of operator ids that have already been visited or
 *            will be visited, and index of next entry to add
 * \param ep_list The list of real_sources/sinks that is being compiled.
 */
static void visit_operator_terminals(unsigned op_ep_mask,
                                     OPERATOR_LIST* ops,
                                     ENDPOINT_LIST *ep_list)
{
    ENDPOINT *ep;
    ENDPOINT **terminals;
    GRAPH_MSG2("        visit_operator_terminals 0x%04x next_new_op %d", op_ep_mask, ops->next_new_op);
    /* Go through the list of endpoints and find all that exist for this operator */
    terminals = opmgr_get_terminal_list(op_ep_mask);
    ep = terminals ? *terminals : NULL;

    while (ep != NULL)
    {
        if (NULL == ep->connected_to || ep->is_real)
        {
            GRAPH_MSG3("        visit_operator_terminals ep 0x%04x real %d or not connected %d",
                       ep->key, ep->is_real, (NULL == ep->connected_to));
        ...
        {
            GRAPH_MSG3("        visit_operator_terminals ep 0x%04x connected_to is_real id 0x%04x key 0x%04x",
                       ep->key, ep->connected_to->id, ep->connected_to->key);
        ...
        else
        {
            ...
            GRAPH_MSG3("        visit_operator_terminals ep 0x%04x connected_to 0x%04x of op 0x%04x",
                       ep->key, ep->connected_to->key, opid);
            add_opid_to_graph_list(opid, ops);
        }
        ep = ep->state.operator.next_terminal;
    }
    GRAPH_MSG2("        visit_operator_terminals saw %d of max %d terminals", fnd_cnt, num_terminals);
}
 ...
 * of opid | STREAM_EP_OP_SOURCE/SINK
 * \param ops The array of operator ids that have already been visited or
 * will be visited, and index of next opid to add.
 ...
 */
static void visit_remote_operator_terminals(unsigned opid, OPERATOR_LIST *ops,
                                            unsigned num_src_terminals,
                                            unsigned num_sink_terminals,
                                            ENDPOINT_LIST *ep_list)
{
    ...
    GRAPH_MSG4("        visit_remote_operator_terminals 0x%04x num_src %d num_sink %d next_new_op %d",
               opid, num_src_terminals, num_sink_terminals, ops->next_new_op);

    if (tfm == NULL)
    {
        /* The list is empty so no connections to search */
        GRAPH_MSG("            kip_tfm_list empty");
        return;
    }
    ...
    do
    {
        ...
        {
            GRAPH_MSG2("            kip_tfm 0x%04x (op) -> 0x%04x (..)", tfm->source_id, tfm->sink_id);
            fnd_sources++;
        ...
        {
            GRAPH_MSG2("            kip_tfm 0x%04x (..) -> 0x%04x (op)", tfm->source_id, tfm->sink_id);
            fnd_sinks++;
        ...
        else
        {
             ...
             * searched for. */
            GRAPH_MSG2("            kip_tfm 0x%04x (..) -> 0x%04x (..)", tfm->source_id, tfm->sink_id);
            continue;
        }
        ...
        if (STREAM_EP_IS_SHADOW_ID(new_opid))
        {
            ...
            if (ep == NULL)
            {
                /* The EP doesn't exist so nothing more we can do. */
                GRAPH_MSG1("            shadow 0x%04x not found on p0", shadow_id);
                continue;
            }
            else if (ep->connected_to == NULL)
            {
                /* The counterpart doesn't exist so nothing more we can do.
                 * with this connection. */
                GRAPH_MSG1("            shadow 0x%04x on p0 unconnected", shadow_id);
                continue;
            }
            else if (ep->connected_to->is_real)
            {
                /* Add to the list and continue */
                GRAPH_MSG1("            shadow connected to real 0x%04x", ep->connected_to->key);
        ...
        GRAPH_MSG1("            new op 0x%04x", new_opid);

        add_opid_to_graph_list(new_opid, ops);
 ...
 */
static void find_op_rate_monitor_terminals(unsigned op_ep_mask,
                                           ENDPOINT_LIST *ep_list)
{
    ENDPOINT *ep;
    ENDPOINT** terminals;

    GRAPH_MSG1("        find_op_rate_monitor_terminals 0x%04x", op_ep_mask);
    ...
    while (ep != NULL)
    {
        if ((ep->connected_to != NULL) && is_rate_monitor(ep))
        {
            ...
            GRAPH_MSG1("        find_op_rate_monitor_terminals ep 0x%04x monitor", ep->key);
        }
        ep = ep->state.operator.next_terminal;
    }
}
...
#ifdef VERBOSE_GRAPH_TRAVERSAL
static void print_fgr_results(ENDPOINT_LIST* real_eps)
{
    if (real_eps == NULL)
    {
        L2_DBG_MSG("    real_eps: none");
    }
    else
    {
        unsigned i;
        L2_DBG_MSG1("    real_eps: %d", real_eps->length);
        for (i = 0; i < real_eps->length; ++ i)
        {
            L2_DBG_MSG2("        0x%04x 0x%04x", real_eps->ep[i]->id, real_eps->ep[i]->key);
        }
    }
}
#define PRINT_FGR_RESULTS print_fgr_results(*real_eps)
#else /* VERBOSE_GRAPH_TRAVERSAL */
#define PRINT_FGR_RESULTS do{}while(0)
#endif /* VERBOSE_GRAPH_TRAVERSAL */
 ...
 * \param real_eps The location to store the list of sinks and sources that are found.
 */
static void find_graph_real_eps(unsigned start_id, ENDPOINT_LIST **real_eps)
{
    unsigned curr_op;
    ENDPOINT *start = NULL;
    OPERATOR_LIST graph_ops;

    patch_fn_shared(stream_ratematch);

    GRAPH_MSG1("    find_graph_real_sources_and_sinks 0x%04x", start_id);
    ...
    if (start != NULL && start->is_real)
    {
        ...
        else if (start->connected_to->is_real)
        {
            *real_eps = new_ep_list(2);
            ...
            (*real_eps)->length = 2;
            (*real_eps)->ep[0] = start;
            (*real_eps)->ep[1] = start->connected_to;
            PRINT_FGR_RESULTS;
            return;
        }
    }
     ...
     * found. */
    *real_eps = new_ep_list(MAX_REAL_EPS);
    ...
    if (! init_operator_list(&graph_ops))
    {
        pfree(*real_eps);
        *real_eps = NULL;
        return;
    ...
    if (start != NULL)
    {
        if (start->is_real)
        {
            ...
            if (STREAM_EP_IS_SHADOW(start->connected_to))
            {
                ...
                first_opid = (start->connected_to->key & STREAM_EP_OPID_MASK) | STREAM_EP_OP_BIT | OP_IS_REMOTE;
            }
            else
#endif /* INSTALL_DUAL_CORE_SUPPORT */
            {
                first_opid = start->connected_to->key & (STREAM_EP_OPID_MASK | STREAM_EP_OP_BIT);
            }
        }
        else
        {
            ...
            if (STREAM_EP_IS_SHADOW(start))
            {
                /* This is on P0 so must be shadowing an operator endpoint on P1
                 * so lets get that operator id and start from there. */
                first_opid = (start->key & STREAM_EP_OPID_MASK) | STREAM_EP_OP_BIT | OP_IS_REMOTE;
            }
            else
#endif /* INSTALL_DUAL_CORE_SUPPORT */
            {
                first_opid = start->key & (STREAM_EP_OPID_MASK | STREAM_EP_OP_BIT);
            }
        }
    }
#ifdef INSTALL_DUAL_CORE_SUPPORT
    else
    {
        ...
        {
            first_opid = (start_id & (STREAM_EP_OPID_MASK | STREAM_EP_OP_BIT))
                                            | OP_IS_REMOTE;
        }
        else
        {
            first_opid = (stream_kip_connected_to_epid(start_id) & STREAM_EP_OPID_MASK)
                                            | (STREAM_EP_OP_BIT | OP_IS_REMOTE);
        }
    }
#endif /* INSTALL_DUAL_CORE_SUPPORT */
    if (first_opid != 0)
    {
        add_opid_to_graph_list(first_opid, &graph_ops);
    }
    ...
    for (curr_op = 0; curr_op < graph_ops.next_new_op; curr_op++)
    {
        unsigned opid = graph_ops.op_ids[curr_op];
#ifdef INSTALL_DUAL_CORE_SUPPORT
        if ((opid & OP_IS_REMOTE) == OP_IS_REMOTE)
        {
            opid &= ~OP_IS_REMOTE; /* mask out the remote flag */
            visit_remote_operator_terminals(opid, &graph_ops,
                                            opmgr_get_num_sink_terminals(opid),
                                            opmgr_get_num_source_terminals(opid), *real_eps);
        }
        else
#endif /* INSTALL_DUAL_CORE_SUPPORT */
        {
             ...
             */
            visit_operator_terminals(opid | STREAM_EP_OP_SINK, &graph_ops,
                                     *real_eps);

            /* Now the connected sources */
            visit_operator_terminals(opid | STREAM_EP_OP_SOURCE, &graph_ops,
                                     *real_eps);

            find_op_rate_monitor_terminals(opid | STREAM_EP_OP_SINK, *real_eps);
            find_op_rate_monitor_terminals(opid | STREAM_EP_OP_SOURCE, *real_eps);
        }
    }

    /* If either of the source or sink lists are empty then release the memory */
    if ((*real_eps)->length == 0)
    {
        pfree(*real_eps);
        *real_eps = NULL;
    }
...
static bool have_same_clock_source(ENDPOINT *ep1, ENDPOINT *ep2, bool both_local)
{
    ...
    if (ep1->stream_endpoint_type != ep2->stream_endpoint_type)
    {
        if (both_local)
        {
            GRAPH_MSG4("        ep_type %d %d both_local -> same clock 0x%04x 0x%04x",
                       ep1->stream_endpoint_type, ep2->stream_endpoint_type,
                       ep1->id, ep2->id);
            return TRUE;
        }
        else
        {
            GRAPH_MSG4("        ep_type %d %d not both_local -> different clock 0x%04x 0x%04x",
                       ep1->stream_endpoint_type, ep2->stream_endpoint_type,
                       ep1->id, ep2->id);
            return FALSE;
        }
    ...
    return (ep1->functions->have_same_clock)(ep1, ep2, both_local);
}
...
#ifdef RATEMATCH_ENACT_SINK_PREFERENCE
            /* To allow searching for sinks among the endpoints of same
             * RATEMATCHING_SUPPORT, save the number of such endpoints.
             * This consists of the number of endpoints in the first
             * non-empty bucket plus the next one if that is valid
             * and has the same RATEMATCHING_SUPPORT value.
             */
            eps->num_lowest_rank = num_eps_per_ord[i];
            if ((i < RM_NUM_ORDS-1) && (rm_ord_to_rm_support[i+1] == eps->ep0_rank))
            {
                eps->num_lowest_rank += num_eps_per_ord[i+1];
            }
#endif /* RATEMATCH_ENACT_SINK_PREFERENCE */
            break;
        }
...
static bool create_pair(ENDPOINT *enacting, ENDPOINT *non_enacting)
{
    ...
    if (SINK == enacting->direction)
    {
        ...
        GRAPH_MSG2("    create_pair : 0x%04x -> 0x%04x", non_enacting->id, enacting->id);
    }
    else
    {
        ...
        GRAPH_MSG2("    create_pair : 0x%04x <- 0x%04x", enacting->id, non_enacting->id);
    }
                                   ...
                                   ENDPOINT *non_enacting)
{
    ...
    if (list != NULL)
    {
        for (i = first_element; i < list->length; i++)
        {
            ...
            {
                GRAPH_MSG2("        same clock pair 0x%04x 0x%04x", non_enacting->id, list->ep[i]->id);
                continue;
            }
 ...
 *
 * \param real_eps List of source and sink endpoints in the graph
 *
 * \return Whether it was possible to setup ratematching for the graph.
 */
static bool ratematch_real_eps(ENDPOINT_LIST *real_eps)
{
    ENDPOINT *non_enacting;
    ...
    if (real_eps != NULL)
    {
        if (!sort_ep_list(real_eps))
        {
            return FALSE;
        }
        if (real_eps->length <= 1)
        {
            return TRUE;
        }
    }
     ...
     * ratematching to do */
    if (! real_eps->not_local)
    {
        return TRUE;
    }

    non_enacting = real_eps->ep[0];
#ifdef RATEMATCH_ENACT_SINK_PREFERENCE
    if (non_enacting->direction == SINK)
    {
        unsigned i;

        for (i = 1; i < real_eps->num_lowest_rank; ++ i)
        {
            if (real_eps->ep[i]->direction == SOURCE)
            {
                non_enacting = real_eps->ep[i];
                GRAPH_MSG3("    prefer non-enacting #%d 0x%04x over #0 0x%04x",
                           i, non_enacting->id, real_eps->ep[0]->id);
                break;
            }
        }
    }
    ...
    GRAPH_MSG1("    non-enacting: 0x%04x", non_enacting->id);
     ...
     */
#ifdef RATEMATCH_ENACT_SINK_PREFERENCE
    if (!create_pairs_from_list(real_eps, 0, non_enacting))
#else /* RATEMATCH_ENACT_SINK_PREFERENCE */
    if (!create_pairs_from_list(real_eps, 1, non_enacting))
#endif /* RATEMATCH_ENACT_SINK_PREFERENCE */
    {
        return FALSE;
    }
...
{
    ENDPOINT_LIST *real_eps = NULL;

    bool result = TRUE;

    GRAPH_MSG1("setup_ratematching 0x%04x", ep_id);
...
#ifdef PROFILE_RM_SETUP
    TIME start_time = time_get_time();
#endif
    find_graph_real_eps(ep_id, &real_eps);
#ifdef PROFILE_RM_SETUP
    TIME end_time = time_get_time();
    L2_DBG_MSG2("setup_rm for 0x%04x fgr took %6d us", ep_id, (end_time - start_time));
#endif

#ifdef PROFILE_RM_SETUP
    start_time = time_get_time();
#endif
    result = ratematch_real_eps(real_eps);
#ifdef PROFILE_RM_SETUP
    end_time = time_get_time();
    L2_DBG_MSG2("setup_rm for 0x%04x rre took %6d us", ep_id, (end_time - start_time));
#endif
...
static void remove_rm_pairs(ENDPOINT_LIST *remove_list)
{
    ...
    for (i = 0; i < remove_list->length; i++)
    {
        ...
        for (p = &rm_list; *p != NULL; p = &((*p)->next))
        {
            ...
            {
                GRAPH_MSG4("        remove ep 0x%04x pair 0x%04x 0x%04x en 0x%04x",
                           remove_list->ep[i]->id, (*p)->ep1->id, (*p)->ep2->id, (*p)->enacting_ep->id);
...
bool cease_ratematching(unsigned ep_id)
{
    ...
    GRAPH_MSG1("cease_ratematching 0x%04x", ep_id);
...
#ifdef PROFILE_RM_SETUP
    TIME end_time = time_get_time();
    L2_DBG_MSG2("cease_rm for 0x%04x fgr took %6d us", ep_id, (end_time - start_time));
#endif

    if (NULL != real_eps)
    {
        remove_rm_pairs(real_eps);

#ifdef PROFILE_RM_SETUP
        TIME start_time_2 = time_get_time();
#endif
        result = ratematch_real_eps(real_eps);
#ifdef PROFILE_RM_SETUP
        end_time = time_get_time();
        L2_DBG_MSG2("cease_rm for 0x%04x rre took %6d us", ep_id, (end_time - start_time_2));
#endif
...
#ifdef PROFILE_RM_SETUP
    end_time = time_get_time();
    L2_DBG_MSG2("cease_rm for 0x%04x (all) took %6d us", ep_id, (end_time - start_time));
#endif
    return result;
...
unsigned* get_rm_pairs_dump(void)
{
    ...
    *p++ = count;
    ...
    {
        *p++ = stream_external_id_from_endpoint(cur_pair->ep1);
        *p++ = stream_external_id_from_endpoint(cur_pair->ep2);
        *p++ = stream_external_id_from_endpoint(cur_pair->enacting_ep);
    }
    return dump;
}
