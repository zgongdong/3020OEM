...
/* Structure for holding closing information for deferred operation */
typedef struct
{
    ENDPOINT *ep;
    unsigned con_id;
    bool (*callback)(unsigned con_id, unsigned status);
} ep_close_info_struct;
...
static ep_close_info_struct *ep_close_info = NULL;
...
void stream_if_ep_close_complete(ENDPOINT *ep, bool status)
{
    if (ep_close_info == NULL)
    {
        /* It is expected that some close operations do not have the close info,
         * this can happen for close operations when create fail. */
        return;
    }
    else if (ep_close_info->ep != ep)
    {
        /* This isn't the endpoint you're looking for... */
        panic_diatribe(PANIC_AUDIO_ENDPOINT_CLOSE_SEQUENCE_ERROR, (DIATRIBE_TYPE)((uintptr_t)ep));
    }
    else if(!status)
    {
        /* Close failed. Send back command failed and clean up the endpoint */
        ep_close_info->callback(REVERSE_CONNECTION_ID(ep_close_info->con_id),
                                    STATUS_CMD_FAILED);
    }
    else
    {
        ...
        bool (*callback)(unsigned con_id, unsigned status, unsigned source_id))
{
    ...
    case STREAM_DEVICE_OPERATOR:
            if (num_params == 2)
            {
                ...
                if ((opmgr_kip_get_processor_id_from_opid(params[0], &proc_id)) &&
                    (!IPC_ON_SAME_CORE(proc_id)))
                {
        ...
        case STREAM_DEVICE_L2CAP:
            ...
            break;
#elif  defined (INSTALL_A2DP) && defined(CHIP_BASE_NAPIER)
        case STREAM_DEVICE_L2CAP:
            ep = stream_a2dp_get_endpoint(con_id, dir, num_params, params, &pending);
            break;
 ...
 *
 */
void stream_if_close_endpoint(unsigned con_id,
                              ...
                              STREAM_STATUS_CBACK callback)
{
    ...
    ENDPOINT *endpoint = stream_endpoint_from_extern_id(endpoint_id);
    if (endpoint == NULL)
    {
...
#endif /* defined(INSTALL_UNINTERRUPTABLE_ANC) && defined(INSTALL_ANC_STICKY_ENDPOINTS) */
    {
         ...
         */
        if (stream_close_endpoint_check_pending(endpoint, &pending))
        {
            status = STATUS_OK;
            /* Check if the  endpoint returned a pending state, so we have to now wait for
            * it to call the completion callback before signalling to the client
            */
            if (pending == TRUE)
            {
                ...
                {
                    /* Should only have one in progress...? */
                    panic_diatribe(PANIC_AUDIO_ENDPOINT_CLOSE_SEQUENCE_ERROR, (DIATRIBE_TYPE)((uintptr_t)(ep_close_info->ep)));
                }
                else
                {
                    /* Allocate some data for the deferred-creation context
                     ...
                     * so we can't do without it...
                     */
                    ep_close_info = pnew(ep_close_info_struct);
                    ep_close_info->callback = callback;
                    ep_close_info->con_id = con_id;
                    ep_close_info->ep = endpoint;
                }
                /* Return immediately avoiding the callback execution. */
                return;
            }
        }
    }
 ...
 *
 */
void stream_if_configure_sid(unsigned con_id,
                             unsigned ep_id,
                             unsigned int key,
                             uint32 value,
                             STREAM_STATUS_CBACK callback)
 ...
 *
 */
void stream_if_mclk_activate(unsigned con_id,
                             unsigned ep_id,
 ...
 *
 */
void stream_if_set_mclk_source(unsigned con_id,
 ...
 *
 */
void stream_if_sync_sids(unsigned con_id,
                         unsigned ep_id1,
                         unsigned ep_id2,
                         STREAM_STATUS_CBACK callback)
{
        ...
        bool (*callback)(unsigned con_id, unsigned status, unsigned transform_id))
{
    ...
    if ( STREAM_EP_IS_OPEP_ID(source_id) )
    {
         ...
         */
        if( opmgr_kip_get_processor_id( source_id, &proc_id) &&
            (!IPC_ON_SAME_CORE(proc_id)))
        {
    ...
    if ( STREAM_EP_IS_OPEP_ID(sink_id) )
    {
         ...
         * it will return failure while attempting to create the endpoints.
         */
        if (opmgr_kip_get_processor_id(sink_id, &proc_id) &&
            (!IPC_ON_SAME_CORE(proc_id)))
        {
            if (remote_processor_id == IPC_PROCESSOR_0)
            {
                remote_processor_id = proc_id;
            }
        ...
        bool (*callback)(unsigned con_id, unsigned status, unsigned sink_id))
{
    ...
    else
    {
        ...
        if ((source_ep->stream_endpoint_type == endpoint_audio) && (hw_type == STREAM_DEVICE_DIGITAL_MIC))
        {
            callback(REVERSE_CONNECTION_ID(con_id), STATUS_CMD_FAILED,0);
            return;
        }
        ...
        bool (*callback)(unsigned con_id, unsigned status, unsigned tr_id))
{
    ...
    /* For P1 stand-alone don't include code below. But do include for */
    /* any other build: non-dual-core build, P0 stand-alone build, and */
    /* combined P0 & P1 image                                          */
