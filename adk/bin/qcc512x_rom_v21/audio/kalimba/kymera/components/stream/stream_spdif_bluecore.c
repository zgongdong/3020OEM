        ...
        case XAP_KAL_MSG_AUDIO_INFO_RESP:
        {
            ...
            for (i = 0; i < no_keys; i++)
            {
                switch (pdu[2 + (i * 3)])
                {
                     ...
                     */
                    case STREAM_INFO_KEY_AUDIO_INSTANCE:
                    {
                    ...
                    case STREAM_INFO_KEY_AUDIO_CHANNEL:
                    {
                    ...
                    case STREAM_INFO_KEY_AUDIO_SAMPLE_FORMAT:
                    {
            ...
            else
            {
                ...
                if(IS_SPDIF_HIGH_RATE(sample_rate))
                {
                    ...
                    ep->state.spdif.extra->highrate_timer_time = time_get_time();
                }
            }
        ...
        unsigned op_id, unsigned num_resp_params, unsigned *resp_params)
{
    ...
    NOT_USED(status);
    NOT_USED(op_id);
    NOT_USED(num_resp_params);
    NOT_USED(resp_params);
    return TRUE;
}
                                     ...
                                     bool *pending)
{
    ...
    if (!endpoint)
    {
        ...
        endpoint->state.spdif.extra->last_read_time = time_get_time();
        ...
        message[1] = 4;
        message[2] = STREAM_INFO_KEY_AUDIO_INSTANCE;
        message[3] = STREAM_INFO_KEY_AUDIO_CHANNEL;
        message[4] = STREAM_INFO_KEY_AUDIO_SAMPLE_FORMAT;
        message[5] = STREAM_INFO_KEY_AUDIO_SAMPLE_RATE;
...
static void spdif_handle_bluecore_highrates(ENDPOINT *endpoint)
{
       ...
       IS_SPDIF_HIGH_RATE(extra->dsp_sample_rate))
    {
         ...
         * it's in progress.
         */
        extra -> highrate_timer_time = time_get_time();
        return;
    }
    ...
    if((extra->confirms_pending == 0) &&
       (extra->stream_valid ||
        (time_sub(time_get_time(),extra -> highrate_timer_time) > SECOND)))
    {
        unsigned msg[2];
