...
*/
static bool spdif_close (ENDPOINT *endpoint, bool *pending);
...
static bool spdif_have_same_clock(ENDPOINT *ep1, ENDPOINT *ep2, bool both_local);
    ...
    switch (event)
    {
        ...
        case SPDIF_EVENT_CHSTS_CHANGE:
        {
               ...
               !ep->state.spdif.is_overridden)
            {
                ...
                opmgr_operator_message(OPMGR_ADAPTOR_OBPM,
                                     ...
                                     bool *pending)
{
    ...
    instance = params[HYDRA_AUDIO_PARAMS_INSTANCE];
    channel  = params[HYDRA_AUDIO_PARAMS_CHANNEL];
    ...
    if(NULL == endpoint)
    {
        ...
        endpoint->state.spdif.channel_order = (SPDIF_CHANNEL_ORDER)channel;
...
static bool spdif_configure(ENDPOINT *endpoint, unsigned key, uint32 value)
{
    ...
    else
    {
        switch (key)
        {
        case STREAM_CONFIG_KEY_STREAM_SPDIF_SET_EP_FORMAT:
        {
        ...
        case STREAM_CONFIG_KEY_STREAM_SPDIF_OUTPUT_RATE:
        {
        ...
        default:
            if (key == STREAM_CONFIG_KEY_STREAM_AUDIO_SAMPLE_SIZE)
            {
                /* set the hardware shift value */
                if (value == STREAM_AUDIO_SAMPLE_SIZE_16)
                {
                    endpoint->state.spdif.shift = DAWTH - 16;
                }
                else if (value == STREAM_AUDIO_SAMPLE_SIZE_24)
                {
