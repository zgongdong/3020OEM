...
/* Enum that describes the packing of a buffer. */
typedef enum
{
...
#define BUF_DESC_IN_PLACE(x)                ((x) & BUF_DESC_IN_PLACE_MASK)
     ...
     */
#define BUF_DESC_USABLE_OCTETS_SHIFT        17
#define BUF_DESC_USABLE_OCTETS_WIDTH        2
#define BUF_DESC_USABLE_OCTETS_MASK         (((1 << BUF_DESC_USABLE_OCTETS_WIDTH) - 1) << BUF_DESC_USABLE_OCTETS_SHIFT)
#define BUF_DESC_USABLE_OCTETS_CLEAR(x)     ((x) &= ~BUF_DESC_USABLE_OCTETS_MASK)
/* see Set and Get methods cbuffer_Xet_usable_octets() */
...
typedef enum
{
    USABLE_OCTETS_32BIT = 0,
    USABLE_OCTETS_8BIT,
    USABLE_OCTETS_16BIT,
    USABLE_OCTETS_24BIT,
} USABLE_OCTETS;
/* NB. not the same like sample_size ENUM in hydra_mmu.h (matching HW BAC32),
     but this is more logical. */
#define BUF_DESC_BAC32_TO_USABLE_OCTETS(spl_sz) ((spl_sz+1)>=4 ? 0 : (spl_sz+1))

/* only define what we currently support */
#define BUF_DESC_UNPK_16BIT_MASK (USABLE_OCTETS_16BIT << BUF_DESC_USABLE_OCTETS_SHIFT)
#define BUF_DESC_UNPK_32BIT_MASK (USABLE_OCTETS_32BIT << BUF_DESC_USABLE_OCTETS_SHIFT)
...
/**
 * Macro returns the number of octets in a word giving the packing dpending on DATAFORMAT
 */
#ifdef DATAFORMAT_32
#define ENCODED_DATA_OCTETS_IN_WORD NR_OF_OCTETS_IN_WORD(USE_32BIT_PER_WORD)
#else
#define ENCODED_DATA_OCTETS_IN_WORD NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD)
#endif
 ...
 * Note. "offset" in _ex context is octet offset whithin a word (range 0...3)
 */
...
/*
 * \brief calculate data available in "effective" octets
 * \param  cb cbuffer pointer
 * \return data available in octets
 * \note only counting effective octets, e.g. 2 octets per word for 16-bit unpacked
 *      (could be odd an number)
 */
extern unsigned cbuffer_calc_amount_data_ex(tCbuffer * cb);

/*
 * \brief calculate (space available in "effective" octets) - (max octets in word)
...
/*
 * \brief copy octets from one cbuffer to another. The cbuffers must be 32-bit
 *  packed or 16-bit unpacked.
 ...
 */
extern unsigned cbuffer_get_size_ex(tCbuffer *cb);
