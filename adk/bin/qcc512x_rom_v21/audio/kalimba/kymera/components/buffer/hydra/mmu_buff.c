...
/* Remote destination block ID.
   ...
   appropriate value which is 9 for AuraPlus.*/
    ...
    while ( idx < NUM_MMU_BUFFER_HANDLES )
    {
        ...
        if (mmu_buffer_handles[idx].buffer_start_p == 0)
        {
            ...
            if (mmu_buffer_handles[idx].buffer_start_p == 0)
            {
                /* according to the loose criteria to find the next free handle above,
                    this should be enough, re-enable interrupts as early as you can */
            /* On Napier the BAC sits in the AUDIO_AHB block which reaches DM memory through its AHB address space mapping  */
...
unsigned mmu_buffer_release_handle(mmu_index index)
{
    ...
    if ((start_p & AUDIO_AHB_SPACE_CLIENT_MASK) == AUDIO_AHB_SPACE_KALIMBA_DM_START_ADDRESS)
    {
        /* This seems to be a BAC start address from AHB_AUDIO address space
         * -> convert it to DM address space first.
         */
        start_p &=AUDIO_AHB_SPACE_KALIMBA_DM_SIZE;
    }
