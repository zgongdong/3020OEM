...
$_cbuffer_copy_unaligned_16bit_be_zero_shift_ex:
    ...
    if Z jump check_wr_offset;
        ...
        if NEG call $error;             // TODO amount to copy = 1
...
// --------- pack_aligned ----------
//void cbufer_pack_aligned(tCbuffer *dst, tCbuffer *src, unsigned num_octets);
// in r0 destination cbuffer address
// in r1 source cbuffer address
// in r2 number of octets to copy (non-zero)
// trashed r3, r10, B0, B4
...
.MODULE $M.cbuffer_pack_aligned_ex;
    .CODESEGMENT BUFFER_PM;
$cbuffer.pack_aligned:
$_cbuffer_pack_aligned:
    LIBS_PUSH_REGS_SLOW_SW_ROM_PATCH_POINT($cbuffer.pack_aligned.PATCH_ID_0)
    // save the input paramerters for later
    ...
    // check if cbuffer base addresses are the same
    Null = r5 - r3;
    if Z call $error;  // in-place TODO not implemented
        // YOU CANNOT only advance pointers for pack in-place, as you do for a simple copy
    ...
    if Z jump no_odd_offset;
        ...
        if Z jump dst_offset_is_1;
            // dst_offset=2|3: octet orphan on LSB of the MS half-word
            ...
            // continue unpacking a half-word_orphan: will be on LS 32-bit
            r0 = M[I0, M0];     // read the next word to pack
    ...
    //  also might reconsider for stre+ and future chips that do not suffer from this problem.
...
// --------- pack_unaligned ----------
//void cbufer_pack_unaligned(tCbuffer *dst, tCbuffer *src, unsigned num_octets);
// in r0 destination cbuffer address
// in r1 source cbuffer address
// in r2 number of octets to copy (non-zero)
// trashed r3, r10, B0, B4
...
.MODULE $M.cbuffer_pack_unaligned_ex;
    .CODESEGMENT BUFFER_PM;
$cbuffer.pack_unaligned:
$_cbuffer_pack_unaligned:
    LIBS_PUSH_REGS_SLOW_SW_ROM_PATCH_POINT($cbuffer.pack_unaligned.PATCH_ID_0)
    // save the input paramerters for later
    ...
    Null = r5 - r3;
    if NZ jump not_in_place_copy;
        // YOU CANNOT only advance pointers for in-place TODO
        r5 = M[FP + 3*ADDR_PER_WORD];            // copy amount
        r0 = M[FP + 1*ADDR_PER_WORD];            // cbuffer_dest
        r1 = r5;
        call $cbuffer.advance_write_ptr_ex;
...
// --------- unpack_aligned ----------
//void cbufer_unpack_aligned(tCbuffer *dst, tCbuffer *src, unsigned num_octets);
// in r0 destination cbuffer address
// in r1 source cbuffer address
// in r2 number of octets to copy (non-zero)
// trashed r3, r10, B0, B4
...
.MODULE $M.cbuffer_unpack_aligned_ex;
    .CODESEGMENT BUFFER_PM;
$cbuffer.unpack_aligned:
$_cbuffer_unpack_aligned:
    LIBS_PUSH_REGS_SLOW_SW_ROM_PATCH_POINT($cbuffer.unpack_aligned.PATCH_ID_0)
    // save the input paramerters for later
    pushm <FP(=SP), r0, r1, r2, r4, r5, r6, rLink>;
    pushm <I0, I4, M0, L0, L4>;
    ...
    if Z jump no_odd_offset;
        ...
        if Z jump src_offset_is_1;
            // src_offset=2|3: first handle octet orphan
            r2 = r0 LSHIFT r1;  // on LS 16-bit
            r2 = r2 AND 0xFF;
...
// --------- unpack_unaligned ----------
//void cbufer_unpack_unaligned(tCbuffer *dst, tCbuffer *src, unsigned num_octets);
// in r0 destination cbuffer address
// in r1 source cbuffer address
// in r2 number of octets to copy (non-zero)
// trashed r3, r10, B0, B4
...
.MODULE $M.cbuffer_unpack_unaligned_ex;
    .CODESEGMENT BUFFER_PM;
$cbuffer.unpack_unaligned:
$_cbuffer_unpack_unaligned:
    LIBS_PUSH_REGS_SLOW_SW_ROM_PATCH_POINT($cbuffer.unpack_unaligned.PATCH_ID_0)
    // save the input paramerters for later
    pushm <FP(=SP), r0, r1, r2, r4, r5, r6, r7, r8, r9, rLink>;
    pushm <I0, I4, L0, L4>;
