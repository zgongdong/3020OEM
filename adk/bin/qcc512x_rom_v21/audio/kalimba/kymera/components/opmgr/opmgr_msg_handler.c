 ...
 * \param  msg pointer to the message payload that was received from the operator.
 */
static void msg_from_op_handler(tRoutingInfo *rinfo, uint16f *msg)
{
    ADAPTOR_FROM_OPERATOR_MSG *adaptor_msg;
    OP_STD_UNSOLICITED_MSG *op_msg;
    
    adaptor_msg = (ADAPTOR_FROM_OPERATOR_MSG*) msg;
    op_msg = (OP_STD_UNSOLICITED_MSG*) msg;
    ...
    adaptor_send_message(PACK_CON_ID(rinfo->src_id, rinfo->dest_id), AMSGID_FROM_OPERATOR,
                         op_msg->length + ADAPTOR_FROM_OPERATOR_MSG_MESSAGE_WORD_OFFSET,
                         (ADAPTOR_DATA) adaptor_msg);
}
    ...
    while(get_message_with_routing(OPMGR_TASK_QUEUE_ID, &cmd_id, &msg_body, &rinfo))
    {
        switch (cmd_id)
        {
            ...
            case OPCMD_FROM_OPERATOR:
            {
                 ...
                 */
                msg_from_op_handler(&rinfo, (uint16f*) msg_body);
                break;
            }
            default:
            {
