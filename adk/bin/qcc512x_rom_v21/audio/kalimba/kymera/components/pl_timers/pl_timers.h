...
#define US_PER_MS         ((TIME_INTERVAL) 1000)
#define MS_PER_SEC        ((TIME_INTERVAL) 1000)
...
/* There are 3 possible combinations that need abstracting away:
 * - 24-bit timers on 24-bit CPU architecture (BlueCore)
 * - 24-bit timers on 32-bit CPU architecture (PC build for BlueCore)
 * - 32-bit timers on 32-bit CPU architecture (CSRA68100 and successors)
 */
#if defined (__KALIMBA32__)
...
#ifdef __GNUC__
/* Sign-extend 24-bit to 32-bit so that time_sub work properly. */
 ...
 */
extern TIME time_get_time(void);
 ...
 */
extern tTimerId create_add_strict_event(
 ...
 * \return returns the timer ID
 */
extern tTimerId create_add_casual_event(
 ...
 */
extern tTimerId create_add_casual_event_alt(
...
INLINE_SECTION static inline void timer_schedule_event_at_atomic(
       TIME event_time,
       tTimerEventFunction TimerEventFunction,
       void *data_pointer,
       tTimerId *id)
 ...
 * These macros avoid the problem by blocking interrupts until the timer ID is assigned.
 */
INLINE_SECTION static inline void timer_schedule_event_in_atomic(
        TIME_INTERVAL time_in,
        tTimerEventFunction TimerEventFunction,
        void *data_pointer,
        tTimerId *id)
...
INLINE_SECTION static inline tTimerId timer_schedule_bg_event_in_between(
        TIME_INTERVAL time_earliest,
        TIME_INTERVAL time_latest,
        tTimerEventFunction TimerEventFunction,
        void *data_pointer)
{
 ...
 *
 */
extern void timer_cancel_event_by_function(tTimerEventFunction TimerEventFunction,
                                    void *data_pointer);
 ...
 * \ingroup alt_interface
 */
extern void timer_cancel_event_by_function_alt(tTimerEventFunctionAlt TimerEventFunction,
                                    uint16 iarg,
                                    void *data_pointer);
 ...
 *
 */
extern void timer_n_us_delay(TIME_INTERVAL delay_duration_us);
 ...
 */
extern bool is_current_time_later_than(TIME t);
 ...
 * if one of them is hal_get_time()
 */
extern bool is_current_time_earlier_than(TIME t);
 ...
 */
extern TIME get_last_fire_time(void);
/**
 ...
 */
extern TIME get_last_casual_fire_time(void);
...
/**
 * Compatibility helper for files in hydra_modules.
 * Do not use this macro outside of hydra_modules.
 */
#define get_time() time_get_time()
