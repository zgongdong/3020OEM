...
#ifdef USE_ACCMD
    if (GET_CON_ID_SEND_ID(OPMGR_GET_OPMSG_CLIENT_ID(obpm_msg)) == OPMGR_ADAPTOR_OBPM)
#endif /* USE_ACCMD */
    {
...
unsigned cps_control_get(void* message_data, unsigned index,unsigned *value,CPS_CONTROL_SOURCE *source)
{
   ...
   if(source!=NULL)
   {
        if (OPMGR_GET_OPMSG_CLIENT_ID(op_msg) != OPMGR_ADAPTOR_OBPM)
        {
