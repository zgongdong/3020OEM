 ...
 * \param touched
 */
unsigned sco_rcv_processing(SCO_COMMON_RCV_OP_DATA *sco_data)
{
    ...
    if (!enough_data_to_run(sco_data, METADATA_HEADER_SIZE))
    {
         ...
         */
        SCO_DBG_ERRORMSG2("Kick: %d, bad kicks: %d:: Warning! Not enough input data to run, faking packet", sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.num_bad_kicks);
    ...
    if (!enough_space_to_run(sco_data, sco_rcv_get_output_size_words(sco_data)))
    {
        ...
        * we will have enough data on the output.
        */
        SCO_DBG_ERRORMSG2("Kick: %d, bad kicks: %d:: Warning! Not enough output space to run", sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.num_bad_kicks);
    ...
    while(packets_to_process > 0)
    {
            ...
            !enough_data_to_run(sco_data, CONVERT_OCTETS_TO_SAMPLES(sco_metadata.packet_length)) )
        {
            ...
            SCO_DBG_ERRORMSG2("Kick: %d, bad kicks: %d:: Warning! Invalid meta data, faking packet", sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.num_bad_kicks);
            /* Fake a packet */
            ret_val |= fake_packet(sco_data, sco_rcv_get_output_size_words(sco_data), SCO_NB);
        ...
        if (sco_metadata.packet_length != sco_data->sco_rcv_parameters.sco_pkt_size)
        {
            ...
            SCO_DBG_ERRORMSG3("Kick: %d, size changed: %d:: Warning! Packet size changed to %d", sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.md_pkt_size_changed, sco_metadata.packet_length);
        }
        ...
        switch(status)
        {
            case SCO_METADATA_LATE:
            {
                ...
                SCO_DBG_ERRORMSG2("Kick: %d, Late: %d:: Warning! Packet late", sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.md_late_pkts);
                if (!enough_data_to_run(sco_data, METADATA_HEADER_SIZE) ||
                    !enough_space_to_run(sco_data, sco_rcv_get_output_size_words(sco_data)))
                {
                     ...
                     * we will have enough data on the output.
                     */
                    SCO_DBG_ERRORMSG2("Kick: %d, Late: %d:: Warning! Packet late and not enough input data or not enoug output data to run again", sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.md_late_pkts);
...
static unsigned copy_packet(SCO_COMMON_RCV_OP_DATA* sco_data, stream_sco_metadata* sco_metadata)
{
     ...
     * by the SCO Rx FW is halved compared to when it is performed by
     * H/W (hence "amount_copied >> 1" instead of "amount_copied"). The
     * input data is read as 16bit words irrespective of whether the data
     * is in PCM or coded CVSD format. However, in the case of CVSD format
     * there is half the amount.
     */
    input_amount = amount_copied >> 1;
#else /* CVSD_CODEC_SOFTWARE */
    input_amount = amount_copied;
#endif /* CVSD_CODEC_SOFTWARE */

#ifdef INSTALL_PLC100
    output_amount = (space_limited ? amount_copied : output_words), /* output_generated (PLC) */
#else /* INSTALL_PLC100 */
    output_amount = amount_copied;
#endif /* INSTALL_PLC100 */

    /* both input and output buffers meta data needs updating,
     * this is done before actual copying. */
    sco_rcv_transport_metadata(
        sco_data,
        input_amount,   /* Input processed in 16bit words */
        output_amount,  /* Output generated (no PLC) */
        SCO_NB);
#endif /* SCO_RX_OP_GENERATE_METADATA */
