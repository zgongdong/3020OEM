...
typedef bool (*FRAME_SIZE_FN)(OPERATOR_DATA *op_data, unsigned *in_size_samples, unsigned *out_size_octets);
...
/**
 * \brief Undoes any base class initialisation. This should be called if 
 * encoder_base_class_init succeeds, but operator creation subsequently fails.
 *
 * \param vt Table containing all the attributes that the inheriting class has
 * implemented that this base class may need to invoke/use
 *
 * \return None.
 */
extern void encoder_base_class_deinit(const ENCODER_CAP_VIRTUAL_TABLE * const vt);
