...
bool build_mic_graph(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    Note: Sidetone part if required will be included later by
    ...
    */
     ...
     */
    if((op_extra_data->using_sidetone&SIDETONE_ENABLE_FLAG) != 0)
    {
        ...
        /* sidetone output */
        st_idx = num_io;
        op_extra_data->mic_st_idx = st_idx;
    ...
    if(st_idx != 0)
    {
        /* mic sidetone path will be inserted after this operator */
...
bool aec_reference_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        resp = cpsPack2Words(op_extra_data->sync_block.rm_adjustment, ref_last_inserts_total, resp);
        resp = cpsPack2Words(ref_spkr_refdrop, ref_st_drop, resp);
        cpsPack1Word(ref_micref_delay, resp);
#endif
...
bool aec_reference_opmsg_ep_configure(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    switch(msg->key)
    {
        ...
        case OPMSG_OP_TERMINAL_CBOPS_PARAMETERS:
        {
            ...
            shift = get_shift_from_cbops_parameters((CBOPS_PARAMETERS *)(uintptr_t)value);
            if (terminal_id == (AEC_REF_MIC_TERMINAL1|TERMINAL_SINK_MASK))
            {
                op_extra_data->mic_shift = shift;
            }
            else if (terminal_id == AEC_REF_SPKR_TERMINAL1)
            {
                op_extra_data->spkr_shift = shift;
            }
            break;
        }
