...
/*
 * APTX Adaptive Header definition
 */
/* Some says that the APTX Adaptive payload header size is 0*/
#define APTXAD_PAYLOAD_HEADER_SIZE     0
...
extern bool rtp_decode_opmsg_set_packing(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data);
extern bool rtp_decode_opmsg_set_src_latency_mapping(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data);
...
extern bool rtp_decode_opmsg_set_latency_change_notification(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data);

/* Functions for unpacking a 16-bit unpacked buffer to an array, octet by octet. */
extern void unpack_cbuff_to_array_16bit(int *dest, tCbuffer *cbuffer_src, unsigned int amount_to_copy);
extern void unpack_cbuff_to_array_from_offset_16bit(int *dest, tCbuffer *cbuffer_src, unsigned int amount_to_copy, unsigned int offset);
extern void unpack_cbuff_to_array_32bit(int *dest, tCbuffer *cbuffer_src, unsigned int amount_to_copy);
extern void unpack_cbuff_to_array_from_offset_32bit(int *dest, tCbuffer *cbuffer_src, unsigned int amount_to_copy, unsigned int offset);
