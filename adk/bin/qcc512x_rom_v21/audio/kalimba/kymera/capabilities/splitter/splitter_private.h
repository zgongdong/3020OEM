...
typedef struct
{
    ...
    bool finish_last_tag:8;

    /** True if splitter reframes the incoming timestamps. */
    bool reframe_enabled:8;

    /** If data format is audio, tag lengths saved to the internal buffer
     * are converted to sample. */
    bool tag_size_to_samples:8;
#ifdef SPLITTER_OFFLOAD
    /** Flag indicating if the internal buffer write and read is
     * offloaded to p1. */
    bool offload_enabled:8;
     ...
     * the buffer access.*/
    bool buffer_access:8;

    /** Flag indicatign the offloding is in use */
    bool offload_activated:8;

    /** Flag used to indicate that the next processing should start with unpack*/
    bool next_run_unpack:8;
#endif
     ...
     * This flag is only used when offloading to p1 is enabled. */
    bool kick_backward:8;

#if defined(SPLITTER_DEBUG)
    /* Debug variables */
    bool internal_buffer_empty:8;
    bool internal_buffer_full:8;
    bool output_buffer_full:8;
#endif
