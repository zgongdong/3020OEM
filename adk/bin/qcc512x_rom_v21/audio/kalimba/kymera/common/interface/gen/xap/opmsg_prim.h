...
/*******************************************************************************
 ...
 VALUES
    ...
    SET_HUFFMAN_TABLES_LOCATION - Sets the location of Huffman tables for AAC
                                  Decoder
...
typedef enum
{
    ...
    OPMSG_AAC_ID_SET_HUFFMAN_TABLES_LOCATION = 0x0003
} OPMSG_AAC_ID;
/*******************************************************************************
...
/*******************************************************************************
 ...
 VALUES
    SET_TASK_PERIOD               - AEC REFERENCE operator by default runs per
                                    1ms task period, this message is for
                                    changing the default task period and is
                                    mainly to reduce the cpu usage by the
                                    operator. There are some limitations, see
                                    the description of the message for details.
    SAME_INPUT_OUTPUT_CLK_SOURCE  - tell the operator that the back-end of the
                                    ...
                                    recommended to be enabled for use cases
                                    where speaker input and microphone output
                                    are synchronised (e.g. SCO and USB voice use
    ...
    ENABLE_SPKR_INPUT_GATE        - Enables AEC reference to have a TTP based
                                    ...
                                    speaker path.
    SET_INPUT_OUTPUT_SAMPLE_RATES - This input message allows a user to set the
                                    input & output sample rate. This is the same
                                    as SET_SAMPLE_RATES message but both input
                                    and output rates must be sent in 25Hz units.
...
typedef enum
{
    ...
    OPMSG_AEC_REFERENCE_ID_SET_INPUT_OUTPUT_SAMPLE_RATES = 0x00FD,
...
/*******************************************************************************
  ...
  DESCRIPTION
    ISO receive configuration messages
...
typedef enum
{
    OPMSG_ISO_RCV_ID_SET_TRANSPORT_DELAY = 0x00FC,
    OPMSG_ISO_RCV_ID_SET_FROM_AIR_INFO = 0x00FE
} OPMSG_ISO_RCV_ID;
/*******************************************************************************
...
/*******************************************************************************
 ...
 VALUES
    ...
    MESSAGE_RESPONSE    - Response to an operator message sent by the operator
                          ...
                          client in the form of an operator message with this id.
...
/*******************************************************************************
  ...
  DESCRIPTION
    TEST_ASYNC_WBS_LINK configuration messages.

 VALUES
    SET_FRAME_SIZE - Set the frame size.

*******************************************************************************/
typedef enum
{
    OPMSG_TAWL_ID_SET_FRAME_SIZE = 0x0001
} OPMSG_TAWL_ID;
/*******************************************************************************
 ...
 VALUES
    SET_CONNECTION_CONFIG - This message sends the usb connection config to the
                            operator.
    SET_RUN_DECIM_FACTOR  - Send this message to config the USB Audio Rx to slow
                            down the rate at which metadata are generated. In
                            future it also might be used to run the operator
                            less often.
...
typedef enum
{
    ...
    OPMSG_USB_AUDIO_ID_SET_RUN_DECIM_FACTOR = 0x0003
} OPMSG_USB_AUDIO_ID;
/*******************************************************************************
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Aac_Set_Huffman_Tables_Location

  DESCRIPTION
    Sets the location of Huffman tables for AAC Decoder

  MEMBERS
    message_id       - message id
    huffman_location - Location RAM (0), flash/ROM (1)
...
/* The following macros take OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION *opmsg_aac_set_huffman_tables_location_ptr */
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_MESSAGE_ID_GET(opmsg_aac_set_huffman_tables_location_ptr) ((OPMSG_AAC_ID)(opmsg_aac_set_huffman_tables_location_ptr)->_data[0])
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_MESSAGE_ID_SET(opmsg_aac_set_huffman_tables_location_ptr, message_id) ((opmsg_aac_set_huffman_tables_location_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_HUFFMAN_LOCATION_WORD_OFFSET (1)
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_HUFFMAN_LOCATION_GET(opmsg_aac_set_huffman_tables_location_ptr) ((opmsg_aac_set_huffman_tables_location_ptr)->_data[1])
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_HUFFMAN_LOCATION_SET(opmsg_aac_set_huffman_tables_location_ptr, huffman_location) ((opmsg_aac_set_huffman_tables_location_ptr)->_data[1] = (uint16)(huffman_location))
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_CREATE(message_id, huffman_location) \
    (uint16)(message_id), \
    (uint16)(huffman_location)
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_PACK(opmsg_aac_set_huffman_tables_location_ptr, message_id, huffman_location) \
    do { \
        (opmsg_aac_set_huffman_tables_location_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_aac_set_huffman_tables_location_ptr)->_data[1] = (uint16)((uint16)(huffman_location)); \
    } while (0)

#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_MARSHALL(addr, opmsg_aac_set_huffman_tables_location_ptr) memcpy((void *)(addr), (void *)(opmsg_aac_set_huffman_tables_location_ptr), 2)
#define OPMSG_AAC_SET_HUFFMAN_TABLES_LOCATION_UNMARSHALL(addr, opmsg_aac_set_huffman_tables_location_ptr) memcpy((void *)(opmsg_aac_set_huffman_tables_location_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  DESCRIPTION
    AEC operator message for SET_INPUT_OUTPUT_SAMPLE_RATES.

  MEMBERS
    message_id  - message id
    input_rate  - input rate in 25Hz units
...
/* The following macros take OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES *opmsg_aec_set_input_output_sample_rates_ptr */
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_MESSAGE_ID_GET(opmsg_aec_set_input_output_sample_rates_ptr) ((OPMSG_AEC_REFERENCE_ID)(opmsg_aec_set_input_output_sample_rates_ptr)->_data[0])
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_MESSAGE_ID_SET(opmsg_aec_set_input_output_sample_rates_ptr, message_id) ((opmsg_aec_set_input_output_sample_rates_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_INPUT_RATE_WORD_OFFSET (1)
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_INPUT_RATE_GET(opmsg_aec_set_input_output_sample_rates_ptr) ((opmsg_aec_set_input_output_sample_rates_ptr)->_data[1])
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_INPUT_RATE_SET(opmsg_aec_set_input_output_sample_rates_ptr, input_rate) ((opmsg_aec_set_input_output_sample_rates_ptr)->_data[1] = (uint16)(input_rate))
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_OUTPUT_RATE_WORD_OFFSET (2)
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_OUTPUT_RATE_GET(opmsg_aec_set_input_output_sample_rates_ptr) ((opmsg_aec_set_input_output_sample_rates_ptr)->_data[2])
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_OUTPUT_RATE_SET(opmsg_aec_set_input_output_sample_rates_ptr, output_rate) ((opmsg_aec_set_input_output_sample_rates_ptr)->_data[2] = (uint16)(output_rate))
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_WORD_SIZE (3)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_CREATE(message_id, input_rate, output_rate) \
    (uint16)(message_id), \
    (uint16)(input_rate), \
    (uint16)(output_rate)
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_PACK(opmsg_aec_set_input_output_sample_rates_ptr, message_id, input_rate, output_rate) \
    do { \
        (opmsg_aec_set_input_output_sample_rates_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_aec_set_input_output_sample_rates_ptr)->_data[1] = (uint16)((uint16)(input_rate)); \
        (opmsg_aec_set_input_output_sample_rates_ptr)->_data[2] = (uint16)((uint16)(output_rate)); \
    } while (0)

#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_MARSHALL(addr, opmsg_aec_set_input_output_sample_rates_ptr) memcpy((void *)(addr), (void *)(opmsg_aec_set_input_output_sample_rates_ptr), 3)
#define OPMSG_AEC_SET_INPUT_OUTPUT_SAMPLE_RATES_UNMARSHALL(addr, opmsg_aec_set_input_output_sample_rates_ptr) memcpy((void *)(opmsg_aec_set_input_output_sample_rates_ptr), (void *)(addr), 3)
...
/*******************************************************************************
  ...
  DESCRIPTION
    NOT IMPLEMENTED - will probably change Operator common message for
    setting algorithmic delay to apply to metadata. This ensures metadata
    tags remain aligned with the processed samples.
...
/*******************************************************************************
  ...
  MEMBERS
    ...
    count       - count
    packed_data - packed data, MSB first [31:16] [15:0]
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Tawl_Set_Frame_Size

  DESCRIPTION
    TEST_ASYNC_WBS_LINK operator message for SET_FRAME_SIZE.
...
/* The following macros take OPMSG_TAWL_SET_FRAME_SIZE *opmsg_tawl_set_frame_size_ptr */
#define OPMSG_TAWL_SET_FRAME_SIZE_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_TAWL_SET_FRAME_SIZE_MESSAGE_ID_GET(opmsg_tawl_set_frame_size_ptr) ((OPMSG_TAWL_ID)(opmsg_tawl_set_frame_size_ptr)->_data[0])
#define OPMSG_TAWL_SET_FRAME_SIZE_MESSAGE_ID_SET(opmsg_tawl_set_frame_size_ptr, message_id) ((opmsg_tawl_set_frame_size_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_TAWL_SET_FRAME_SIZE_FRAME_SIZE_WORD_OFFSET (1)
#define OPMSG_TAWL_SET_FRAME_SIZE_FRAME_SIZE_GET(opmsg_tawl_set_frame_size_ptr) ((opmsg_tawl_set_frame_size_ptr)->_data[1])
#define OPMSG_TAWL_SET_FRAME_SIZE_FRAME_SIZE_SET(opmsg_tawl_set_frame_size_ptr, frame_size) ((opmsg_tawl_set_frame_size_ptr)->_data[1] = (uint16)(frame_size))
#define OPMSG_TAWL_SET_FRAME_SIZE_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_TAWL_SET_FRAME_SIZE_CREATE(message_id, frame_size) \
    (uint16)(message_id), \
    (uint16)(frame_size)
#define OPMSG_TAWL_SET_FRAME_SIZE_PACK(opmsg_tawl_set_frame_size_ptr, message_id, frame_size) \
    do { \
        (opmsg_tawl_set_frame_size_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_tawl_set_frame_size_ptr)->_data[1] = (uint16)((uint16)(frame_size)); \
    } while (0)

#define OPMSG_TAWL_SET_FRAME_SIZE_MARSHALL(addr, opmsg_tawl_set_frame_size_ptr) memcpy((void *)(addr), (void *)(opmsg_tawl_set_frame_size_ptr), 2)
#define OPMSG_TAWL_SET_FRAME_SIZE_UNMARSHALL(addr, opmsg_tawl_set_frame_size_ptr) memcpy((void *)(opmsg_tawl_set_frame_size_ptr), (void *)(addr), 2)
...
/*******************************************************************************
  ...
  NAME
    Opmsg_Usb_Audio_Set_Run_Decim_Factor

  DESCRIPTION
    sends the SET_RUN_DECIM_FACTOR to the USB Audio Rx operator

  MEMBERS
    message_id      - message id
    decim_factor_ms - decimation factor in ms (i.e. how often)
...
/* The following macros take OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR *opmsg_usb_audio_set_run_decim_factor_ptr */
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_MESSAGE_ID_WORD_OFFSET (0)
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_MESSAGE_ID_GET(opmsg_usb_audio_set_run_decim_factor_ptr) ((OPMSG_USB_AUDIO_ID)(opmsg_usb_audio_set_run_decim_factor_ptr)->_data[0])
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_MESSAGE_ID_SET(opmsg_usb_audio_set_run_decim_factor_ptr, message_id) ((opmsg_usb_audio_set_run_decim_factor_ptr)->_data[0] = (uint16)(message_id))
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_DECIM_FACTOR_MS_WORD_OFFSET (1)
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_DECIM_FACTOR_MS_GET(opmsg_usb_audio_set_run_decim_factor_ptr) ((opmsg_usb_audio_set_run_decim_factor_ptr)->_data[1])
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_DECIM_FACTOR_MS_SET(opmsg_usb_audio_set_run_decim_factor_ptr, decim_factor_ms) ((opmsg_usb_audio_set_run_decim_factor_ptr)->_data[1] = (uint16)(decim_factor_ms))
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_WORD_SIZE (2)
/*lint -e(773) allow unparenthesized*/
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_CREATE(message_id, decim_factor_ms) \
    (uint16)(message_id), \
    (uint16)(decim_factor_ms)
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_PACK(opmsg_usb_audio_set_run_decim_factor_ptr, message_id, decim_factor_ms) \
    do { \
        (opmsg_usb_audio_set_run_decim_factor_ptr)->_data[0] = (uint16)((uint16)(message_id)); \
        (opmsg_usb_audio_set_run_decim_factor_ptr)->_data[1] = (uint16)((uint16)(decim_factor_ms)); \
    } while (0)

#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_MARSHALL(addr, opmsg_usb_audio_set_run_decim_factor_ptr) memcpy((void *)(addr), (void *)(opmsg_usb_audio_set_run_decim_factor_ptr), 2)
#define OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR_UNMARSHALL(addr, opmsg_usb_audio_set_run_decim_factor_ptr) memcpy((void *)(opmsg_usb_audio_set_run_decim_factor_ptr), (void *)(addr), 2)
