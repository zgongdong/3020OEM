...
/*******************************************************************************
  ...
  NAME
    Accmd_Audio_Endpoint_disable_processing

  DESCRIPTION
    Permissable Values for STREAM_AUDIO_DISABLE_ENDPOINT_PROCESSING
...
typedef enum
{
    ACCMD_AUDIO_ENDPOINT_DISABLE_PROCESSING_ALL = 0x0000,
    ACCMD_AUDIO_ENDPOINT_DISABLE_PROCESSING_DC_REMOVE = 0x0001
} ACCMD_AUDIO_ENDPOINT_DISABLE_PROCESSING;
/*******************************************************************************
...
/*******************************************************************************
  ...
  DESCRIPTION
    ...
    digital mic keys 0x0700 - 0x07FF typically used for keys applicable to
    all interfaces 0x0710 - 0x071F for end-point processing (Cbops) 0x0900 -
    0x09FF typically used for FastPipe keys 0x1000 - 0x10FF typically used
    for PWM keys 0x1100 - 0x11FF typically used for ANC 0x1200 - 0x12FF for
    rate measurement/rate matching.

 VALUES
    STREAM_SOURCE_NOTIFY_WHEN_DRAINED
                   - Set the "Notify when Drained" flag. When set, firmware will
                     generate HQ event when the stream buffer has been drained
                     completely.
    STREAM_PCM_SYNC_RATE
                   - PCM synchronisation rate in hertz.
    STREAM_PCM_MASTER_CLOCK_RATE
                   - 0: master clock rate will be derived from synchronisation
                     rate, slot count and sample format Else: Bit clock rate in
                     Hz (Note: the name MASTER_CLOCK_RATE is historical; this
                     key pertains to the bit clock frequency, not the frequency
                     of any MCLK signal in the system.)
    STREAM_PCM_MASTER_MODE
                   - Enable PCM master mode 0: Device is PCM slave 1: Device is
                     PCM master
    STREAM_PCM_SLOT_COUNT
                   - PCM slot count 0: Derived from the clock and the sync
                     rates. 1..4: Number of slots present between sync pulses.
    STREAM_PCM_MANCH_MODE_ENABLE
                   - -Enable PCM Manchester encoding mode 0: Disable 1: Enable
                     Only available when not using "low jitter mode". This key
                     is no longer used on HYDRA based chips
    STREAM_PCM_SHORT_SYNC_ENABLE
                   - Enable PCM short synchronisation - 0: Long frame sync
                     (rising edge indicates start of frame and Sync pulse width
                     depends on STREAM_PCM_LONG_LENGTH_SYNC_ENABLE) 1: Short
                     frame sync (falling edge indicates start of frame and sync
                     pulse width is of 1 bit clock cycle)
    STREAM_PCM_MANCH_SLAVE_MODE_ENABLE
                   - Enable PCM Manchester slave mode : 0: does not force
                     synchronization of transmit and receive frames. 1: forces
                     transmit frames to follow receive frames with constant
                     delay. This key is no longer used on HYDRA based chips
    STREAM_PCM_SIGN_EXTEND_ENABLE
                   - 0: padding of 8 or 13-bit sample into a 16-bit slot by
                     inserting extra LSBs. When padding is selected with 13-bit
                     sample, the 3 padding bits are the audio gain setting; with
                     8-bit sample the 8 padding bits are zeroes. 1:
                     sign-extension
    STREAM_PCM_LSB_FIRST_ENABLE
                   - Enable PCM LSB first - control order in which samples are
                     transmitted and received: 0: most significant bit first 1:
                     least significant bit first
    STREAM_PCM_TX_TRISTATE_ENABLE
                   - Enable PCM Tx tristate - 0: drive PCM_OUT continuously. 1:
                     tri-state PCM_OUT immediately after falling edge of PCM_CLK
                     in the last bit of an active slot, assuming the next slot
                     is not active
    STREAM_PCM_TX_TRISTATE_RISING_EDGE_ENABLE
                   - Enable PCM Tx tristate rising edge (Only effective if TX
                     tri-state mode is enabled) 0: sets PCM_OUT to high
                     impedance state after the falling edge of PCM_CLK of the
                     last bit of of an active slot (assuming the next slot is
                     also not active) 1: sets PCM_OUT to high impedance after
                     the rising edge of PCM_CLK after the last data bit of an
                     active slot
    STREAM_PCM_SYNC_SUPPRESS_ENABLE
                   - Enable PCM synchronisation suppress 0: enables PCM_SYNC
                     output when master 1: suppresses PCM_SYNC whilst keeping
                     PCM_CLK running (some external codec devices use this to
                     enter a low power state)
    STREAM_PCM_GCI_MODE_ENABLE
                   - 0: Disable 1: Enable
    STREAM_PCM_MUTE_ENABLE
                   - (This is deprecated in favor of STREAM_AUDIO_MUTE_ENABLE.)
                     0: Disable Mute 1: Force PCM_OUT to 0
    STREAM_PCM_LONG_LENGTH_SYNC_ENABLE
                   - Only applies if STREAM_PCM_SHORT_SYNC_ENABLE is 0. 0: sets
                     PCM_SYNC length to 8 PCM_CLK cycles 1: sets length to 16
                     PCM_CLK cycles
    STREAM_PCM_SAMPLE_RISING_EDGE_ENABLE
                   - Enable PCM sample rising edge 0: sample PCM_DATA on falling
                     edge of PCM_CLK 1: sample PCM_DATA on rising edge of PCM_CLK
    STREAM_PCM_SAMPLE_FORMAT
                   - This key is only provided for backward compatibility with
                     bluecore. New code should use STREAM_AUDIO_SAMPLE_SIZE that
                     supports 24-bit format in addition to the formats below.
                     Selects one of the following formats of audio sample on PCM
                     interface(all channels, both directions): 0: 13 bits in a
                     16-bit slot 1: 16 bits in a 16-bit slot 2: 8 bits in a
                     16-bit slot 3: 8 bits in an 8-bit slot
    STREAM_PCM_MANCH_RX_OFFSET
                   - PCM manchester receive offset: when in Manchester mode,
                     selects delay between receiving start bit and sampling the
                     first significant bit from the sample. valid range : 0..3
                     inclusive. This key is no longer used on HYDRA based chips
    STREAM_PCM_AUDIO_GAIN
                   - PCM audio gain. Valid range: 0 to 7. Used to pad the end 3
                     bits of a 13 bit PCM sample. It is used by some CODECs
                     (connected to the PCM interface) to allow their gain to be
                     controlled.
    STREAM_PCM_MASTER_CLK_SOURCE
                   - Set the source of the master clock for a PCM interface. By
                     ...
                     can be from internal MPLL or an external source MCLK_IN. 0
                     (SYSTEM_ROOT_CLK): Interface clock derived from fixed
                     internal "root" clock. 1 (MCLK): Interface clock derived
                     from MCLK source configured with
                     AudioMasterClockConfigure(). Before the interface can be
                     used, Source/SinkMasterClockEnable() must be called per
                     interface. 2 (BORROW_PCM0): Interface clock derived from
                     the same source as the first PCM/I2S interface. 3
                     (BORROW_EPCM): Interface clock derived from the same source
                     as the fourth PCM/I2S interface.
    STREAM_PCM_MASTER_MCLK_MULT
                   - Ratio of MCLK to sample rate. Valid choices are 128, 256 or
                     384.
    STREAM_PCM_24BITS_IN_32BIT_SLOT
                   - The Key when set, overrides both STREAM_PCM_SAMPLE_FORMAT
                     and STREAM_AUDIO_SAMPLE_SIZE and enables 24 bit in 32 bit
                     mode. 0: Default behavior dictated by
                     STREAM_AUDIO_SAMPLE_SIZE or STREAM_PCM_SAMPLE_FORMAT 1: 24
                     Bits in 32-bit slot
    STREAM_I2S_SYNC_RATE
                   - I2S synchronisation rate in hertz
    STREAM_I2S_MASTER_CLOCK_RATE
                   - 0: Auto-calculated, Else: bit clock (SCLK) rate in Hz
                     (Note: the name MASTER_CLOCK_RATE is historical; this key
                     pertains to the bit clock frequency, not the frequency of
                     any MCLK signal in the system.)
    STREAM_I2S_MASTER_MODE
                   - Enable I2S master mode - clock and sync will be generated
                     by the I2S hardware 0: Device is I2S slave 1: Device is I2S
                     master
    STREAM_I2S_JSTFY_FORMAT
                   - 0: Left justified 1: Right justified
    STREAM_I2S_LFT_JSTFY_DLY
                   - If using left-justified format: 0: MSB of SD data occurs in
                     the first SCLK period following the WS transition 1: MSB of
                     SD data occurs in the second SCLK period This should always
                     be set to 1 for standard I2S format.
    STREAM_I2S_CHNL_PLRTY
                   - I2S channel polarity. Valid values: 0: SD data is left
                     channel when WS is high 1: SD data is right channel when WS
                     is high This should always be set to 1 for standard I2S
                     format.
    STREAM_I2S_AUDIO_ATTEN_ENABLE
                   - Enables/disables the attenuation applied to incoming (into
                     device) audio samples in cases where
                     STREAM_AUDIO_SAMPLE_SIZE for the I2S instance is set to 16
                     but STREAM_I2S_BITS_PER_SAMPLE is set to 20/24 bits and
                     STREAM_I2S_CROP_ENABLE is 0. Used with
                     "STREAM_I2S_AUDIO_ATTEN". 0: 17 bit SD data is rounded down
                     to 16 bits 1: the attenuation specified by
                     STREAM_I2S_AUDIO_ATTEN is applied over 24 bits with
                     saturated rounding
    STREAM_I2S_AUDIO_ATTEN
                   - I2S attenuation in 6 dB steps. Valid range: 0 to 15
                     inclusive
    STREAM_I2S_JSTFY_RES
                   - I2S justify resolution - Resolution of data on SD_IN, 00=16
                     bit, 01=20 bit, 10=24 bit, 11=Reserved. This is required
                     for right justified format and with left justified LSB
                     first
    STREAM_I2S_CROP_ENABLE
                   - In this mode the most significant bits after sync are taken
                     to be the sample data. If I2sBitsPerSample is configured
                     for 24/32 bits per sample, but STREAM_AUDIO_SAMPLE_SIZE is
                     configured for 16 bits per sample only, then crop enable
                     decides whether the I2S interface rounds or truncates
                     incoming 24/32 bits to 16 bits. Additionally, it must be
                     enabled if in 16 bit per sample mode when CLK rate =
                     32*sample rate. 0: Disable Cropping(or select rounding) 1:
                     Enable Cropping From CSRA681xx, the crop mode is selected
                     automatically, and there is no need to set this key; it
                     should be left at its default value.
    STREAM_I2S_BITS_PER_SAMPLE
                   - This key specifies the number of bit clocks reserved for
                     each (mono) sample on the first I2S interface. Valid
                     settings are 16, 20, or 24 bits per sample. This key does
                     not specify how many of those bits are used for internal
                     data processing; see STREAM_AUDIO_SAMPLE_SIZE for that. If
                     this key is set to a value larger than
                     STREAM_AUDIO_SAMPLE_SIZE, the additional bits are output as
                     zeroes in the least significant bits. On the input, if
                     sample size is 16 bits and bits-per-sample is greater, an
                     attenuation can be applied, or samples larger than 16bit
                     can be cropped to 16bits. This key is also used to
                     calculate the bit clock rate when in master mode and the
                     I2sMasterClockRate key is not set. (I2sMasterClockRate
                     takes precedence over this key for the purposes of
                     supplying the master clock rate.) The calculation is bits
                     per sample * 2 (channels) * sample rate.
    STREAM_I2S_TX_START_SAMPLE
                   - I2S start Tx sampling 0: during low wclk phase. 1: during
                     high wclk phase.
    STREAM_I2S_RX_START_SAMPLE
                   - I2S start Rx sampling 0: during low wclk phase. 1: during
                     high wclk phase.
    ...
    STREAM_DIGITAL_MIC_CLOCK_RATE
                   - Digital MIC clock rate in kHz. Valid values for clock rate
                     :  500 (500 kHz), 571 (571.43 KHz), 666 (666.67 KHz), 1000
                     (1 MHz), 1333 (1.33MHz), 2000 (2 MHz), 4000 (4 MHz).
    ...
    STREAM_AUDIO_DISABLE_ENDPOINT_PROCESSING
                   - Allow disabling endpoint processing before starting
                     (connecting) an endpoint 0: Disable all endpoint procesing
                     for this end point 1: Disable DC Remove for this end point
    STREAM_AUDIO_SRA_FILTER
                   - Configure output SRA filter quality. 0 - normal    quality.
                     1 - high      quality. 2 - very high quality.
    ...
    STREAM_RM_USE_RATE_ADJUST_OPERATOR
                   - Configure audio endpoint to use a standalone RATE_ADJUST
                     operator for performing rate adjustment. value: operator id
                     of the rate adjustment operator.
...
typedef enum
{
    ...
    ACCMD_CONFIG_KEY_STREAM_PCM_24BITS_IN_32BIT_SLOT = 0x011a,
    ...
    ACCMD_CONFIG_KEY_STREAM_AUDIO_DISABLE_ENDPOINT_PROCESSING = 0x0710,
    ACCMD_CONFIG_KEY_STREAM_AUDIO_SRA_FILTER = 0x0711,
    ...
    ACCMD_CONFIG_KEY_STREAM_RM_USE_RATE_ADJUST_OPERATOR = 0x1208
} ACCMD_CONFIG_KEY;
/*******************************************************************************
  ...
  DESCRIPTION
    ...
    error at the request. Implementation note: This type must be kept in line
    with the internal type "CLK_MGR_CPU_CLK". reserved for future use
...
/*******************************************************************************
  ...
  DESCRIPTION
    Implementation note: This type must be kept in line with the internal
    type "CLK_MGR_POWER_SAVE_MODES". reserved for future use
...
/*******************************************************************************
  ...
  DESCRIPTION
    Request messages 0x001B formerly "SET_SYSTEM_STREAM_RATE_REQ", now
    unused. 0x001C formerly "GET_SYSTEM_STREAM_RATE_REQ", now unused.
    Response messages 0x4014 formerly "SET_SYSTEM_STREAM_RATE_RESP", now
    unused. 0x4015 formerly "GET_SYSTEM_STREAM_RATE_RESP", now unused. Test
    Request messages 0x2003 formerly "SET_DEBUG_ENABLES_REQ", now unused.
    Test response messages Debug messages, sent as indications 0x8000
    formerly "DEBUG_MESSAGE_IND", now unused. 0x8002 formerly
    "DEBUG_SEND_CONFIG", now unused.
...
typedef enum
{
    ...
    ACCMD_SIGNAL_ID_FILE_DETAILS_REQ = 0x0041,
    ACCMD_SIGNAL_ID_OPERATOR_DELEGATE_REQ = 0x0042,
    ACCMD_SIGNAL_ID_PRESERVE_REQ = 0x0043,
    ...
    ACCMD_SIGNAL_ID_FILE_DETAILS_RESP = 0x403D,
...
/*******************************************************************************
  ...
  NAME
    Accmd_Stream_Audio_Sample_Size

  DESCRIPTION
    Permissable Values for STREAM_AUDIO_SAMPLE_SIZE

 VALUES
    13BITS_16CLKS - 13 bits in 16 cycle slot duration
    16BITS_16CLKS - 16 bits in 16 cycle slot duration
    8BITS_16CLKS  - 8 bits in 16 cycle slot duration
    8BITS_8CLKS   - 8 bits in 8 cycle slot duration
    16            -
    24            -
...
typedef enum
{
    ACCMD_STREAM_AUDIO_SAMPLE_SIZE_13BITS_16CLKS = 0x0000,
    ACCMD_STREAM_AUDIO_SAMPLE_SIZE_16BITS_16CLKS = 0x0001,
    ACCMD_STREAM_AUDIO_SAMPLE_SIZE_8BITS_16CLKS = 0x0002,
    ACCMD_STREAM_AUDIO_SAMPLE_SIZE_8BITS_8CLKS = 0x0003,
    ACCMD_STREAM_AUDIO_SAMPLE_SIZE_16 = 0x0010,
    ACCMD_STREAM_AUDIO_SAMPLE_SIZE_24 = 0x0018
} ACCMD_STREAM_AUDIO_SAMPLE_SIZE;
/*******************************************************************************
  ...
  DESCRIPTION
    ...
    0x0008 FILE = 0x000A APPDATA = 0x000B I2C = 0x000D USB_ENDPOINT = 0x000E
    TIMESTAMPED = 0x0011
...
typedef enum
{
    ...
    ACCMD_STREAM_DEVICE_L2CAP = 0x000C,
...
/*******************************************************************************
  ...
  DESCRIPTION
    ANC control bitfield ANC ZCD_SHIFT bit-field position ANC ZCD_SHIFT
...
typedef enum
{
    ...
    ACCMD_ANC_CONTROL_DEFAULT_ZCD_SHIFT_MASK = 0x1000,
    ACCMD_ANC_CONTROL_ZCD_SHIFT_MASK = 0x1c00,
    ACCMD_ANC_CONTROL_COEF_SMP_EN_MASK = 0x2000,
    ACCMD_ANC_CONTROL_FFGAIN_SMP_EN_MASK = 0x4000,
    ACCMD_ANC_CONTROL_VALID_MASK = 0x7fff,
    ACCMD_ANC_CONTROL_ZCD_SHIFT_POSITION = 10
} ACCMD_ANC_CONTROL;
/*******************************************************************************
...
/*******************************************************************************
  ...
  MEMBERS
    low_power_mode_clk - Clock Configuration for Low Power Mode
    custom_mode_clk    - Clock Configuration for Custom Mode
    active_mode_clk    - Clock Configuration for Active Mode
...
#define ACCMD_CLOCK_CONFIGURE_REQ_CUSTOM_MODE_CLK_WORD_OFFSET (4)
#define ACCMD_CLOCK_CONFIGURE_REQ_CUSTOM_MODE_CLK_GET(addr) ((ACCMD_CPU_CLK)*((addr) + 4))
#define ACCMD_CLOCK_CONFIGURE_REQ_CUSTOM_MODE_CLK_SET(addr, custom_mode_clk) (*((addr) + 4) = (uint16)(custom_mode_clk))
...
#define ACCMD_CLOCK_CONFIGURE_REQ_PACK(addr, low_power_mode_clk, custom_mode_clk, active_mode_clk) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(low_power_mode_clk)); \
        *((addr) + 4) = (uint16)((uint16)(custom_mode_clk)); \
...
#define ACCMD_CLOCK_CONFIGURE_REQ_MARSHALL(addr, accmd_clock_configure_req_ptr) \
    do { \
        ...
        *((addr) + 4) = (uint16)((accmd_clock_configure_req_ptr)->custom_mode_clk); \
...
#define ACCMD_CLOCK_CONFIGURE_REQ_UNMARSHALL(addr, accmd_clock_configure_req_ptr) \
    do { \
        ...
        (accmd_clock_configure_req_ptr)->custom_mode_clk = ACCMD_CLOCK_CONFIGURE_REQ_CUSTOM_MODE_CLK_GET(addr); \
...
/*******************************************************************************
  ...
  NAME
    Accmd_Cap_Download_Add_Kcs_Req

  DESCRIPTION
    request KCS to be downloaded.

  MEMBERS
    processor_id - Id of the Processor to add the KCS
...
/* The following macros take ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ *accmd_cap_download_add_kcs_req_ptr or uint16 *addr */
#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_PROCESSOR_ID_WORD_OFFSET (3)
#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_PROCESSOR_ID_GET(addr) (*((addr) + 3))
#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_PROCESSOR_ID_SET(addr, processor_id) (*((addr) + 3) = (uint16)(processor_id))
#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_WORD_SIZE (4)
#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_PACK(addr, processor_id) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(processor_id)); \
    } while (0)

#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_MARSHALL(addr, accmd_cap_download_add_kcs_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_cap_download_add_kcs_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_cap_download_add_kcs_req_ptr)->processor_id); \
    } while (0)

#define ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_UNMARSHALL(addr, accmd_cap_download_add_kcs_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_cap_download_add_kcs_req_ptr)->header)); \
        (accmd_cap_download_add_kcs_req_ptr)->processor_id = ACCMD_CAP_DOWNLOAD_ADD_KCS_REQ_PROCESSOR_ID_GET(addr); \
    } while (0)
...
/*******************************************************************************
  ...
  NAME
    Accmd_Cap_Download_Complete_Kcs_Req

  DESCRIPTION
    KCS download is complete.

  MEMBERS
    Status - Returns STATUS_OK for successfull retrieval
    KCS_id - Id of KCS that has been added
...
/* The following macros take ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ *accmd_cap_download_complete_kcs_req_ptr or uint16 *addr */
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_STATUS_WORD_OFFSET (3)
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_STATUS_GET(addr) ((ACCMD_STATUS)*((addr) + 3))
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_STATUS_SET(addr, status) (*((addr) + 3) = (uint16)(status))
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_KCS_ID_WORD_OFFSET (4)
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_KCS_ID_GET(addr) (*((addr) + 4))
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_KCS_ID_SET(addr, kcs_id) (*((addr) + 4) = (uint16)(kcs_id))
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_WORD_SIZE (5)
#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_PACK(addr, Status, KCS_id) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(Status)); \
        *((addr) + 4) = (uint16)((uint16)(KCS_id)); \
    } while (0)

#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_MARSHALL(addr, accmd_cap_download_complete_kcs_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_cap_download_complete_kcs_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_cap_download_complete_kcs_req_ptr)->Status); \
        *((addr) + 4) = (uint16)((accmd_cap_download_complete_kcs_req_ptr)->KCS_id); \
    } while (0)

#define ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_UNMARSHALL(addr, accmd_cap_download_complete_kcs_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_cap_download_complete_kcs_req_ptr)->header)); \
        (accmd_cap_download_complete_kcs_req_ptr)->Status = ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_STATUS_GET(addr); \
        (accmd_cap_download_complete_kcs_req_ptr)->KCS_id = ACCMD_CAP_DOWNLOAD_COMPLETE_KCS_REQ_KCS_ID_GET(addr); \
    } while (0)
...
/*******************************************************************************
  ...
  NAME
    Accmd_File_Details_Req

  DESCRIPTION
    From AudioSS This command requests the address and size of a file
    residing in the Apps Flash. The Filename is a NULL-teminated string
    packed as uint16s: data endianness - first octet of Filename goes in the
    LSB; length(Filename) below doesn't include the NULL termination,
    if(length(Filename) == odd), nothing unusual, last uint16 contains the
    NULL termination in the MSB; if(length(Filename) == even), last uint16
    will contain two NULL characters,

  MEMBERS
    Filename - Filename - string packed as uint16s

*******************************************************************************/
typedef struct
{
    ACCMD_HEADER header;
    uint16       Filename[ACCMD_PRIM_ANY_SIZE];
} ACCMD_FILE_DETAILS_REQ;

/* The following macros take ACCMD_FILE_DETAILS_REQ *accmd_file_details_req_ptr or uint16 *addr */
#define ACCMD_FILE_DETAILS_REQ_FILENAME_WORD_OFFSET (3)
#define ACCMD_FILE_DETAILS_REQ_FILENAME_GET(addr) (*((addr) + 3))
#define ACCMD_FILE_DETAILS_REQ_FILENAME_SET(addr, filename) (*((addr) + 3) = (uint16)(filename))
#define ACCMD_FILE_DETAILS_REQ_WORD_SIZE (4)
#define ACCMD_FILE_DETAILS_REQ_PACK(addr, Filename) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(Filename)); \
    } while (0)

#define ACCMD_FILE_DETAILS_REQ_MARSHALL(addr, accmd_file_details_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_file_details_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_file_details_req_ptr)->Filename); \
    } while (0)

#define ACCMD_FILE_DETAILS_REQ_UNMARSHALL(addr, accmd_file_details_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_file_details_req_ptr)->header)); \
        (accmd_file_details_req_ptr)->Filename = ACCMD_FILE_DETAILS_REQ_FILENAME_GET(addr); \
    } while (0)
...
/*******************************************************************************
  ...
  NAME
    Accmd_Operator_Delegate_Req

  DESCRIPTION
    From Client Delegate control of a list of operators to another 'graph
    manager' operator, or return control to the ACCMD client. Response is a
    Standard_Resp.

  MEMBERS
    op_client - The ID of the operator to which control is delegated, or 0 to
                revert control to the client
    count     - The number of delegated operators in the list
    op_list   - The list of delegated operators
...
typedef struct
{
    ...
    uint16       op_client;
    uint16       count;
    uint16       op_list[ACCMD_PRIM_ANY_SIZE];
} ACCMD_OPERATOR_DELEGATE_REQ;

/* The following macros take ACCMD_OPERATOR_DELEGATE_REQ *accmd_operator_delegate_req_ptr or uint16 *addr */
#define ACCMD_OPERATOR_DELEGATE_REQ_OP_CLIENT_WORD_OFFSET (3)
#define ACCMD_OPERATOR_DELEGATE_REQ_OP_CLIENT_GET(addr) (*((addr) + 3))
#define ACCMD_OPERATOR_DELEGATE_REQ_OP_CLIENT_SET(addr, op_client) (*((addr) + 3) = (uint16)(op_client))
#define ACCMD_OPERATOR_DELEGATE_REQ_COUNT_WORD_OFFSET (4)
#define ACCMD_OPERATOR_DELEGATE_REQ_COUNT_GET(addr) (*((addr) + 4))
#define ACCMD_OPERATOR_DELEGATE_REQ_COUNT_SET(addr, count) (*((addr) + 4) = (uint16)(count))
#define ACCMD_OPERATOR_DELEGATE_REQ_OP_LIST_WORD_OFFSET (5)
#define ACCMD_OPERATOR_DELEGATE_REQ_OP_LIST_GET(addr) (*((addr) + 5))
#define ACCMD_OPERATOR_DELEGATE_REQ_OP_LIST_SET(addr, op_list) (*((addr) + 5) = (uint16)(op_list))
#define ACCMD_OPERATOR_DELEGATE_REQ_WORD_SIZE (6)
#define ACCMD_OPERATOR_DELEGATE_REQ_PACK(addr, op_client, count, op_list) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(op_client)); \
        *((addr) + 4) = (uint16)((uint16)(count)); \
        *((addr) + 5) = (uint16)((uint16)(op_list)); \
    } while (0)

#define ACCMD_OPERATOR_DELEGATE_REQ_MARSHALL(addr, accmd_operator_delegate_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_operator_delegate_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_operator_delegate_req_ptr)->op_client); \
        *((addr) + 4) = (uint16)((accmd_operator_delegate_req_ptr)->count); \
        *((addr) + 5) = (uint16)((accmd_operator_delegate_req_ptr)->op_list); \
    } while (0)

#define ACCMD_OPERATOR_DELEGATE_REQ_UNMARSHALL(addr, accmd_operator_delegate_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_operator_delegate_req_ptr)->header)); \
        (accmd_operator_delegate_req_ptr)->op_client = ACCMD_OPERATOR_DELEGATE_REQ_OP_CLIENT_GET(addr); \
        (accmd_operator_delegate_req_ptr)->count = ACCMD_OPERATOR_DELEGATE_REQ_COUNT_GET(addr); \
        (accmd_operator_delegate_req_ptr)->op_list = ACCMD_OPERATOR_DELEGATE_REQ_OP_LIST_GET(addr); \
    } while (0)
...
/*******************************************************************************
  ...
  DESCRIPTION
    From Client Mark operators and/or endpoints for preservation even when
    the ACCMD client goes away, or revert this status. Response is a
    Standard_Resp.

  MEMBERS
    preserve - Boolean semantics (for now): 0xFFFF = preserve
               operators/endpoints 0      = don't preserve (default)
    op_count - The number of operators in the list
    op_list  - The list of operators
    ep_count - The number of endpoints in the list
    ep_list  - The list of endpoints
...
typedef struct
{
    ...
    uint16       op_list[ACCMD_PRIM_ANY_SIZE];
    uint16       ep_count;
    uint16       ep_list[ACCMD_PRIM_ANY_SIZE];
} ACCMD_PRESERVE_REQ;

/* The following macros take ACCMD_PRESERVE_REQ *accmd_preserve_req_ptr or uint16 *addr */
#define ACCMD_PRESERVE_REQ_PRESERVE_WORD_OFFSET (3)
#define ACCMD_PRESERVE_REQ_PRESERVE_GET(addr) (*((addr) + 3))
#define ACCMD_PRESERVE_REQ_PRESERVE_SET(addr, preserve) (*((addr) + 3) = (uint16)(preserve))
#define ACCMD_PRESERVE_REQ_OP_COUNT_WORD_OFFSET (4)
#define ACCMD_PRESERVE_REQ_OP_COUNT_GET(addr) (*((addr) + 4))
#define ACCMD_PRESERVE_REQ_OP_COUNT_SET(addr, op_count) (*((addr) + 4) = (uint16)(op_count))
#define ACCMD_PRESERVE_REQ_OP_LIST_WORD_OFFSET (5)
#define ACCMD_PRESERVE_REQ_OP_LIST_GET(addr) (*((addr) + 5))
#define ACCMD_PRESERVE_REQ_OP_LIST_SET(addr, op_list) (*((addr) + 5) = (uint16)(op_list))
#define ACCMD_PRESERVE_REQ_EP_COUNT_WORD_OFFSET (6)
#define ACCMD_PRESERVE_REQ_EP_COUNT_GET(addr) (*((addr) + 6))
#define ACCMD_PRESERVE_REQ_EP_COUNT_SET(addr, ep_count) (*((addr) + 6) = (uint16)(ep_count))
#define ACCMD_PRESERVE_REQ_EP_LIST_WORD_OFFSET (7)
#define ACCMD_PRESERVE_REQ_EP_LIST_GET(addr) (*((addr) + 7))
#define ACCMD_PRESERVE_REQ_EP_LIST_SET(addr, ep_list) (*((addr) + 7) = (uint16)(ep_list))
#define ACCMD_PRESERVE_REQ_WORD_SIZE (8)
#define ACCMD_PRESERVE_REQ_PACK(addr, preserve, op_count, op_list, ep_count, ep_list) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(preserve)); \
        *((addr) + 4) = (uint16)((uint16)(op_count)); \
        *((addr) + 5) = (uint16)((uint16)(op_list)); \
        *((addr) + 6) = (uint16)((uint16)(ep_count)); \
        *((addr) + 7) = (uint16)((uint16)(ep_list)); \
    } while (0)

#define ACCMD_PRESERVE_REQ_MARSHALL(addr, accmd_preserve_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_preserve_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_preserve_req_ptr)->preserve); \
        *((addr) + 4) = (uint16)((accmd_preserve_req_ptr)->op_count); \
        *((addr) + 5) = (uint16)((accmd_preserve_req_ptr)->op_list); \
        *((addr) + 6) = (uint16)((accmd_preserve_req_ptr)->ep_count); \
        *((addr) + 7) = (uint16)((accmd_preserve_req_ptr)->ep_list); \
    } while (0)

#define ACCMD_PRESERVE_REQ_UNMARSHALL(addr, accmd_preserve_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_preserve_req_ptr)->header)); \
        (accmd_preserve_req_ptr)->preserve = ACCMD_PRESERVE_REQ_PRESERVE_GET(addr); \
        (accmd_preserve_req_ptr)->op_count = ACCMD_PRESERVE_REQ_OP_COUNT_GET(addr); \
        (accmd_preserve_req_ptr)->op_list = ACCMD_PRESERVE_REQ_OP_LIST_GET(addr); \
        (accmd_preserve_req_ptr)->ep_count = ACCMD_PRESERVE_REQ_EP_COUNT_GET(addr); \
        (accmd_preserve_req_ptr)->ep_list = ACCMD_PRESERVE_REQ_EP_LIST_GET(addr); \
    } while (0)
...
#define ACCMD_CLOCK_CONFIGURE_RESP_CUSTOM_MODE_CLK_WORD_OFFSET (5)
#define ACCMD_CLOCK_CONFIGURE_RESP_CUSTOM_MODE_CLK_GET(addr) (*((addr) + 5))
#define ACCMD_CLOCK_CONFIGURE_RESP_CUSTOM_MODE_CLK_SET(addr, custom_mode_clk) (*((addr) + 5) = (uint16)(custom_mode_clk))
...
#define ACCMD_CLOCK_CONFIGURE_RESP_PACK(addr, status, low_power_mode_clk, custom_mode_clk, active_mode_clk) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(status)); \
        *((addr) + 4) = (uint16)((uint16)(low_power_mode_clk)); \
        *((addr) + 5) = (uint16)((uint16)(custom_mode_clk)); \
...
#define ACCMD_CLOCK_CONFIGURE_RESP_MARSHALL(addr, accmd_clock_configure_resp_ptr) \
    do { \
        ...
        *((addr) + 5) = (uint16)((accmd_clock_configure_resp_ptr)->custom_mode_clk); \
...
#define ACCMD_CLOCK_CONFIGURE_RESP_UNMARSHALL(addr, accmd_clock_configure_resp_ptr) \
    do { \
        ...
        (accmd_clock_configure_resp_ptr)->custom_mode_clk = ACCMD_CLOCK_CONFIGURE_RESP_CUSTOM_MODE_CLK_GET(addr); \
...
/*******************************************************************************
  ...
  NAME
    Accmd_File_Details_Resp

  DESCRIPTION
    Response message containing the file details - address and size.

  MEMBERS
    Status       - The status of the request
    file_address - Address of start of file in Apps subsystem Tbus address
                   space.
    file_size    - File size in octets.
...
/* The following macros take ACCMD_FILE_DETAILS_RESP *accmd_file_details_resp_ptr or uint16 *addr */
#define ACCMD_FILE_DETAILS_RESP_STATUS_WORD_OFFSET (3)
#define ACCMD_FILE_DETAILS_RESP_STATUS_GET(addr) ((ACCMD_STATUS)*((addr) + 3))
#define ACCMD_FILE_DETAILS_RESP_STATUS_SET(addr, status) (*((addr) + 3) = (uint16)(status))
#define ACCMD_FILE_DETAILS_RESP_FILE_ADDRESS_WORD_OFFSET (4)
#define ACCMD_FILE_DETAILS_RESP_FILE_ADDRESS_GET(addr)  \
    (((uint32)(*((addr) + 4)) | \
      ((uint32)(*((addr) + 5)) << 16)))
#define ACCMD_FILE_DETAILS_RESP_FILE_ADDRESS_SET(addr, file_address) do { \
        *((addr) + 4) = (uint16)((file_address) & 0xffff); \
        *((addr) + 5) = (uint16)((file_address) >> 16); } while (0)
#define ACCMD_FILE_DETAILS_RESP_FILE_SIZE_WORD_OFFSET (6)
#define ACCMD_FILE_DETAILS_RESP_FILE_SIZE_GET(addr)  \
    (((uint32)(*((addr) + 6)) | \
      ((uint32)(*((addr) + 7)) << 16)))
#define ACCMD_FILE_DETAILS_RESP_FILE_SIZE_SET(addr, file_size) do { \
        *((addr) + 6) = (uint16)((file_size) & 0xffff); \
        *((addr) + 7) = (uint16)((file_size) >> 16); } while (0)
#define ACCMD_FILE_DETAILS_RESP_WORD_SIZE (8)
#define ACCMD_FILE_DETAILS_RESP_PACK(addr, Status, file_address, file_size) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(Status)); \
        *((addr) + 4) = (uint16)((uint16)((file_address) & 0xffff)); \
        *((addr) + 5) = (uint16)(((file_address) >> 16)); \
        *((addr) + 6) = (uint16)((uint16)((file_size) & 0xffff)); \
        *((addr) + 7) = (uint16)(((file_size) >> 16)); \
    } while (0)

#define ACCMD_FILE_DETAILS_RESP_MARSHALL(addr, accmd_file_details_resp_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_file_details_resp_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_file_details_resp_ptr)->Status); \
        *((addr) + 4) = (uint16)(((accmd_file_details_resp_ptr)->file_address) & 0xffff); \
        *((addr) + 5) = (uint16)(((accmd_file_details_resp_ptr)->file_address) >> 16); \
        *((addr) + 6) = (uint16)(((accmd_file_details_resp_ptr)->file_size) & 0xffff); \
        *((addr) + 7) = (uint16)(((accmd_file_details_resp_ptr)->file_size) >> 16); \
    } while (0)

#define ACCMD_FILE_DETAILS_RESP_UNMARSHALL(addr, accmd_file_details_resp_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_file_details_resp_ptr)->header)); \
        (accmd_file_details_resp_ptr)->Status = ACCMD_FILE_DETAILS_RESP_STATUS_GET(addr); \
        (accmd_file_details_resp_ptr)->file_address = ACCMD_FILE_DETAILS_RESP_FILE_ADDRESS_GET(addr); \
        (accmd_file_details_resp_ptr)->file_size = ACCMD_FILE_DETAILS_RESP_FILE_SIZE_GET(addr); \
    } while (0)
