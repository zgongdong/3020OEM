...
.MODULE $M.cbops.reset;
...
#ifdef CBOPS_DEBUG
   .CONST $CBOPS_RESET_COUNT             1*ADDR_PER_WORD;
   .CONST $CBOPS_RESET_LOCAL_SIZE        1*ADDR_PER_WORD;
#endif
   ...
   r8 = M[r8 + $cbops_c.cbops_graph_struct.FIRST_FIELD];
      ...
      push r1;  // M[SP - $CBOPS_RESET_COUNT]
   #endif
   ...
   reset_loop:
         ...
         r1 = M[SP - $CBOPS_RESET_COUNT];
         r1 = r1 - 1;
         M[SP - $CBOPS_RESET_COUNT] = r1;
...
.CONST $CBOPS_COUNT             3*ADDR_PER_WORD;
.CONST $CBOPS_LOCAL_SIZE	3*ADDR_PER_WORD;
...
$_cbops_process_data:
   ...
   // start profiling if enabled
    ...
    push r0;   // operator count      : M[SP-$CBOPS_COUNT]
    ...
    if Z jump jp_buf_reset_done;
      ...
      do lp_get_buffers;
       ...
       if NZ jump jp_no_processing_done;
   ...
   if Z jump jp_post_amount_override_done;
      ...
      if Z jump jp_no_processing_done;
...
jp_continue_processing:
   ...
   do lp_update_buffers;
      // No update if buffer isn't used
      NULL = M[r8 + $cbops_c.cbops_buffer_struct.BUFFER_FIELD];
...
jp_no_processing_done:
    /* No cbops processing, set the transfer amount to 0
     * Note: 1- Only required for SINK buffer as it's already 0
     *          for SORCE buffers, and not needed for internal buffers.
     *          However we do it unconditionally here since its more efficient.
     *
     *       2- User cannot (yet) rely on transfer amount for active graphs,
     *          i.e. graphs that include at least one operator (including override
     *          operator) that alters buffer pointers internally.
     */
    r0  = M[SP - $CBOPS_LOCAL_GRAPH_PTR];
    r10 = M[r0 + $cbops_c.cbops_graph_struct.NUM_IO_FIELD];
    I2 = r0 + ($cbops_c.cbops_buffer_struct.TRANSFER_AMOUNT_FIELD + $cbops_c.cbops_graph_struct.BUFFERS_FIELD);
    M3 = $CBOP_BUFTAB_ENTRY_SIZE_IN_ADDR;
    r1 = 0;
    do clear_transfer_amount_loop;
         M[I2, M3] = r1;
    clear_transfer_amount_loop:
