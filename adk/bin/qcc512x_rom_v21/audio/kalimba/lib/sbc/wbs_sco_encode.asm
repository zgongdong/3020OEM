...
$wbsenc.wbs_pre_proc_func:
   ...
   r0 = 16;
#ifdef DATAFORMAT_32
   r8 = M[r9 + $sbc.mem.WBS_BITS_TO_COPY_FIELD];
   r8 = r8 + r0;     // add the no of bits put in the buffer
   M[r9 + $sbc.mem.WBS_BITS_TO_COPY_FIELD] = r8;
#endif // DATAFORMAT_32
   call $sbcenc.putbits;
...
.MODULE $M.wbsencode.init_encoder;
   ...
   push rLink;
   pushm <r0, r3>;
   // set usable octets for the encoder temporary output buffer
   r0 = r9 + $sbc.mem.WBS_ENC_TMP_OP_CB_STRUCT_FIELD;
   r1 = 4;
   call $cbuffer.set_usable_octets_ex;
   r1 = r9 + $sbc.mem.WBS_ENC_TMP_OP_BUFF_FIELD;
...
// Assume trashed everything else
// outputs: none
.MODULE $M.wbsenc.process_frame;
...
#ifdef DATAFORMAT_32
   // configure the temporary packed output buffer
   // the cbuffer structure is only needed for the copy
   // function $cbuffer.copy_packed_to_unpacked_ex;
   r0 = r9 + $sbc.mem.WBS_ENC_TMP_OP_CB_STRUCT_FIELD;
   M[r9 + $sbc.mem.WBS_BITS_TO_COPY_FIELD] = 0;
#endif // DATAFORMAT_32
   ...
   I4 = r9 + $sbc.mem.WBS_ENC_TMP_OP_CB_STRUCT_FIELD;
   r0 = I4;
#else
   r0 = M[r7 + $wbs_struct.WBS_ENC_OP_DATA_struct.BUFFERS_FIELD + $wbs_struct.SCO_TERMINAL_BUFFERS_struct.OP_BUFFER_FIELD];
#endif // DATAFORMAT_32
   ...
   // get the usable octets value for the real output buffer
   // (currently, this is set to 4 in SCO, even though there are only 2 usable octets in the word)
   // not hard coding the value in setting it back just in case it changes to reflect the reality
   BUFFER_GET_USABLE_OCTETS(r3, r0);
   push r3;
   // set the usable octets value to 2 for the real output buffer
   r1 = 2;
   call $cbuffer.set_usable_octets_ex;

   r1 = I4;
   r8 = M[r9 + $sbc.mem.WBS_BITS_TO_COPY_FIELD];
   r2 = r8 LSHIFT -3;    // r8 is in bits, convert to bytes

    // unpack frame
    r3 = M[r1 + $cbuffer.READ_ADDR_FIELD];
    r3 = r3 AND 1;      // get src offset (keep only lsb)
    r4 = M[r0 + $cbuffer.WRITE_ADDR_FIELD];
    r4 = r4 AND BUFFER_EX_OFFSET_MASK;      // get dst offset
    Null = r3 - r4;
    if Z jump in_phase;
        call $cbuffer.unpack_unaligned;
        jump cont_set_uo;
    in_phase:
        call $cbuffer.unpack_aligned;

   cont_set_uo:
   pop r3;
   r1 = r3;
   // set the usable octets for the real output buffer back to what it was
   call $cbuffer.set_usable_octets_ex;
