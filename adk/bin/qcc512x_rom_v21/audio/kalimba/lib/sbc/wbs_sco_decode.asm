...
#ifdef DATAFORMAT_32
.CONST $wbs.decoder.WBS_SYNC_EVEN_HIWORD_MASK_PACKED  ($M.wbs.decoder.WBS_SYNC_EVEN_HIWORD_MASK << 16);
.CONST $wbs.decoder.WBS_SYNC_EVEN_HIWORD_PACKED       ($M.wbs.decoder.WBS_SYNC_EVEN_HIWORD << 16);
#endif // DATAFORMAT_32
...
$sco_decoder.wbs.init_param:
   ...
   r1 = $wbs.decoder.WBS_ENCODED_FRAME_SIZE_B;
   M[r9 + $sbc.mem.CUR_FRAME_LENGTH_FIELD] = r1;
...
$sco_decoder.wbs.initialize:
   ...
   push rLink;
   pushm <r0, r1, r3>;
   // set usable octets for the decoder frame cbuffer
   r0 = r9 + $sbc.mem.WBS_FRAME_CB_STRUCT_FIELD;
   r1 = 4;
   call $cbuffer.set_usable_octets_ex;
   r1 = $wbs.decoder.WBS_ENCODED_FRAME_SIZE_B;
   M[r0 + $cbuffer.SIZE_FIELD] = r1;
   r1 = M[r9 + $sbc.mem.WBS_FRAME_BUFFER_PTR_FIELD];
   M[r0 + $cbuffer.WRITE_ADDR_FIELD] = r1;
   // we are only interested in the write address of the frame cbuff, therefore the fields
   // below will stay fixed to base address
   M[r0 + $cbuffer.READ_ADDR_FIELD] = r1;
   M[r0 + $cbuffer.START_ADDR_FIELD] = r1;
   M[r0 + $cbuffer.AUX_ADDR_FIELD] = r1;

   // set usable octets for the decoder cloned input cbuffer
   r0 = r9 + $sbc.mem.WBS_CLONED_IP_CB_STRUCT_FIELD;
   r1 = 2;
   call $cbuffer.set_usable_octets_ex;
   // it would be nice to be completely initialised here, but at init stage
   // the info about the payload buffer is not known;
   // it will be initialised in wbs.process, just make sure we mark
   // the cbuffer struct as "uninitialised"
   M[r0 + $cbuffer.SIZE_FIELD] = Null;
...
havesync:
   // find out whether two frames can be produced from the payload plus whatever is in frame buffer
   r2 = $wbs.decoder.WBS_DECODED_FRAME_SIZE_W * 2; // two frames in output
   r4 = M[r9 + $sbc.mem.CUR_FRAME_LENGTH_FIELD];
   r4 = r4 LSHIFT 1;                                 // two packets in input
   r0 = M[r9 + $sbc.mem.WBS_BYTES_IN_FRAME_BUFFER_FIELD];

   r0 = r0 + r5; // available data in input
   r1 = $M.wbs.decoder.VALIDATE_NO_OUTPUT;      // default to no output signal
   Null = r0 - r4; // do we have enough encoded input data to produce two frames?
   if POS r1 = r2;

   // make sure there's space to produce two frames
   Null = r1 - $M.wbs.decoder.VALIDATE_NO_OUTPUT;
   if Z jump oneframe;
   Null = r6 - r1; // do we have space for two frames?
   if POS jump $pop_rLink_and_rts;

oneframe:
   // We can't produce two frames maybe we can produce one
   r2 = $wbs.decoder.WBS_DECODED_FRAME_SIZE_W;  // one frame in output
   r4 = M[r9 + $sbc.mem.CUR_FRAME_LENGTH_FIELD];  // one packet in input
   Null = r0 - r4; // do we have enough encoded input data to produce one frame?
   if POS r1 = r2;

   // make sure there's space to produce the desired output
   Null = r1 - $M.wbs.decoder.VALIDATE_NO_OUTPUT;
   if Z jump $pop_rLink_and_rts;
   Null = r6 - r1; // do we have space for one frame?
   if NEG r1 = Null;
...
buffer_decode_loop:
   M1 = ADDR_PER_WORD;
   r2 = M[r9 + $sbc.mem.CUR_FRAME_LENGTH_FIELD];
   ...
   push r6;
#ifdef DATAFORMAT_32
   // configure input cloned cbuffer structure
   // this cbuffer structure exists only for calling $cbuffer.copy_unpacked_to_packed_ex.
   // initialise only once
   r1 = r9 + $sbc.mem.WBS_CLONED_IP_CB_STRUCT_FIELD;
   Null = M[r1 + $cbuffer.SIZE_FIELD];
   if Z jump init_cloned_ip_cb_struct;

   continue_to_copy_frame:
   // TODO the rdp should have been updated by some prev process
   // r4 = WBS_PAYLOAD_ALIGN_FIELD, if not zero (=1) means that
   // it should copy only one octet from first word.
   r3 = I0 + r4;
   M[r1 + $cbuffer.READ_ADDR_FIELD] = r3;

   r3 = r9 + $sbc.mem.WBS_FRAME_CB_STRUCT_FIELD;
   ...
   //    I4 contains frame buffer (ifndef DATAFORMAT_32)
   //    I0/L0 contains payload buffer
   // #ifdef DATAFORMAT_32
   //    r3 the cbuffer structure for the frame buffer
   //    r1 cloned cbuffer structure for the payload buffer
   // #endif
...
#ifdef DATAFORMAT_32
         r3 = r3 AND $wbs.decoder.WBS_SYNC_EVEN_HIWORD_MASK_PACKED;
         Null = r3 - $wbs.decoder.WBS_SYNC_EVEN_HIWORD_PACKED;
#else
   ...
   r10 = M[r9 + $sbc.mem.CUR_FRAME_LENGTH_FIELD];
   Null = r0 - r10;
...
#ifdef DATAFORMAT_32
// configure cloned input cbuffer (first time in wbs.process)
init_cloned_ip_cb_struct:
   push B0;
   pop r6;
   pushm <r0, r1, r2>;
   r0 = r1;
   M[r0 + $cbuffer.SIZE_FIELD] = L0;
   M[r0 + $cbuffer.WRITE_ADDR_FIELD] = I0;
   M[r0 + $cbuffer.START_ADDR_FIELD] = r6;
   M[r0 + $cbuffer.AUX_ADDR_FIELD] = r6;
   r1 = I0;
   r2 = r4;
   call $cbuffer.set_read_address_ex;
   popm <r0, r1, r2>;

   jump continue_to_copy_frame;
#endif      // DATAFORMAT_32
...
$sco_decoder.wbs.decode:
   ...
   r0 = M[I0, MK1];
#endif // DATAFORMAT_32
   r0 = 16;                // the word would be 0xAD00
   ...
   Null = r0 - $sbc.DATA_WIDTH_PER_WORD;
...
//    r1    : cloned cbuffer structure for payload buffer (ifdef DATAFORMAT_32)
//    r2    : size of frame_buffer in bytes
//    r3    : frame cbuffer (ifdef DATAFORMAT_32, I4 ignored)
//    I0/L0 : circular payload buffer (source)
//    r4    : alignment of payload buffer (0=word, 1=byte)
//    I4    : frame buffer (dest) (if DATAFORMAT_32 is NOT defined)
...
//
// TRASHED REGISTERS:
//    r1,r3,r6,r10,I4, doloop (not considering output regs)
//
// CPU USAGE:
//    D-MEMORY: xxx
//    P-MEMORY: xxx
...
$sco_decoder.wbs.copy_frame:
   ...
   // r2 = #bytes to copy
    ...
    push rLink;
    push r0;
    r0 = r3;    // frame cbuff i.e. destination cbuff

    // pack frame
    r3 = M[r1 + $cbuffer.READ_ADDR_FIELD];
    r3 = r3 AND BUFFER_EX_OFFSET_MASK;      // get src offset
    r4 = M[r0 + $cbuffer.WRITE_ADDR_FIELD];
    r4 = r4 AND 1;                          // get dst offset, keep only lsb
    Null = r3 - r4;
    if Z jump in_phase;
        call $cbuffer.pack_unaligned;
        jump cont_ret_regs;
    in_phase:
        call $cbuffer.pack_aligned;

    cont_ret_regs:
    r3 = M[r1 + $cbuffer.READ_ADDR_FIELD];
    // the offset can only be 0 or 1 (payload buff is 16-bit unpacked)
    r4 = r3 AND ~BUFFER_EX_OFFSET_MASK;     // get word aligned read address to set the index reg
    I0 = r4;
    r4 = r3 AND BUFFER_EX_OFFSET_MASK;      // get payload buff offset
    pop r0;
