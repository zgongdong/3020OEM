...
static void audio_data_service_a2dp_kick(void* priv, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    if (ep->deferred.kick_is_deferred)
    {
        ...
        ep->deferred.interrupt_handled_time = time_get_time();
        raise_bg_int(ep->deferred.bg_task);
    }
    else
    {
        a2dp_kick(ep, kick_dir);
    }
}
...
static void a2dp_self_kick(void *priv)
{
    ...
    audio_data_service_a2dp_kick(priv,  STREAM_KICK_INTERNAL);
}
 ...
 * endpoint is source.
 */
static metadata_tag *metadata_transport_and_set_toa(tCbuffer *src, tCbuffer *dst, unsigned trans_octets, ENDPOINT_DIRECTION dir, TIME toa)
{
    ...
    if (dst != NULL)
    {
        while (list_tag != NULL)
        {
            if (dir == SOURCE)
            {
                ...
                METADATA_TIME_OF_ARRIVAL_SET(list_tag, toa);
            }
...
static inline void a2dp_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
            ...
            if (buff_metadata_tag_threshold_exceeded())
            {
                ...
                if (a2dp->self_kick_timer_id == TIMER_ID_INVALID)
                {
                    a2dp->self_kick_timer_id = timer_schedule_event_in(
                                                 ...
                                                 a2dp_self_kick,
                                                 (void *)(ep));
                    L3_DBG_MSG("a2dp: setting self kick timer (tag threshold exceeded)");
                }
                return;
            }
...
#endif
        copied = cbuffer_copy_ex(a2dp->sink_buf, a2dp->source_buf, max_copy);
            ...
            if (octets)
            {
                ...
                if (a2dp->self_kick_timer_id == TIMER_ID_INVALID)
                {
                    a2dp->self_kick_timer_id = timer_schedule_event_in(
                                                 ...
                                                 a2dp_self_kick,
                                                 (void *)(ep));
                    L3_DBG_MSG1("a2dp: setting self kick timer (pending octets=%d)", octets);
                }
            }
            else
            {
    ...
    if (copied != 0)
    {
        ...
        if (ep->deferred.kick_is_deferred)
        {
            toa = ep->deferred.interrupt_handled_time;
        }
        else
        {
            toa = time_get_time();
        ...
        metadata_transport_and_set_toa(a2dp->source_buf,
                a2dp->sink_buf, copied, ep->direction, toa);
    }
#endif /* INSTALL_METADATA */
...
static bool a2dp_start(ENDPOINT *ep, KICK_OBJECT *ko)
{
    ...
    if (a2dp_start_common(ep))
    {
        ...
        audio_data_service_a2dp_kick(ep, STREAM_KICK_INTERNAL);
        a2dp->running = TRUE;
