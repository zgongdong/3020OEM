...
/*
 * timed_playback_default_cback
 */
void timed_playback_default_cback(unsigned con_id, unsigned ep_id, unsigned data)
{
    ADAPTOR_UNACHIEVABLE_LATENCY_MSG msg;
    msg.ep_id = ep_id;
    /* TODO: This violates the protocol by putting more than 16 bits per word. */
    msg.data = data;
    (void) adaptor_send_message(con_id, AMSGID_UNACHIEVABLE_LATENCY,
                                ADAPTOR_UNACHIEVABLE_LATENCY_MSG_WORD_SIZE,
                                (ADAPTOR_DATA) &msg);
}
