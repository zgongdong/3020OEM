...
#define TOA_DEBUG_LIMIT (100 * MILLISECOND)
#endif
...
#ifdef TTP_PLOTTER
/**
 * Variable used to share the raw latency with the plotter tool.
 */

static unsigned ttp_raw_latency;
#endif /* TTP_PLOTTER */
...
void ttp_update_ttp_from_source_time(ttp_context *context, TIME toa, TIME source_time, ttp_status *status)
{
    ...
    TTP_PLOTTER_MACRO(ttp_raw_latency = time_sub(context->ttp, toa));
}
...
void ttp_update_ttp(ttp_context *context, TIME time, unsigned samples, ttp_status *status)
{
    ...
    period_adj = (uint32)(1000000ul*samples + frac_mul_long(1000000ul*samples, context->sp_adjustment));
    ...
    TTP_PLOTTER_MACRO(ttp_raw_latency = time_sub(context->ttp, time));
}
