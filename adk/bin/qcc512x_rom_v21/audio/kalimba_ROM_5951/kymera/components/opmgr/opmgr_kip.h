...
static inline bool opmgr_kip_get_processor_id(unsigned ep_id, IPC_PROCESSOR_ID_NUM *processor_id)
{
    OP_PROCESSOR_ID proc_id;

    if (!opmgr_get_processor_id(ep_id, &proc_id))
    {
        return FALSE;
    }

    *processor_id = (IPC_PROCESSOR_ID_NUM)proc_id;
    return TRUE;
}

static inline bool opmgr_kip_get_processor_id_from_opid(unsigned op_id, IPC_PROCESSOR_ID_NUM *processor_id)
{
    OP_PROCESSOR_ID proc_id;

    if (!opmgr_get_processor_id_from_opid(op_id, &proc_id))
    {
        return FALSE;
    }
...
bool opmgr_kip_build_send_list_cmd(unsigned con_id, unsigned num_ops,
                                   unsigned* op_list, KIP_MSG_ID kip_msg_id,
                                   void* callback);
