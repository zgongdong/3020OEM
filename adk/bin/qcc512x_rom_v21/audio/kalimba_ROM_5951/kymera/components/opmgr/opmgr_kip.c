...
bool opmgr_kip_build_send_list_cmd(unsigned con_id,
                                   unsigned num_ops, unsigned* op_list,
                                   KIP_MSG_ID kip_msg_id, void* callback)
{
    bool status = FALSE;
    unsigned length;
    uint16* msg_data;
 ...
 */
bool opmgr_kip_unsolicited_message(unsigned con_id, OP_UNSOLICITED_MSG *msg)
{
    bool status = FALSE;
    unsigned kip_len;
    ...
    KIP_MSG_MESSAGE_FROM_OPERATOR_REQ_CON_ID_SET(kip_msg, con_id);
    ...
    status = kip_adaptor_send_message(REVERSE_CONNECTION_ID(con_id),
