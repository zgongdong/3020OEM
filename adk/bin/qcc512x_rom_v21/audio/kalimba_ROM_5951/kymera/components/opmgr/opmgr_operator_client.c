...
static bool op_client_remove_delegated_op(unsigned con_id, unsigned ext_op_id);
 ...
 */
bool opmgr_op_client_delegate(unsigned con_id, unsigned ext_op_id,
                              unsigned num_owned_ops, unsigned * owned_op_array)
{
    OP_CLIENT *client;
    ...
    client = op_client_with_op_id(ext_op_id);
    if (client == NULL)
    {
    ...
    opmgr_set_creator_id(owned_op_array, num_owned_ops, client->con_id);
 ...
 */
unsigned opmgr_op_client_cancel_delegation(unsigned con_id,
                                           unsigned num_ops, unsigned * op_array)
{
    unsigned i, j;
    unsigned *tmp_array;
    ...
    tmp_array = xpnewn(num_ops, unsigned);
    if (tmp_array == NULL)
    {
                                       ...
                                       unsigned num_owned_ops, unsigned *owned_op_array)
{
    ...
    for (i = 0; i < num_owned_ops; i++)
    {
        ...
        op->ext_op_id = owned_op_array[i];
        op->next = NULL;
        (*plist) = op;
        plist = &op->next;

        op_data = get_anycore_op_data_from_id(EXT_TO_INT_OPID(op->ext_op_id));
        if (op_data == NULL)
        {
    ...
    for (i = 0; i < MAX_NUM_OP_CLIENT; i++)
    {
       client = op_client_db[i];
        if ((client != NULL) && client->valid)
        {
           ...
           while ((*pop) != NULL && (*pop)->ext_op_id != ext_op_id)
           {
               pop = &(*pop)->next;
           }
           if ((*pop) != NULL)
           {
               /* We found it. */
               op_data = get_anycore_op_data_from_id(EXT_TO_INT_OPID(
                                                       client->ext_op_id));
               if (con_id != op_data->con_id)
               {
