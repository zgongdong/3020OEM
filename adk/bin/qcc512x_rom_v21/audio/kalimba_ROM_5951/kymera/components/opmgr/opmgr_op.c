...
static opmsg_handler_function lookup_opmsg_handler(const opmsg_handler_lookup_table_entry* func_table, unsigned opmsg_id)
{
    ...
    /* This code assumes the message table is correctly formed, with a NULL handler 
     * as the final entry. If so, on exit from the loop entry->handler is either valid or NULL
     */
...
static bool handle_operator_message(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    if((op_data->cap_data != NULL) && (op_data->cap_data->opmsg_handler_table != NULL))
    {
        ...
        {
            bool result = (*op_msg_handler)(op_data, message_data, &resp_length, &resp_data);
 #ifdef INSTALL_OPERATOR_CREATE_PENDING
            /* The Operator Create command is allowed to be asynchronous and can
             * return HANDLER_INCOMPLETE. */
            if ((PENDABLE_OP_HANDLER_RETURN)result == HANDLER_INCOMPLETE)
            {
                L4_DBG_MSG1("Operator message pending for  %x",(void*)op_data);
            ...
            &op_cmd_id, &msg_body, &rinfo))
    {
        ...
        if (handler)
        {
            ...
            if (result)
            {
                ...
                if ( ( op_cmd_id == OPCMD_CREATE || op_cmd_id == OPCMD_MESSAGE)
                    && (PENDABLE_OP_HANDLER_RETURN)result == HANDLER_INCOMPLETE)
                {
                    pending_operator_cb pending_handler = (pending_operator_cb)resp_data;
                    L4_DBG_MSG2("Pending operator %x command 0x%x (0 - OPCMD_CREATE, 0xFFFD - OPCMD_MESSAGE)",
                                (void*)current_op, op_cmd_id);

                    pending_handler(current_op, op_cmd_id, msg_body,
                                    (pendingContext*) &rinfo, resp_msg_id);
 ...
 * opmgr_kick_operator
 *
 * Helper function for kicking the operator in the background from an
 * abnormal source. e.g. some special interrupt the operator setup
 */
RUN_FROM_PM_RAM
void opmgr_kick_operator(OPERATOR_DATA *op_data)
{
...
void opmgr_operator_bgint_handler(void **bg_data)
{
    ...
    /* If processing is suspended, don't proceed, but leave
     * a flag to re-kick when resumed
     */
    if (current_op->processing_suspended)
    {
        current_op->processing_rerun_requested = TRUE;
        return;
    }
#endif /* OPMGR_SYNC_KICKED_IMPLEMENTATION == OPMGR_SYNC_KICKED_BASIC */
 ...
 */
ENDPOINT** opmgr_get_terminal_list(unsigned opidep)
{
    ...
    if (op_data != NULL)
    {
        ...
        {
            return &op_data->sink_terminals;
        }
        else
        {
            return &op_data->source_terminals;
        }
    }
    else
    {
...
bool opmgr_op_thread_offload(OPERATOR_DATA *op_data)
{
    return op_data->thread_offload_enabled;
}
