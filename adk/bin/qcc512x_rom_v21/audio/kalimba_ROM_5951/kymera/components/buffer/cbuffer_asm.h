...
#define BUFFER_GET_USABLE_OCTETS(dest_reg, cb_reg) dest_reg = M[cb_reg + $cbuffer.DESCRIPTOR_FIELD]; \
                                                   dest_reg = dest_reg AND $cbuffer.USABLE_OCTETS_MASK; \
                                                   dest_reg = dest_reg LSHIFT -$cbuffer.USABLE_OCTETS_SHIFT
/* BUFFER_SET_USABLE_OCTETS is implemented as a function $cbuffer.set_usable_octets_ex in cbuffer_ex_asm.c */
#else /* KAL_ARCH4 */
#define BUFFER_GET_WRITE_ADDR(dest_reg, cb_reg) dest_reg = M[cb_reg + $cbuffer.WRITE_ADDR_FIELD]
#define BUFFER_GET_READ_ADDR(dest_reg, cb_reg)  dest_reg = M[cb_reg + $cbuffer.READ_ADDR_FIELD]
