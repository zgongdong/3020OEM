...
#if defined(__KCC__) && defined(INSTALL_MIB) && !defined(UNIT_TEST_BUILD)
#include "mib/mib.h"
#endif
...
#if (defined(INSTALL_DUAL_CORE_SUPPORT) || defined(AUDIO_SECOND_CORE)) && !defined(UNIT_TEST_BUILD)  && defined(__KCC__)
...
#else
#define HEAP_SIZE_SHARED        (HEAP_SIZE_SHARED_MAX / 2)
#endif
...
#if !defined(HEAP_OVERFLOW_TO_FREE_RAM) && !defined(CHIP_BASE_BC7)
#undef HEAP1_MAX_SIZE_SINGLE_MODE
#undef HEAP2_MAX_SIZE_SINGLE_MODE
#undef HEAP3_MAX_SIZE_SINGLE_MODE
#undef HEAP4_MAX_SIZE_SINGLE_MODE
#undef HEAP1_MAX_SIZE
#undef HEAP2_MAX_SIZE
#undef HEAP3_MAX_SIZE
#undef HEAP4_MAX_SIZE

#undef HEAP2_MAX_SIZE_THREAD_OFFLOAD

/* and initialise max sizes with the min sizes from config */
#define HEAP1_MAX_SIZE_SINGLE_MODE          HEAP_SIZE_DM1
#define HEAP2_MAX_SIZE_SINGLE_MODE          HEAP_SIZE_DM2
#define HEAP3_MAX_SIZE_SINGLE_MODE          HEAP_SIZE_SHARED
#define HEAP4_MAX_SIZE_SINGLE_MODE          HEAP_SIZE_DM1_EXT
#define HEAP2_MAX_SIZE_THREAD_OFFLOAD       HEAP_SIZE_DM2
/* must be at least one bank size otherwise aligning to a block boundary won't work */
#define HEAP1_MAX_SIZE          (2*HEAP_SIZE_DM1 + DM_RAM_BANK_SIZE)
#define HEAP2_MAX_SIZE          (2*HEAP_SIZE_DM2 + DM_RAM_BANK_SIZE)
#define HEAP3_MAX_SIZE          (2*HEAP_SIZE_SHARED + DM_RAM_BANK_SIZE)
#define HEAP4_MAX_SIZE          (2*HEAP_SIZE_DM1_EXT + DM_RAM_BANK_SIZE)
...
#define ext_heap_is_disabled()  ((freelist[HEAP_SRAM] == NULL))

#if !defined(UNIT_TEST_BUILD)

#define SRAM_START_ADDR (DM_NVMEM_WINDOW_START + (DM_NVMEM_WINDOW_SIZE * EXT_RAM_WRITE_WINDOW)) 

#else  /* !UNIT_TEST_BUILD */
 
DM2_SHARED_HEAP_LOCATION char   ext_heap[EXT_HEAP_SIZE];
#define       SRAM_START_ADDR   ext_heap
...
*/
#if defined(HAVE_ADDITIONAL_HEAP) && \
       defined(__KCC__)  && !defined(UNIT_TEST_BUILD)
/* Add unused memory sections to DM1 heap
 * This is only available for Crescendo.
 * Remove this after the memory is re-arranged.
 */
extern unsigned                 _heap1_p0_DM1_addition_start;
extern unsigned                 _heap1_p0_DM1_addition_end;
extern unsigned                 _heap1_p1_DM1_addition_start;
extern unsigned                 _heap1_p1_DM1_addition_end;

/* Save the additional heap data. This is used by ACAT too.*/
DM_SHARED_ZI char*                heap1_p0_DM1_addition_start;
DM_SHARED_ZI char*                heap1_p0_DM1_addition_end;
DM_SHARED_ZI unsigned             heap1_p0_DM1_addition_size;
DM_SHARED_ZI char*                heap1_p1_DM1_addition_start;
DM_SHARED_ZI char*                heap1_p1_DM1_addition_end;
DM_SHARED_ZI unsigned             heap1_p1_DM1_addition_size;


/**
 * This function creates a new heap node with the unused memory
 * and adds it to  HEAP_DM1.
 */
static void additional_node(void)
{
    mem_node *freelist_node;
    mem_node *new_node;
    unsigned length;

    patch_fn_shared(heap);
    freelist_node = freelist[HEAP_DM1];

#if (defined(INSTALL_DUAL_CORE_SUPPORT) || defined(AUDIO_SECOND_CORE))
    if (hal_get_reg_processor_id() != 0)
    {
        /* Secondary processor. */
        new_node= (mem_node *)heap1_p1_DM1_addition_start;
        length = heap1_p1_DM1_addition_size;
    }
    else
#endif
    {
        /* Populate the additional heap data. */
        heap1_p0_DM1_addition_start = GET_LINKER_ADDRESS(_heap1_p0_DM1_addition_start);
        heap1_p0_DM1_addition_end = GET_LINKER_ADDRESS(_heap1_p0_DM1_addition_end);
        heap1_p0_DM1_addition_size = (uintptr_t)heap1_p0_DM1_addition_end - (uintptr_t)heap1_p0_DM1_addition_start;
#if (defined(INSTALL_DUAL_CORE_SUPPORT) || defined(AUDIO_SECOND_CORE))
        heap1_p1_DM1_addition_start = GET_LINKER_ADDRESS(_heap1_p1_DM1_addition_start);
        heap1_p1_DM1_addition_end = GET_LINKER_ADDRESS(_heap1_p1_DM1_addition_end);
        heap1_p1_DM1_addition_size = (uintptr_t)heap1_p1_DM1_addition_end - (uintptr_t)heap1_p1_DM1_addition_start;
#endif
        /* processor 0 init */
        new_node= (mem_node *)heap1_p0_DM1_addition_start;
        length = heap1_p0_DM1_addition_size;
    }

    DBG_MEM_CFG_MSG2("Additional address:0%08x length:0%08x", new_node, length);
    new_node->u.next = NULL;
    new_node->length = length - sizeof(mem_node);
    freelist_node->u.next = new_node;

#ifdef HEAP_DEBUG
    length = new_node->length - GUARD_SIZE;
    /* Additional heap node added.
     * Increment the minimum and currently available free memory.*/
    heap_debug_free += length;
    heap_debug_min_free += length;
    heap_debug_freenodes++;
#endif

    pheap_info->heap[HEAP_DM1].heap_free += new_node->length - GUARD_SIZE;
}

/*
 * This function checks if the address provided is part of the additional
 * memory added by additional_node and returns the heap number where the
 * address belongs.
 */
static unsigned get_additional_heap_num(void *ptr)
{
    patch_fn_shared(heap);
    if (hal_get_reg_processor_id() != 0)
    {
        /* Secondary processor. */
        if ((heap1_p1_DM1_addition_start <= (char*)ptr) && ((char*)ptr < heap1_p1_DM1_addition_end))
        {
            return HEAP_DM1;
        }
    }
    ...
    {
        /* processor 0 init */
        if ((heap1_p0_DM1_addition_start<= (char*)ptr) && ((char*)ptr < heap1_p0_DM1_addition_end))
        {
            return HEAP_DM1;
        }
    }
...
#else
/* Dummy functions. */
#define additional_node()               ((void)0)
#define get_additional_heap_num(addr)   HEAP_INVALID
#endif
/**
 * NAME
 *   get_heap_num
...
#ifdef HAVE_ADDITIONAL_HEAP
    heap_num = get_additional_heap_num(ptr);
...
#ifdef __KCC__
#if defined(CHIP_STRE) && defined(INSTALL_DUAL_CORE_SUPPORT) && !defined(UNIT_TEST_BUILD)
/* 
 * If P1 is not enabled, claim the P1 audio log buffer size too
 * and add to P0 shared heap 
 */
#if !defined(AUDIO_LOG_BUF_SIZE)
#error "AUDIO_LOG_BUF_SIZE not defined, not safe to expand shared heap (heap3)"
#endif

#define AUDIO_P1_LOG_BUFFER_SIZE          (AUDIO_LOG_BUF_SIZE*4)
/* Exception stack size defines locally in 'error.asm' */
#define AUDIO_P1_EXCEPTION_STACK_SIZE     (0x80)
/* P1 Memory that we can re-use */
#define AUDIO_LOG_BUFFER_SIZE             (AUDIO_P1_LOG_BUFFER_SIZE + AUDIO_P1_EXCEPTION_STACK_SIZE)

static void expand_heap3_if_p1_not_used(void)
{
    patch_fn_shared(heap);
    /*
     * P1 is not used if CoreEnable=[00] *and*
     * AudioThreadOffload=false.
     */
    if (   (hal_get_active_num_cores() <= 1)
#ifdef INSTALL_THREAD_OFFLOAD 
        && (!audio_thread_offload_is_configured())
#endif
       )
    {
        mem_node *new_node;
        ...
        /* No second processor. Extend the heap to audio log buffer */
        processor_heap_info_list[0].heap[HEAP_SHARED].heap_start -= AUDIO_LOG_BUFFER_SIZE;
        processor_heap_info_list[0].heap[HEAP_SHARED].heap_size  += AUDIO_LOG_BUFFER_SIZE;

        /* initialise the new node */
        new_node = (mem_node *)processor_heap_info_list[0].heap[HEAP_SHARED].heap_start;
        free_size = processor_heap_info_list[0].heap[HEAP_SHARED].heap_size - sizeof(mem_node);
        new_node->u.next = NULL;
        new_node->length = free_size;

        /* replace the new node in the freelist */
        freelist[HEAP_SHARED] = new_node;

        processor_heap_info_list[0].heap[HEAP_SHARED].heap_free = free_size;

#ifdef HEAP_DEBUG
        heap_debug_free += AUDIO_LOG_BUFFER_SIZE;
#endif
    }
}
#else
#define expand_heap3_if_p1_not_used()          ((void)0)
#endif /* CHIP_STRE && INSTALL_DUAL_CORE_SUPPORT && !UNIT_TEST_BUILD */
#endif /* __KCC__ */
/**
 * NAME
 *   is_dm2_heap_loaded
...
{
    uint32 ext_sram_size = 0, ext_p0_heap_size = 0,ext_p1_heap_size = 0;
    ...
    len_read = mibgetreqstr(HEAPALLOCATIONSRAM, (uint8*)&mib_value, sizeof(mib_value));
    if (len_read != sizeof(mib_value))
    {
        return FALSE;
    }

    /*size of external SRAM and heaps in KBYTE*/
    ...
    if (ext_p0_heap_size > 0)
    {
        ...
        set_heap_info(&pheap_info->heap[HEAP_SRAM],
                (char*) SRAM_START_ADDR,
                (char*) SRAM_START_ADDR + ext_p0_heap_size,
                ext_p0_heap_size, FALSE);
...
#if defined(INSTALL_DUAL_CORE_SUPPORT) && defined(__KCC__) && !defined(UNIT_TEST_BUILD)
    ...
    if (ext_p1_heap_size > 0)
    {
         ...
         * freelist
         */
        set_heap_info(&processor_heap_info_list[HEAP_PROC_1].heap[HEAP_SRAM],
                (char*) (pheap_info->heap[HEAP_SRAM].heap_end),
                (char*) (pheap_info->heap[HEAP_SRAM].heap_end) + ext_p1_heap_size,
                        ext_p1_heap_size, FALSE);
    }
#endif /*NSTALL_DUAL_CORE_SUPPORT*/

     PL_PRINT_P2(TR_PL_MALLOC,"UT: SRAM_START ADDR = %lx, size = %d ", (unsigned long)pheap_info->heap[HEAP_SRAM].heap_start, pheap_info->heap[HEAP_SRAM].heap_size);
    return TRUE;
}
#endif /* INSTALL_EXTERNAL_MEM */
 ...
 *
 * \brief Gets the value of MIB key that controls the heap overflow.
 */
static bool heap_overflow_to_free_ram(void )
{
    bool overflow_to_free_ram;

    overflow_to_free_ram = mibgetreqbool(HEAPOVERFLOWTOFREERAM);

    DBG_MEM_CFG_MSG1("HEAPOVERFLOWTOFREERAM = %d. ", overflow_to_free_ram);
    ...
    else
    {
        ...
        /* NB. We have an alternative static HEAP_OVERFLOW_TO_FREE_RAM.
            If that's not defined and this dynamic configuration is not valid,
            pay attention to min heap values in specific platforms config
         */
#if !defined(HEAP_OVERFLOW_TO_FREE_RAM) && !defined(CHIP_BASE_BC7)
        COMPILE_TIME_ASSERT( 2*HEAP_SIZE_DM1 >= DM_RAM_BANK_SIZE, Assumption_about_heap1_sizes_in_config_violated);
        COMPILE_TIME_ASSERT( 2*HEAP_SIZE_DM2 >= DM_RAM_BANK_SIZE, Assumption_about_heap2_sizes_in_config_violated);
#endif
    }
...
static void set_heap_max_sizes(bool second_core_enabled, heap_sizes* heap_max_sizes)
{
    ...
    DBG_MEM_CFG_MSG3("Heap config: Heap 1 size: internal 0x%05x, linker 0x%05x, thread offload 0x%05x",
            HEAP_SIZE_DM1_MAX, HEAP1_MAX_SIZE, HEAP1_MAX_SIZE_SINGLE_MODE);
#endif
#ifdef HEAP_SIZE_DM2_MAX
    DBG_MEM_CFG_MSG3("Heap config: Heap 2 size: internal 0x%05x, linker 0x%05x, thread offload 0x%05x",
            HEAP_SIZE_DM2_MAX, HEAP2_MAX_SIZE, HEAP2_MAX_SIZE_THREAD_OFFLOAD);
#endif
#ifdef HEAP_SIZE_SHARED_MAX
    DBG_MEM_CFG_MSG3("Heap config: Heap 3 size: internal 0x%05x, linker 0x%05x, thread offload 0x%05x",
            HEAP_SIZE_SHARED_MAX, HEAP3_MAX_SIZE, HEAP3_MAX_SIZE_SINGLE_MODE);
#endif
#ifdef HEAP_SIZE_DM1_EXT_MAX
    DBG_MEM_CFG_MSG3("Heap config: Heap 4 size: internal 0x%05x, linker 0x%05x, thread offload 0x%05x",
    ...
    if (audio_thread_offload_is_configured())
    {
        ...
        heap_max_sizes->heap_size[HEAP_DM2] =        HEAP2_MAX_SIZE_THREAD_OFFLOAD;
    ...
    mem_node **freelist)
{
    ...
    for (heap_num = 0; heap_num < HEAP_ARRAY_SIZE; heap_num++)
    {
        ...
        /* Increment the required heap size to the dynamic heap configuration
         * values if it is bigger than the minimum heap size. */
        if (have_dyn_config)
        {
        ...
        {
            /* this is a possible option when MIB keys are invalid,
            (dyn_heap_config is invalid) */
            p0_size = heap_min;
            if (second_core != NULL)
            {
                p1_size = heap_min;
        ...
        /* This will allow us to round down the P1 heap start address to a memory
         * bank boundary, provided P1 heap size is non-zero. Note this can possibly
         * increasing the heap size.*/
        p1_end = p0_start + current_heap_max_size;
        p1_start = p1_end - p1_size;
        if ((heap_num != HEAP_SHARED) && (p1_size))
        {
        ...
        if (p1_size >= current_heap_max_size)
        {
            WARN_MEM_CFG_MSG3("Heap %d configuration error while aligning\n"
                ...
                "exceeds maximum heap size : 0x%05x, this heap will be unusable",
        ...
        {
            unsigned len = p0_size_info->heap_size - heap_min;
            /* Update the P0 freelist */
            coalesce_free_mem( pfreelist,
                               p0_start + heap_min,
                               len);
#ifdef HEAP_DEBUG
            /* Increment the minimum available heap after a reconfiguration.
             * Note: heap_debug_free is alredy incremented in coalesce_free_mem. */
...
#if defined(INSTALL_EXTERNAL_MEM) 
#if defined(__KCC__) && !defined(RUNNING_ON_KALSIM) && (defined(INSTALL_DUAL_CORE_SUPPORT) || defined(AUDIO_SECOND_CORE))
...
void init_heap(heap_config *config)
{
    ...
    {
        /* We should have been passed a pointer to config prepared on and by P0 */
...
#else
    UNUSED(config);
#endif  /* (defined(INSTALL_DUAL_CORE_SUPPORT) || defined(AUDIO_SECOND_CORE)) && defined(__KCC__)  && !defined(UNIT_TEST_BUILD) */
    {
        /* processor 0 init */
        ...
        set_heap_info(&pheap_info->heap[HEAP_SHARED],   heap3, heap3 + HEAP_SIZE_SHARED,  HEAP_SIZE_SHARED, FALSE);
        set_heap_info(&pheap_info->heap[HEAP_DM1_EXT],  heap4, heap4 + HEAP_SIZE_DM1_EXT, HEAP_SIZE_DM1_EXT, FALSE);
    }
...
#if defined(HAVE_ADDITIONAL_HEAP) && !defined(UNIT_TEST_BUILD)
        /* Signal to ACAT that the second core's additional heap is not used.*/
        heap1_p1_DM1_addition_start = (char*)0;
        heap1_p1_DM1_addition_end = (char*)0;
        heap1_p1_DM1_addition_size = 0;
#endif
    }
...
#ifdef HAVE_ADDITIONAL_HEAP
    if (get_additional_heap_num(addr) != HEAP_INVALID)
    {
        return TRUE;
    }
#endif
...
#endif
{
    ...
    /* Failed to allocate memory. Use the shared heap  as the last fallback */
    if((addr == NULL) && 
...
void heap_free(void *ptr)
{
    ...
    if((heap_num == HEAP_SRAM) && !hal_get_sram_enabled())
    {
        ...
        PL_PRINT_P1(TR_PL_FREE, "free ptr %lx  on a disabled SRAM HEAP ..", (uintptr_t)ptr);
        return;
    }
#endif /* INSTALL_EXTERNAL_MEM */
