 ...
 */
static void send_exp_const_kip_msg(IPC_PROCESSOR_ID_NUM proc,
                                   exp_const_msgid id,
                                   void *const_table,
                                   unsigned operator,
                                   void *callback)
{
    ...
    con_id = (uint16) PACK_CON_ID(PACK_SEND_RECV_ID(ipc_get_processor_id(),0),
                                  PACK_SEND_RECV_ID(proc,0));
    ...
    msg_id = KIP_MSG_ID_TO_ADAPTOR_MSGID(KIP_MSG_ID_EXPORTED_CONSTANTS_ACTION_REQ);
    adaptor_send_message(proc, msg_id,
...
static void send_message_to_task(exp_const_msgid id, void *const_table, unsigned operator, exp_const_msg_cb *cb_param)
{
    ...
    {
        send_exp_const_kip_msg(IPC_PROCESSOR_0,id,const_table,operator,cb_param);
    }
#endif
}
 ...
 */
static void send_callback_over_kip(IPC_PROCESSOR_ID_NUM proc,void *callback_struct)
{
...
void external_constant_handle_kip_p0(unsigned length,uint16 *msg_data)
{
    ...
    void *const_table = (void*)(uintptr_t)KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_CONSTANT_P_GET(kip_msg);
    unsigned operator = KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_OPERATOR_ID_GET(kip_msg);
    void *cb_param = (void*)(uintptr_t)KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_CALLBACK_INFO_GET(kip_msg);
...
void external_constant_handle_kip_px(unsigned length,uint16 *msg_data)
{
    ...
    exp_const_msg_cb *cb = (exp_const_msg_cb*)(uintptr_t)KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_CALLBACK_INFO_GET(kip_msg);
...
static void process_activity_queue(void)
{
    ...
    case EXP_CONST_MSG_CALLBACK_ID:
        ...
        if (current->proc != ipc_get_processor_id())
        {
...
void mem_utils_task_init(void **data)
{
     ...
     */ 
    if( ipc_get_processor_id() != IPC_PROCESSOR_0)
    {
 ...
 */
void external_constant_callback_when_available(OPERATOR_DATA *op_data,
                                               void *source,
                                               uint16 cmdId,
                                               void *cmdMsg,
                                               pendingContext *context)
{
    ...
    cb->q = mkqid(op_data->task_id, 1);
    cb->id = cmdId;
    cb->msg = cmdMsg;
    cb->rinfo = *((tRoutingInfo*) context);
