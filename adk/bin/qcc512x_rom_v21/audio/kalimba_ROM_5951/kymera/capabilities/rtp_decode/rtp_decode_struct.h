...
#define SRC_ID_INVALID 0xABCD1234u
...
typedef enum RTP_CODEC_STRUCT
{
    ...
    APTXHD = 5,              /* APTXHD */
    APTXADAPTIVE = 6,        /* APTX Adaptive */
    NR_OF_CODECS
} RTP_CODEC_TYPE;

typedef struct RTP_SRC_LATENCY_MAP_ENTRY_STRUCT
{
    uint32 source_id;
    uint32 target_latency;
} RTP_SRC_LATENCY_MAP_ENTRY;

typedef struct RTP_SRC_LATENCY_MAP_STRUCT
{
    unsigned num_entries;
    RTP_SRC_LATENCY_MAP_ENTRY entries[];
} RTP_SRC_LATENCY_MAP;
...
typedef struct RTP_DECODE_OP_DATA
{
    ...
    /** latency chance  notification enable for aptX adaptive */
    unsigned int latency_change_notify_enable:1;
    ...
    union {
        ...
        {
            /**
             * Internal buffer used to buffer a whole frame of encoded data.
             */
            tCbuffer *clone_frame_buffer;

            /**
             * Internal buffer used to buffer a whole frame of encoded data.
             */
            tCbuffer *frame_buffer;

            /**
             * Internal buffer used to hold the latency buffer.
             */
    ...
    /** Pack internal buffer to save some space. */
    bool pack_latency_buffer;

    /** Mapping of source ID to target latency for aptX adaptive */
...
/** RTP header fields. */
typedef struct RTP_HEADER_DATA
{
    /** Sequence number. */
    uint16 seq_num;
    /** RTP "timestamp" - units and meaning are codec-dependent */
    uint32 timestamp;
    /** Source ID */
    uint32 source;
} RTP_HEADER_DATA;
