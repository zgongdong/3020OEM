        ...
        unsigned *terminal)
{
    ...
    switch (terminal_id)
    {
        case INPUT_TERMINAL_ID:
        {
            ...
            {
                bufs->ip_buffer = OPMGR_GET_OP_CONNECT_BUFFER(message_data);
            }
            else
            {
        ...
        case OUTPUT_TERMINAL_ID:
        {
            ...
            {
                bufs->op_buffer = OPMGR_GET_OP_CONNECT_BUFFER(message_data);
            }
            else
            {
            	base_op_change_response_status(response_data, STATUS_CMD_FAILED);
...
bool sco_common_rcv_set_from_air_info_helper(OPERATOR_DATA *op_data, SCO_COMMON_RCV_OP_DATA* sco_rcv_op_data, void *message_data)
{
    ...
    sco_rcv_op_data->sco_rcv_parameters.sco_pkt_size =
        OPMSG_FIELD_GET(message_data, OPMSG_SCO_COMMON_RCV_SET_FROM_AIR_INFO, PKT_SIZE) << 1;

    /* expected packets */
    sco_rcv_op_data->sco_rcv_parameters.exp_pkts =
        OPMSG_FIELD_GET(message_data, OPMSG_SCO_COMMON_RCV_SET_FROM_AIR_INFO, EXP_PKTS);

    /* T_esco */
    sco_rcv_op_data->sco_rcv_parameters.t_esco =
        OPMSG_FIELD_GET(message_data, OPMSG_SCO_COMMON_RCV_SET_FROM_AIR_INFO, T_ESCO);

    /* nuke the expected time stamp */
    sco_rcv_op_data->sco_rcv_parameters.expected_time_stamp = -1;

#if defined(SCO_DEBUG) && defined(INSTALL_AUDIO_LOG)
    sco_fw_sco_info_msg_set((uint16)base_op_get_cap_id(op_data), sco_rcv_op_data->sco_rcv_parameters.t_esco,
        ...
        void **response_data, unsigned input_block_size, unsigned output_block_size)
{
    ...
    *response_data = resp;
    ...
    /* return the terminal's block size - since we are mono-to-mono,
...
bool sco_common_rcv_opmsg_set_ttp_latency(OPERATOR_DATA *op_data, void *message_data,
                                     unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    SCO_COMMON_RCV_OP_DATA *sco_common_rcv_data = get_instance_data(op_data);
    ...
    L4_DBG_MSG2("sco rcv output ttp latency set, op=0x%04x, latency=%d",
                ...
                sco_common_rcv_data->sco_rcv_parameters.timestamp_latency);
    return TRUE;
}
                                          ...
                                          unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    L4_DBG_MSG2("sco rcv output buffer size, op=0x%04x, size=%d",
                base_op_get_ext_op_id(op_data),
                sco_common_rcv_data->sco_rcv_parameters.output_buffer_size);
