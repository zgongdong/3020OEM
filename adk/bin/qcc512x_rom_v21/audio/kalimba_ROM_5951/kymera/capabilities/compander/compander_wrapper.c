...
MAP_INSTANCE_DATA(CAP_ID_COMPANDER, COMPANDER_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_COMPANDER, COMPANDER_OP_DATA)
#endif /* CAPABILITY_DOWNLOAD_BUILD */

/****************************************************************************
Private Function Declarations
*/
static inline COMPANDER_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (COMPANDER_OP_DATA *) base_op_get_instance_data(op_data);
}
...
{
    COMPANDER_OP_DATA *p_ext_data = get_instance_data(op_data);
...
{
    COMPANDER_OP_DATA  *op_extra_data = get_instance_data(op_data);
...
bool compander_wrapper_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (resp)
    {
        ...
        cpsPack2Words(op_state_stat, op_extra_data->lookahead_status, resp);
    }
                 ...
                 uint16 length, unsigned* data, STATUS_KYMERA status,uint16 extra_status_info)
{
    ...
    COMPANDER_OP_DATA *op_extra_data = get_instance_data((OPERATOR_DATA*)instance_data);
