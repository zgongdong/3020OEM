...
{
    SCO_SEND_OP_DATA* opx_data = get_instance_data(op_data);
...
bool sco_send_opmsg_set_to_air_info(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    x_data->frame_size = OPMSG_FIELD_GET(message_data, OPMSG_SCO_SEND_SET_TO_AIR_INFO, FRAME_SIZE);
