...
MAP_INSTANCE_DATA(CAP_ID_BASIC_PASS, BASIC_PASSTHROUGH_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_PASSTHROUGH, BASIC_PASSTHROUGH_OP_DATA)
...
#if !defined(CAPABILITY_DOWNLOAD_BUILD)
MAP_INSTANCE_DATA(CAP_ID_TTP_PASS, BASIC_PASSTHROUGH_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_TTP_PASS, BASIC_PASSTHROUGH_OP_DATA)
#endif /* CAPABILITY_DOWNLOAD_BUILD */
#endif /* INSTALL_OPERATOR_TTP_PASS */
...
*/
static inline BASIC_PASSTHROUGH_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (BASIC_PASSTHROUGH_OP_DATA *) base_op_get_instance_data(op_data);
}



/* Function check if one of the outputs/inputs are connected. */
static bool basic_passthrough_terminals_connected( BASIC_PASSTHROUGH_OP_DATA *opx_data)
{
    unsigned i;

    /* If the number of active channels are bigger than 0 we know
     * that at least one channel is connected. */
    if (opx_data->num_active_chans > 0)
    {
        return TRUE;
    }

    /* Otherwise check all the already created channels. */
    for (i = 0; i < MAX_CHANS; i++)
    {
        if (opx_data->channel[i] != NULL )
        {
            if ((opx_data->channel[i]->op_buffer != NULL)
                    ...
                    ||(opx_data->channel[i]->ip_buffer != NULL)
#endif
                )
            {
                /* Channel with number is connected*/
                return TRUE;
            }
        }
        else
        {
            /* This is the last active channel in the list. No need to go further. */
            return FALSE;
        }
    }

    /* Can happen that only the unput channels are connected. */
    return FALSE;
}
...
{
    BASIC_PASSTHROUGH_OP_DATA *opx_data = get_instance_data(op_data);
                    ...
                    if (opx_data->channel[i]->op_buffer == NULL)
                    {
                        ...
                        if (opx_data->metadata_op_buffer == NULL)
                        {
                            if (buff_has_metadata(opx_data->channel[i]->op_buffer))
                            {
                                ...
                                cbuffer_set_usable_octets(opx_data->metadata_op_buffer,
...
static void metadata_transport_with_ttp(BASIC_PASSTHROUGH_OP_DATA *opx_data, unsigned samples)
{
    ...
    if (mtag != NULL)
    {
        ...
        else
        {
            ...
            ttp_reference_time = time_get_time();
        }
...
#endif
    /*
     * in octets when buffer_ex is true, otherwise in words
     */
...
static void basic_passthrough_processing_ex(BASIC_PASSTHROUGH_OP_DATA *op_data, unsigned octets)
{
    ...
    for (i = 0;i < op_data->num_active_chans;i++)
    {
        ...
        {
            cbuffer_copy_ex(channel->op_buffer,channel->ip_buffer,octets);
        }
    }
}
#endif
...
{
    BASIC_PASSTHROUGH_OP_DATA *op_extra_data = get_instance_data(op_data);
...
bool basic_passthrough_opmsg_obpm_get_status(OPERATOR_DATA *op_data,
                                             void *message_data,
                                             ...
                                             OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (!common_obpm_status_helper(message_data, resp_length, resp_data,
                                   sizeof(BASIC_PASSTHROUGH_STATISTICS), &resp))
    {
         return FALSE;
    }
    ...
    resp = cpsPack2Words(op_extra_data->Cur_mode,
                         op_extra_data->peak_level_1, resp);
    resp = cpsPack2Words(op_extra_data->peak_level_2,
                         op_extra_data->peak_level_3, resp);
    resp = cpsPack2Words(op_extra_data->peak_level_4,
                         op_extra_data->peak_level_5, resp);
    resp = cpsPack2Words(op_extra_data->peak_level_6,
                         op_extra_data->peak_level_7, resp);
    cpsPack2Words(op_extra_data->peak_level_8,
                  op_extra_data->Ovr_Control, resp);
...
{
    BASIC_PASSTHROUGH_OP_DATA *p_ext_data = get_instance_data(op_data);
                 ...
                 uint16 length,  unsigned* data, STATUS_KYMERA status,uint16 extra_status_info)
{
    ...
    BASIC_PASSTHROUGH_OP_DATA *op_extra_data = get_instance_data((OPERATOR_DATA*)instance_data);
...
static bool common_change_terminal_data_type(OPERATOR_DATA *op_data, bool is_input, void *message_data)
{
    ...
    {
        opx_data->op_format = OPMSG_FIELD_GET(message_data, OPMSG_PASSTHROUGH_CHANGE_OUTPUT_DATA_TYPE, DATA_TYPE);
    }
                                            ...
                                            unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
     ...
     * Audio = apply a gain.
     */
    if ((BASIC_PASS_CAP_ID == base_op_get_cap_id(op_data)) || (TTP_PASS_CAP_ID == base_op_get_cap_id(op_data)))
    {
        opx_data = get_instance_data(op_data);
        if (AUDIO_DATA_FORMAT_FIXP == OPMSG_FIELD_GET(message_data, OPMSG_PASSTHROUGH_CHANGE_INPUT_DATA_TYPE, DATA_TYPE))
        {
            opx_data->copy_function = (void *)(&mono_audio_loop);
        }
        else
        {
            opx_data->copy_function = (void *)(&mono_data_loop);
        }
    }
                                            ...
                                            unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    opx_data->simple_data_test_safe = OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_DATA_STREAM_BASED, IS_STREAM_BASED);
...
bool basic_passthrough_opmsg_set_buffer_size(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (opmgr_op_is_running(op_data))
    {
        L2_DBG_MSG("basic_passthrough_opmsg_set_buffer_size: Cannot set the buffer size for a running operator!");
        return FALSE;
    }

    /* basic passthrough is an in-place operator, so if any of the terminals are connected
     * then the buffer size cannot be changed. */
    if (basic_passthrough_terminals_connected(opx_data))
    {
        L2_DBG_MSG("Cannot set the buffer size for an operator with connected terminals!");
        return FALSE;
    }
