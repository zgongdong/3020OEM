...
{
    tCbuffer *metadata_buffer = splitter->internal_metadata.buffer;
    ...
    if (metadata_buffer->metadata != NULL)
    {
        ...
        SPLITTER_MSG1("Deleting remaining metadata tags 0x%08x", tag_list);
        buff_metadata_tag_list_delete(tag_list);
        SPLITTER_MSG1("Deleting internal metadata list 0x%08x", metadata_buffer->metadata);
        /* Delete the internal buffer metadata. */
        pdelete(metadata_buffer->metadata);
        metadata_buffer->metadata = NULL;
    }

    /* There is no buffer without a tCbuffer so even for an
     * sram buffer a cbuffer is created. For internal dsp
     * buffer the normal cbuffer is used. */
...
void splitter_metadata_transport_to_internal(SPLITTER_OP_DATA *splitter,  unsigned trans_octets)
{
    ...
    if (splitter->reframe_enabled)
    {
        ...
        else
        {
            if (splitter->reframe_data.last_tag_samples != LAST_TAG_SAMPLES_INVALID)
            {
                splitter->reframe_data.last_tag_samples += trans_octets;
            }
        }
