...
extern ui_config_table_content_t ui_config_table[];
...
static ui_input_t getUiInput(unsigned logical_input)
{
    ...
    for(uint8 index=0; index<Ui_GetConfigTableSize(); index++)
    {
...
static const char * const ui_GetUiInputAsString(ui_input_t supplied_ui_input)
{
    ...
    message_group_t msg_group = UI_INPUTS_TELEPHONY_MESSAGE_GROUP;
    const char * ret_val = "Bad UI_Input";

    do
    {
        unsigned generated_ui_input = MSG_GRP_TO_ID(msg_group) + msg_id_in_curr_msg_group;
    ...
    while (ui_IsMsgGroupValid(msg_group));
...
void Ui_RegisterContextConsumers(ui_providers_t ui_provider,Task ui_provider_ctxt_consumer_task)
{
    size_t new_size;
