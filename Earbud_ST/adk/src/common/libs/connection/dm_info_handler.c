...
/* Send a link quality cfm message to the client if requested with a sink  */
...
/* Send a link quality cfm message to the client if requested with bdaddr */
static void sendLinkQualityBdaddrCfm(
        Task task,
        hci_status status,
        uint8 link_quality,
        const BD_ADDR_T *bd_addrt
        )
{
    if (task)
    {
        /* Create and send the message */
        MAKE_CL_MESSAGE(CL_DM_LINK_QUALITY_BDADDR_CFM);
        message->status = status;
        message->link_quality = link_quality;
        BdaddrConvertBluestackToVm(&message->bdaddr, bd_addrt);
        MessageSend(task, CL_DM_LINK_QUALITY_BDADDR_CFM, message);
    }
}

/* Send an RSSI cfm message to the client if requested with a sink */
...
/* Send an RSSI cfm message to the client if requested with bdaddr */
        ...
        )
{
    ...
    if (!state->stateInfoLock)
    {
         ...
         * OR sink is NULL and the address passed in the req message must be
         * used.
         */
        else
        {
