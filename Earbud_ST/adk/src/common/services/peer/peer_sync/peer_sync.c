...
peerSyncTaskData    app_peer_sync;
...
{
    peerSyncTaskData* ps = PeerSyncGetTaskData();
        ...
        (!ps->peer_hfp_connected && !ps->peer_avrcp_connected))
    {
        ...
        appConnRulesSetEvent(SmGetTask(),
    ...
    if (!current_a2dp_state && connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_A2DP_CONNECTED);
    }
    else if (current_a2dp_state && !connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_A2DP_DISCONNECTED);
    }
#endif
}
...
static void appPeerSyncUpdateAvrcpConnected(bool connected)
{
    ...
    if (!current_avrcp_state && connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_AVRCP_CONNECTED);
    }
    else if (current_avrcp_state && !connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_AVRCP_DISCONNECTED);
    }
#endif
}
...
static void appPeerSyncUpdateHfpConnected(bool connected)
{
    ...
    if (!current_hfp_state && connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_HFP_CONNECTED);
    }
    else if (current_hfp_state && !connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_HFP_DISCONNECTED);
    }
#endif
}
...
//        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_IN_CASE);
    }
    else if (current_peer_in_case_state && !peer_in_case)
    {
        DEBUG_LOGF("appPeerSyncUpdatePeerInCase Prev %u New %u", current_peer_in_case_state, peer_in_case);
//        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_OUT_CASE);
    }
}
...
//        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_IN_EAR);
    }
    else if (current_peer_in_ear_state && !peer_in_ear)
    {
        DEBUG_LOGF("appPeerSyncUpdatePeerInEar Prev %u New %u", current_peer_in_ear_state, peer_in_ear);
//        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_OUT_EAR);
    }
}
#endif
...
static void appPeerSyncUpdateAdvertising(bool advertising)
{
    ...
    if (current_advertising_state != advertising)
    {
        ...
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_BLE_CONNECTABLE_CHANGE);
    }
}
...
static void appPeerSyncSendStatus(void)
{
    ...
    TaskList_MessageSend(ps->peer_sync_status_tasks, PEER_SYNC_STATUS, message);
}
...
static void appPeerSyncHandlePeerSigMsgChannelTxCfm(const PEER_SIG_MSG_CHANNEL_TX_CFM_T *cfm)
{
    ...
    if (cfm->status == peerSigStatusSuccess)
    {
        ...
        if (appPeerSyncIsComplete())
        {
            ...
            appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_SYNC_VALID);
        }
    }
    else
    {
...
static void appPeerSyncHandlePeerSigMsgChannelTxInd(const PEER_SIG_MSG_CHANNEL_RX_IND_T *ind)
{
    ...
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_A2DP_SUPPORTED);
    }
    if (!(was_supported & DEVICE_PROFILE_AVRCP) & ps->peer_avrcp_connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_AVRCP_SUPPORTED);
    }
    if (!(was_supported & DEVICE_PROFILE_HFP) & ps->peer_hfp_connected)
    {
        appConnRulesSetEvent(SmGetTask(), RULE_EVENT_PEER_HFP_SUPPORTED);
    }
...
void appPeerSyncResetSync(void)
{
    ...
    PEER_SYNC_STATE_RESET(PeerSyncGetTaskData()->peer_sync_state);
}
...
void appPeerSyncStatusClientRegister(Task task)
{
    ...
    TaskList_AddTask(ps->peer_sync_status_tasks, task);
}
...
void appPeerSyncStatusClientUnregister(Task task)
{
    ...
    TaskList_RemoveTask(ps->peer_sync_status_tasks, task);
}
...
bool appPeerSyncInit(Task init_task)
{
    ...
    ps->peer_sync_status_tasks = TaskList_Create();
