...
*/
static bool usb_audio_close (ENDPOINT *ep, bool *pending);
...
static void usb_audio_tx_handle_host_pause(endpoint_usb_audio_state *usb_audio,
                                           unsigned packets_copied, unsigned *input_data_available);
...
bool stream_query_usb_audio(AUDIO_DATA_SERVICE_EP_PARAMS *params)
{
    ...
    L2_DBG_MSG1("EP_USB_AUDIO, QUERY RECEIVED, time=%08x", time_get_time());

#ifdef BAC32
    params->data_buf_flags = MMU_UNPACKED_32BIT_MASK;
#else
...
void stream_create_usb_audio(const AUDIO_DATA_SERVICE_EP_CFG *cfg, AUDIO_DATA_SERVICE_EP_CREATE_RESULT* create_res)
{
    ...
    L2_DBG_MSG2("EP_USB_AUDIO, CREATE RECEIVED, time=0x%08x dir=%d",  time_get_time(), cfg->dir);
    ...
    /* Allocation for PID controller for TTP playback */
    if(cfg->dir == SINK)
    {
        usb_audio->pid = ttp_pid_controller_create();
        if(usb_audio->pid == NULL)
        {
            create_res->success = FALSE;
    ...
    {
        ttp_pid_controller_destroy(usb_audio->pid);
        create_res->success = FALSE;
        stream_destroy_endpoint(ep);
        return;
    }
    ...
    else
    {
        ...
        ttp_pid_controller_set_parameters(usb_audio->pid,
...
bool stream_destroy_usb_audio(void *ep)
{
    ...
    L2_DBG_MSG2("EP_USB_AUDIO, DESTROY RECEIVED, time=0x%08x dir=%d", time_get_time(), ((ENDPOINT*)ep)->direction);
    ...
    /* free allocated ttp_pid structure */
    ttp_pid_controller_destroy(endpoint->state.usb_audio.pid);
    endpoint->state.usb_audio.pid = NULL;
...
static bool usb_audio_close (ENDPOINT *ep, bool *pending)
{
    L2_DBG_MSG2("EP_USB_AUDIO, CLOSE RECEIVED,time=0x%08x, dir=%d", time_get_time(), ep->direction);
...
static bool usb_audio_connect(ENDPOINT *ep, tCbuffer *cb,  ENDPOINT *ep_to_kick, bool* start_on_connect)
{
    ...
    if (ep->direction == SOURCE)
    {
        ...
        L2_DBG_MSG3("EP_USB_AUDIO, CONNECT RECEIVED, time=0x%08x, dir=SOURCE, source_buf=%d (octets), sink_buf=%d (words)",
                    time_get_time(), cbuffer_get_size_in_addrs(cb) , cbuffer_get_size_in_words(usb_audio->source_buf));
    }
    else
    {
        ...
        L2_DBG_MSG3("EP_USB_AUDIO, CONNECT RECEIVED, time=0x%08x, dir=SINK, source_buf=%d (words), sink_buf=%d (octets)",
                    time_get_time(), cbuffer_get_size_in_words(cb) , cbuffer_get_size_in_addrs(usb_audio->sink_buf));
    }
...
static bool usb_audio_disconnect(ENDPOINT *ep)
{
    ...
    L2_DBG_MSG2("EP_USB_AUDIO, DISCONNECT RECEIVED, time=0x%08x dir=%d", time_get_time(), ep->direction);
    ...
    if (buff_has_metadata(usb_audio->source_buf))
    {
        ...
        if((mtag != NULL) && (IS_TIMESTAMPED_TAG(mtag)))
        {
            ...
            if(!usb_audio->timed_playback)
            {
                ...
                ttp_pid_controller_reset(usb_audio->pid);
...
static void usb_audio_tx_transfer_metadata(endpoint_usb_audio_state *usb_audio, unsigned frames_consumed, unsigned frames_written)
{
        ...
        buff_has_metadata(usb_audio->sink_buf))
    {
        ...
        audio_data_service_dequeue_meta(usb_audio->service_priv, afteridx, FALSE);
    }
...
static void usb_audio_tx_reset_timer_period_adjust(endpoint_usb_audio_state *usb_audio)
{
    ...
    usb_audio_rate_measure_reset(usb_audio, time_get_time());
}
...
static void usb_audio_tx_control_ttp_error(endpoint_usb_audio_state *usb_audio, TIME_INTERVAL error, unsigned input_frames_left, unsigned packets_sent)
{
    ...
    if(usb_audio->ttp_control_error_acc_counter == USB_AUDIO_TX_TTP_CONTROL_ERROR_AVARAGING_PERIOD)
    {
        ...
        ttp_pid_controller_run(usb_audio->pid, error_avg);
        STREAM_USB_AUDIO_DBG_MSG4("EP_USB_AUDIO: TTP error control, time=%d, error=%d, period=%d,  warp=%-8d",
                                  time_get_time(),
                                  error_avg,
                                  usb_audio->kick_period,
                                  ttp_pid_controller_get_warp(usb_audio->pid));

        /* Set the SRA */
        cbops_sra_set_rate_adjust(usb_audio->rate_adjust_op, ttp_pid_controller_get_warp(usb_audio->pid));
...
/**
 * usb_audio_tx_handle_host_pause
 * \brief detects and handle to-host pause/resume
 * \param usb_audio pointer to the usb_audio state structure
 * \param packets_copied packets that just copied to sink buffer
 * \param input_data_available pointer, data that are available in
 *         source buffer in usb audio frames (will be updated during
 *         pause period if frames are discarded).
 */
static void usb_audio_tx_handle_host_pause(endpoint_usb_audio_state *usb_audio,
                                           unsigned packets_copied, unsigned *input_data_available)
{
    ...
    /* To host pause detection */
    if(packets_copied == 0)
    {
        if(!usb_audio->tx_host_in_pause)
        {
            /* To-host is assumed paused if we cannot copy for at least a certain amount of time */
            usb_audio->no_copy_counter++;
            if(USB_AUDIO_TX_HOST_PAUSE_THRESHOLD_KICKS <= usb_audio->no_copy_counter)
            {
                /* Do a one-off silencing of packets that are already in the sink buffer so once
                 * resumed stale packets will not cause glitches.                         *
                 */

                /* Always there are max_packets_in_tx_output_buffer packets in the buffer,
                 * calculate the total length in octets.
                 */
                unsigned octets_to_silence = usb_audio->packet_len_in_frames *
                                              usb_audio->frame_in_octets *
                                              usb_audio->max_packets_in_tx_output_buffer;

                /* for 44.1KHz and likes add equivalent of one sample
                 * in case one of the packets is an odd one.
                 */
                if(usb_audio->frames_in_packet_rem != 0)
                {
                    octets_to_silence += usb_audio->frame_in_octets;
                }

                /* Silence the packets in sink buffer */
                stream_usb_audio_silence_sink_buffer(usb_audio->sink_buf,
                                                     octets_to_silence);

                /* Pause detected */
                usb_audio->tx_host_in_pause = TRUE;
                L2_DBG_MSG1("USB AUDIO TX: Host pause detected!, time = %d", hal_get_time());
            }
        }

        /* When pause is detected this endpoint, which is at the end of an audio chain,
         * needs to discard audio samples as they are arriving from source side.
         * In timed playback mode this is not needed here as it will be done in timely
         * manner when handling late tags.
         */
        if(usb_audio->tx_host_in_pause && !usb_audio->timed_playback)
        {
            /* host has paused, trash some data from input buffer so
             * data won't become stale. We trash at nominal rate because
             * we will report nominal rate for the endpoint in this situation.
             * However please note that the amount of trash will also be limited
             * to amount of data available in the input buffer, assuming rate
             * matching is working reasonably well the buffer should have the
             * number of frames that we want to trash.
             */
            unsigned i;
            unsigned frames_to_trash = 0;

            for (i=0; i < USB_AUDIO_TX_PACKETS_PER_KICK; ++i)
            {
                frames_to_trash += usb_audio->packet_len_in_frames;
                usb_audio_tx_update_packet_length(usb_audio);
            }

            /* limit to the amount of data available */
            frames_to_trash = MIN(frames_to_trash, *input_data_available);
            if(frames_to_trash > 0)
            {
                /* advance read pointer to trash */
                cbuffer_advance_read_ptr(usb_audio->source_buf,
                                         frames_to_trash*usb_audio->n_channels);
                usb_audio_tx_transfer_metadata(usb_audio, frames_to_trash, 0);
                L3_DBG_MSG2("EP_USB_AUDIO TX - host is paused, %d frames discarded, time =%d",
                            frames_to_trash, hal_get_time());
                *input_data_available -= frames_to_trash;
            }

        } /* usb_audio->tx_host_in_pause */

    } /* packets_copied == 0 */
    else
    {
        usb_audio->no_copy_counter = 0;
        if(usb_audio->tx_host_in_pause)
        {
            usb_audio->tx_host_in_pause = FALSE;
            L2_DBG_MSG1("USB AUDIO TX: Host resume detected!, time = %d", hal_get_time());
        }
    }
}
...
static void usb_audio_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
       ...
       )
    {
        ...
        else /* ep->direction == SOURCE */
        {
            ...
            TIME cur_time = ep->deferred.kick_is_deferred? ep->deferred.interrupt_handled_time:time_get_time();
            ...
            while(packets_in_buf < usb_audio->max_packets_in_tx_output_buffer)
            {
...

                {
                    ...
                    STREAM_USB_AUDIO_DBG_MSG2("EP_USB_AUDIO, Tx, raw data written,  time = %08x, subframes=%d",
                                              time_get_time(), subframes_written);
            ...
            /* handle host pause/resume */
            usb_audio_tx_handle_host_pause(usb_audio, packets_copied, &input_data_available);
    ...
    else /* ep->connected_to */
    {
        STREAM_USB_AUDIO_DBG_MSG3("EP_USB_AUDIO, KICK NOT PROCESSED, time=0x%08x dir=%d, ep=0x%x",
                                  time_get_time(), ep->direction, (unsigned)ep);
        if(ep->direction == SOURCE)
        {
            /* discard anything in the input buffer */
...
static bool usb_audio_start(ENDPOINT *ep, KICK_OBJECT *ko)
{
    L2_DBG_MSG4("EP_USB_AUDIO, START RECEIVED, time=0x%08x dir=%d, ep=0x%x, running=%d",
                time_get_time(), ep->direction, (unsigned)ep, ep->state.usb_audio.running);
