 ...
 * stream_close_endpoint_check_pending <br>
 */
...
/****************************************************************************
 *
 * stream_con_id_from_ep
 *
 */
unsigned int stream_con_id_from_ep(ENDPOINT *ep)
{
    if (ep == NULL)
    {
        return INVALID_CON_ID;
    }
    return ep->con_id;
}
 ...
 *
 */
bool stream_get_endpoint_device(ENDPOINT *endpoint, STREAM_DEVICE* device)
{
 ...
 *
 */
bool stream_get_connected_to_endpoint_config(ENDPOINT *endpoint,
...
#ifdef HAVE_ANC_HARDWARE
    COMPILE_TIME_ASSERT(OVERRIDE_AUDIO_SAMPLE_RATE == ACCMD_INFO_KEY_AUDIO_SAMPLE_RATE,
                        Assumption_about_OVERRIDE_AUDIO_SAMPLE_RATE_in_stream_violated);
#endif /* HAVE_ANC_HARDWARE */

    COMPILE_TIME_ASSERT(OVERRIDE_RATEMATCH_ABILITY == EP_RATEMATCH_ABILITY,
                        Assumption_about_OVERRIDE_RATEMATCH_ABILITY_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_RATEMATCH_ENACTING == EP_RATEMATCH_ENACTING,
                        Assumption_about_OVERRIDE_RATEMATCH_ENACTING_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_RATEMATCH_RATE == EP_RATEMATCH_RATE,
                        Assumption_about_OVERRIDE_RATEMATCH_RATE_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_RATEMATCH_ADJUSTMENT == EP_RATEMATCH_ADJUSTMENT,
                        Assumption_about_OVERRIDE_RATEMATCH_ADJUSTMENT_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_SET_INPUT_GAIN == EP_SET_INPUT_GAIN,
                        Assumption_about_OVERRIDE_SET_INPUT_GAIN_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_SET_OUTPUT_GAIN == EP_SET_OUTPUT_GAIN,
                        Assumption_about_OVERRIDE_SET_OUTPUT_GAIN_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_RATE_ADJUST_OP == EP_RATE_ADJUST_OP,
                        Assumption_about_OVERRIDE_RATE_ADJUST_OP_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_HW_WARP_APPLY_MODE == EP_HW_WARP_APPLY_MODE,
                        Assumption_about_OVERRIDE_HW_WARP_APPLY_MODE_in_stream_violated);
    COMPILE_TIME_ASSERT(OVERRIDE_CURRENT_HW_WARP == EP_CURRENT_HW_WARP,
                        Assumption_about_OVERRIDE_CURRENT_HW_WARP_in_stream_violated);

    if (!result_value ||
        !endpoint ||
        !endpoint->connected_to ||
        !endpoint->connected_to->functions->get_config)
    {
        return FALSE;
    }

    result.u.value = config_value;
    status = endpoint->connected_to->functions->get_config(endpoint->connected_to,
                                                           (unsigned)key,
                                                           &result);
    *result_value = result.u.value;
 ...
 *
 */
bool stream_configure_connected_to_endpoint(ENDPOINT *endpoint,
                                            OVERRIDE_CONFIG_KEYS key,
                                            uint32 value)
{
    if (!endpoint ||
        !endpoint->connected_to ||
        !endpoint->connected_to->functions->configure)
    {
        return FALSE;
    }

    return endpoint->connected_to->functions->configure(endpoint->connected_to,
                                                        (unsigned)key,
                                                        value);
}
...
/****************************************************************************
 *
 * stream_connected_to_endpoints_have_sidetone_route
 *
 */
bool stream_connected_to_endpoints_have_sidetone_route(ENDPOINT *mic_ep,
                                                              ENDPOINT *spkr_ep)
{
    if (!mic_ep || !spkr_ep || !mic_ep->connected_to || !spkr_ep->connected_to)
    {
        return FALSE;
    }

    mic_ep = mic_ep->connected_to;
    spkr_ep= spkr_ep->connected_to;
    /* only audio endpoints */
    if (mic_ep->stream_endpoint_type != endpoint_audio ||
        spkr_ep->stream_endpoint_type != endpoint_audio)
    {
        return FALSE;
    }

    return stream_audio_have_sidetone_path_setup(mic_ep, spkr_ep);
}
...
/****************************************************************************
 *
 * stream_close_endpoint_check_pending
 *
 */
bool stream_close_endpoint_check_pending(ENDPOINT *endpoint, bool *pending)
{
    patch_fn_shared(stream);
    ...
    if (!endpoint->functions->close(endpoint, pending))
    {
        return FALSE;
    }
...
{
    bool pending = FALSE;
    bool result = stream_close_endpoint_check_pending(endpoint, &pending);
    /* Verify that the endpoints have not change the pending state unexpectedly. */
    PL_ASSERT(pending==FALSE);
    return result;
}
...
void stream_close_endpoints_with_con_id(unsigned con_id)
{
    ...
    /* Below idx is used to make us go round the loop
       twice. The first time ep is set to the source
 ...
 *
 */
bool stream_close_dummy(ENDPOINT *ep, bool *pending)
{
    return TRUE;
}
...
void stream_debug_buffer_levels ( ENDPOINT *endpoint)
{
    ...
    TIME curr_time;
    curr_time = time_get_time() & 0xFFFFFF;
...
/*
 * get current sample rate of an endpoint
 */
bool stream_get_sample_rate(ENDPOINT *ep, uint32 *value)
{
    return stream_get_endpoint_config(ep, EP_SAMPLE_RATE, value);
}
