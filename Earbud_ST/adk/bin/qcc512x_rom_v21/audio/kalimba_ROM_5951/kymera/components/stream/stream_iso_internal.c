...
static bool set_from_air_iso_specific_info_cback(unsigned con_id, unsigned status,
 ...
 * Close the endpoint
 */
bool iso_close(ENDPOINT *endpoint, bool *pending)
{
    patch_fn_shared(stream_iso);
    return sco_close(endpoint, pending);
}
