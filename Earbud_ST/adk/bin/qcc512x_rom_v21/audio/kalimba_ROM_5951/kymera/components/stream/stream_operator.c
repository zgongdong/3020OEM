...
static bool operator_close (ENDPOINT *endpoint, bool *pending);
...
static bool operator_have_same_clock(ENDPOINT *ep1, ENDPOINT *ep2, bool both_local);
static AUDIO_DATA_FORMAT operator_get_data_format (ENDPOINT *endpoint);
static OPMSG_CONFIGURATION_KEYS transfom_endpoint_key_to_operator_key(unsigned int key);
static bool destroy_op_ep_internal(ENDPOINT *ep);
static void add_ep_to_terminal_list(ENDPOINT *ep, ENDPOINT **terminal_list);
static void remove_ep_from_terminal_list(ENDPOINT *ep, ENDPOINT **terminal_list);

DEFINE_ENDPOINT_FUNCTIONS (operator_functions, operator_close, operator_connect,
...
ENDPOINT *stream_create_operator_endpoint( unsigned opidep, unsigned con_id)
{
    ...
    opmgr_get_terminal_details_from_operator(opidep, &op_endpoint_details);
    ...
    add_ep_to_terminal_list(ep, opmgr_get_terminal_list(ep->key));
...
static bool destroy_op_ep_internal(ENDPOINT *ep)
{
    remove_ep_from_terminal_list(ep, opmgr_get_terminal_list(ep->key));
...
/**
 * \brief Cleanup when closing this endpoint
 */
static bool operator_close (ENDPOINT *endpoint, bool *pending)
{
    remove_ep_from_terminal_list(endpoint, opmgr_get_terminal_list(endpoint->key));
    return TRUE;
}
...
static bool operator_get_config(ENDPOINT *endpoint, unsigned int key, ENDPOINT_GET_CONFIG_RESULT *result)
{
    ...
    switch (key)
    {
        case EP_RATEMATCH_MEASUREMENT:
        {
            ...
            success = opmgr_get_ratematch_measure_from_operator(
                          stream_external_id_from_endpoint(endpoint),
                          &sp_deviation,
                          &measurement);
            result->u.rm_meas.sp_deviation = sp_deviation;
            result->u.rm_meas.measurement = measurement;
        ...
        default:
        {
            ...
            success = opmgr_get_generic_value_from_operator(
                          stream_external_id_from_endpoint(endpoint),
                          transfom_endpoint_key_to_operator_key(key),
                          &value);
...
/**
 * \brief Add endpoint to operator terminal list
 */
static void add_ep_to_terminal_list(ENDPOINT *ep, ENDPOINT **terminal_list)
{
    if (terminal_list != NULL && ep != NULL)
    {
        interrupt_block();
        ...
        while (*terminal_list != NULL)
        {
            ...
            terminal_list = &((*terminal_list)->state.operator.next_terminal);
        }

        ep->state.operator.next_terminal = NULL;
        *terminal_list = ep;
...
/**
 * \brief Remove endpoint from operator terminal list
 */
static void remove_ep_from_terminal_list(ENDPOINT *ep, ENDPOINT **terminal_list)
{
    if (terminal_list != NULL && ep != NULL)
    {
        ...
        while (*terminal_list != NULL)
        {
            if (*terminal_list == ep)
            {
                *terminal_list = ep->state.operator.next_terminal;
                ep->state.operator.next_terminal = NULL;
                break;
            }
            terminal_list = &((*terminal_list)->state.operator.next_terminal);
