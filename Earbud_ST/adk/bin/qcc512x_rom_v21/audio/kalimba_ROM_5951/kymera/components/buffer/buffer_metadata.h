...
typedef struct metadata_eof_callback_ref metadata_eof_callback_ref;

typedef struct metadata_eof_callback metadata_eof_callback;
...
struct metadata_list_struct
{
    ...
    unsigned ref_cnt:8;       /**< number of buffers sharing this metadata */
    unsigned buffer_size:16;  /**< cached size of the buffers using this metadata, in octets */
...
/**
 * \brief Get total length of existing private data
 *
 * \param tag The metadata tag
 * \return Length of private data (in allocation units)
 */
extern unsigned buff_metadata_get_priv_data_length(metadata_tag *tag);
 ...
 *
 */
extern void* buff_metadata_add_private_data(metadata_tag *tag, METADATA_PRIV_KEY key, unsigned length, void *data);
 ...
 */
extern bool buff_metadata_find_private_data(metadata_tag *tag, METADATA_PRIV_KEY key, unsigned *out_length, void **out_data);
 ...
 *
 * \param buff Pointer to cbuffer structure
 ...
 *  There are no checks the cbuffer is valid nor that it has metadata.
 */
extern unsigned int buff_metadata_get_buffer_size(tCbuffer *buff);

/**
 * Sets the buffer size (as cached by metadata submodule).
 *
 * \param buff Pointer to cbuffer structure
 * \param usable_octets usable octets per word.
 * \note It is the caller's responsibility to make sure the buffer is
 * configured WITH metadata (will do nothing otherwise).
 */
extern void buff_metadata_set_buffer_size(tCbuffer *buff, unsigned usable_octets);
 ...
 */
extern void metadata_strict_transport( tCbuffer *src, tCbuffer *dst, unsigned tran_octets);
