...
int main(void)
{
    ...
    */
    ...
    if (hal_get_reg_processor_id() != IPC_PROCESSOR_0)
    {
...
#error "Capability download only supports dual-core on single image Kymera builds"
    ...
    uint32 patchSize = 0; // todo initialize_patches should return the current size to reserve for patches at the start of pm_ram
    if (patchStartAddress != 0){
		patchSize=initialize_patches(patchStartAddress); // (unsigned)(&PM_RAM_PATCH_CODE_START_HEADER) from linker script
    }

#if defined(INSTALL_CAP_DOWNLOAD_MGR)
    /* PM heap starts after patches */
    malloc_pm_init_start_offset(patchSize);
    /* Malloc PM initialisation ideally needs to happen after MIB is loaded.
     * But Napier does not have a system of loading the MIB at boot. So hope
     * that the default values are good enough and proceed */
    init_malloc_pm();
#endif /* INSTALL_CAP_DOWNLOAD_MGR */
#endif /* CHIP_BASE_NAPIER */
...
#elif !defined(CHIP_BASE_NAPIER) && !defined(INSTALL_PM_HEAP_SHARE) && defined(INSTALL_CAP_DOWNLOAD_MGR)
    /* Napier already did the malloc_pm init above. On all other platforms that
     * have malloc_pm, all of PM can be used by the PM heap */
    init_malloc_pm();
#endif /* INSTALL_HYDRA */
    }
