 ...
 *
 */
static bool stream_anc_dummy_callback(unsigned dummy_con_id,
 ...
 *
 */
void stream_anc_set_anc_iir_coeffs(STREAM_ANC_INSTANCE instance,
                                   ...
                                   const uint16 *coeffs)
{
    ...
    audio_hwm_anc_set_anc_iir_filter(instance, path_id,
                                     (uint16) num_coeffs, coeffs);
}
 ...
 *
 */
void stream_anc_set_anc_lpf_coeffs(STREAM_ANC_INSTANCE instance,
                                   STREAM_ANC_PATH path_id,
                                   uint16 shift1, uint16 shift2)
{
    patch_fn_shared(stream_anc);

    audio_hwm_anc_set_anc_lpf_filter(instance, path_id, shift1, shift2);
}
 ...
 */
void stream_anc_enable_wrapper(unsigned con_id,
                               uint16 anc_enable_0,
                               uint16 anc_enable_1,
                               bool (*resp_callback)(unsigned con_id,
                                                     STATUS_KYMERA status))
{
    bool (*wrapper_callback)(unsigned con_id, STATUS_KYMERA status);
                                 ...
                                 uint32 value)
{
    ...
    switch (key)
    {
        ...
        case STREAM_CONFIG_KEY_STREAM_ANC_CONTROL:
        {
            ...
            result = audio_hwm_anc_source_set_anc_control(instance_id, val);
            if (result == FALSE)
            {
                STREAM_ANC_L5_DBG_MSG2("stream_anc: stream_anc_source_configure(): "
