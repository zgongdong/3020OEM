        ...
        tCbuffer *buffer)
{
    ...
    {
        fault_diatribe(FAULT_AUDIO_IN_PLACE_CHAIN_TOO_LONG, inplace_buffer_list->count);
        return FALSE;
    }
        ...
        IN_PLACE_DISCONNECT_PARAMETERS* in_place_disconnect_parameters)
{
    ...
    if (buff_to_destroy == NULL)
    {
        panic_diatribe(PANIC_AUDIO_IN_PLACE_DISCONNECT, 1);
    }

    if ((inplace_buffer_list_to_source != NULL) &&
        (inplace_buffer_list_to_sink != NULL))
    {
        /* Case 1 */
        ...
        in_place_set_head_aux_ptr(inplace_buffer_list_to_source,
                                  in_place_list_get_tail(inplace_buffer_list_to_source));
        in_place_set_head_aux_ptr(inplace_buffer_list_to_sink,
                                  in_place_list_get_tail(inplace_buffer_list_to_sink));
    ...
    else if ((inplace_buffer_list_to_source == NULL) &&
             (inplace_buffer_list_to_sink == NULL))
    {
        /* Case 4 */
        pfree(buff_to_destroy->base_addr);
        buff_to_destroy->base_addr = NULL;
    }
    else
    {
        IN_PLACE_BUFFER_LIST* inplace_buffer_list;
        unsigned max_buffer_size;

        if (inplace_buffer_list_to_source != NULL)
        {
            /* Case 2 */
            inplace_buffer_list = inplace_buffer_list_to_source;
        }
        else
        {
            /* Case 3 */
            inplace_buffer_list = inplace_buffer_list_to_sink;
        }
        max_buffer_size = inplace_buffer_list->max_buffer_size;

        /* Compare in addresses. */
        if (buff_to_destroy->size > max_buffer_size)
        {
            /* Reduce the buffer size. */
            int *new_base;

            /* Free the buffer and allocate a new one with a smaller size.
               This is done with interrupted blocked to guarantee success. */
            interrupt_block();
            pfree(buff_to_destroy->base_addr);
            buff_to_destroy->base_addr = NULL;
            /* max_buffer_size is already in bytes.*/
            new_base = (int *) xppmalloc(max_buffer_size, MALLOC_PREFERENCE_FAST);
            interrupt_unblock();

            update_in_place_chain(inplace_buffer_list,
                                  new_base,
                                  max_buffer_size);
        }

        /* Update the head of the list. */
        in_place_set_head_aux_ptr(inplace_buffer_list,
                                  in_place_list_get_tail(inplace_buffer_list));

        /* inplace_buffer_list was pointing to parameters_inplace_buffer_list.
         * No need to destroy the in place buffer list. */
    }
}
