...
static inline bool KIP_PRIMARY_CORE_ID(IPC_PROCESSOR_ID_NUM id)
{
    return (id) == IPC_PROCESSOR_0;
}
#define KIP_SECONDARY_CORE_ID(id) (!KIP_PRIMARY_CORE_ID(id))

#define KIP_PRIMARY_CONTEXT()   IPC_PRIMARY_CONTEXT()
#define KIP_SECONDARY_CONTEXT() IPC_SECONDARY_CONTEXT()
#define KIP_ON_SAME_CORE(x)     IPC_ON_SAME_CORE(x)
...
IPC_PROCESSOR_ID_NUM kip_get_max_processor_id(void);
...
bool kip_aux_processor_has_started(IPC_PROCESSOR_ID_NUM proc_id);
 ...
 */
bool kip_aux_processor_disable(IPC_PROCESSOR_ID_NUM processor_id);
...
unsigned kip_aux_processor_start(unsigned con_id, IPC_PROCESSOR_ID_NUM processor_id, bool (*callback)(unsigned con_id, unsigned status));
/* Stop secondary processor. Return readily usable STATUS_OK or STATUS_CMD_FAILED toward the command
 * adaptor (it translates IPC status) */
unsigned kip_aux_processor_stop(unsigned con_id, IPC_PROCESSOR_ID_NUM processor_id, bool (*callback)(unsigned con_id, unsigned status));
