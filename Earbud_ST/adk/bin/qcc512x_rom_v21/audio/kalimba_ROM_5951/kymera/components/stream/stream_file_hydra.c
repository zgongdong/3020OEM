...
static bool file_close (ENDPOINT *ep, bool *pending);
...
void stream_create_file(const AUDIO_DATA_SERVICE_EP_CFG *cfg, AUDIO_DATA_SERVICE_EP_CREATE_RESULT* create_res)
{
    ...
    /** configure for SBC 16-bit file streaming, by default */
    file->shift = 0;
    file->byte_swap = TRUE;
    file->usable_octets = NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD);
...
static bool file_connect(ENDPOINT *ep, tCbuffer *cb,  ENDPOINT *ep_to_kick, bool* start_on_connect)
{
    ...
    cbuffer_set_usable_octets(cb, file->usable_octets);
    cbuffer_set_usable_octets(file->source_buf, FILE_INPUT_OCTETS_PER_WORD);
...
static void file_internal_kick(void *priv)
{
    ...
    audio_data_service_file_kick(priv, STREAM_KICK_INTERNAL);
}
    ...
    switch(key)
    {
        ...
        case EP_DATA_FORMAT:
                ...
                (format == AUDIO_DATA_FORMAT_16_BIT_WITH_METADATA))
            {
                ...
                ep->state.file.usable_octets = NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD);
                ep->state.file.data_format = format;
                return TRUE;
            }
            else if (format == AUDIO_DATA_FORMAT_ENCODED_DATA)
            {
                ep->state.file.usable_octets = ENCODED_DATA_OCTETS_IN_WORD;
                ep->state.file.shift = 0;
                /** For encoded (sbc) data byte swap ought to be set */
                ep->state.file.byte_swap = TRUE;
                ep->state.file.data_format = format;
                ...
                (format == AUDIO_DATA_FORMAT_FIXP_WITH_METADATA))
            {
                ...
                ep->state.file.usable_octets = NR_OF_OCTETS_IN_WORD(USE_32BIT_PER_WORD);
                ep->state.file.data_format = format;
                return TRUE;
            }
            else
...
static bool file_get_config(ENDPOINT *ep, unsigned int key, ENDPOINT_GET_CONFIG_RESULT* result)
{
    ...
    {
        case EP_DATA_FORMAT:
            result->u.value = file->data_format;
            return TRUE;
