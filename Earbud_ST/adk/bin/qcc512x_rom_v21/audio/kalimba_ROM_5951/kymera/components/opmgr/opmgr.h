...
typedef enum
{
    OPMGR_ADAPTOR_DEFAULT = 0x0,
    OPMGR_ADAPTOR_OBPM = 0x1F,
#ifdef CHIP_BASE_BC7
    OPMGR_ADAPTOR_STIBBONS = 0x1
#endif /* CHIP_BASE_BC7 */
} OPMGR_ADAPTOR_MSG;
 ...
 * \param  callback - Function pointer to the function to call with the response.
 */
extern void opmgr_operator_message(unsigned int con_id, EXT_OP_ID op_id,
...
extern unsigned int opmgr_get_list_remote_ops_count(unsigned int num_ops, unsigned int *op_list, OP_PROCESSOR_ID proc_id);
 ...
 */
extern bool opmgr_get_processor_id_from_opid(unsigned ext_opid, OP_PROCESSOR_ID *processor_id);
 ...
 * \return TRUE if the operator endpoint exits
 */
extern bool opmgr_get_processor_id( unsigned int ep_id, OP_PROCESSOR_ID *processor_id);
