...
MAP_INSTANCE_DATA(CAP_ID_VSE, VSE_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_VSE, VSE_OP_DATA)
...
*/
/* ******************************* Helper functions ************************************ */
static inline VSE_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (VSE_OP_DATA *) base_op_get_instance_data(op_data);
}

/* ********************************** API functions ************************************* */
bool vse_wrapper_create(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    VSE_OP_DATA *p_ext_data = get_instance_data(op_data);
...
{
    VSE_OP_DATA *op_extra_data = get_instance_data(op_data);
...
bool vse_wrapper_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (resp)
    {
        ...
        op_state_status = (chan_mask == VSE_TWO_CHANNEL_MASK) ? OPSTATE_INTERNAL_CONNECTED : OPSTATE_INTERNAL_READY;
        state = opmgr_op_is_running(op_data) ? 0 : 1;

        resp = cpsPack2Words(op_extra_data->Cur_mode, op_extra_data->Ovr_Control, resp);
        resp = cpsPack2Words(comp_config, state, resp);
...
{
    VSE_OP_DATA *op_extra_data = get_instance_data((OPERATOR_DATA*)instance_data);
...
bool vse_channel_create(OPERATOR_DATA *op_data, MULTI_CHANNEL_CHANNEL_STRUC *chan_ptr, unsigned chan_idx)
{
    ...
    p_dobject = xzppnew(t_vse_object, MALLOC_PREFERENCE_DM1);
    if (p_dobject == NULL)
    {
        return FALSE;
    }
    ...
    out_ipsi = (unsigned int *) xzppmalloc(VSE_CAP_INTERNAL_BUFFER_SIZE * sizeof(unsigned int),
    ...
    out_contra = (unsigned int *) xzppmalloc(VSE_CAP_INTERNAL_BUFFER_SIZE * sizeof(unsigned int),
                                             MALLOC_PREFERENCE_DM1);
    if (out_contra == NULL)
    {
        pfree(out_ipsi);
    ...
    /* Only link to the external world once all allocations have been succesful. */
