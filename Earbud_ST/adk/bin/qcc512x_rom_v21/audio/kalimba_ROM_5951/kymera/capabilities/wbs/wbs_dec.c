...
bool wbs_dec_create(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    if(!cpsInitParameters(&wbs_dec->sco_rcv_op_data.parms_def,(unsigned*)WBS_DEC_GetDefaults(base_op_get_cap_id(op_data)),(unsigned*)&wbs_dec->sco_rcv_op_data.sco_rcv_parameters.force_plc_off,sizeof(WBS_DEC_PARAMETERS)))
    {
        wbs_dec_free_state_data(op_data);
        /* Change the already allocated response to command failed. No extra error info. */
        base_op_change_response_status(response_data, STATUS_CMD_FAILED);
...
bool wbs_dec_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        resp = cpsPack2Words(wbs_dec_params->wbs_dec_no_output, wbs_dec_params->wbs_dec_fake_pkt, resp);
        resp = cpsPack2Words(wbs_dec_params->wbs_dec_good_output, wbs_dec_params->wbs_dec_bad_output, resp);
    }
