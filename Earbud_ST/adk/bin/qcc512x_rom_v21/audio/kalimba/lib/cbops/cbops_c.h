 ...
 */
extern void cbops_mute_enable(cbops_op *op, bool enable);
...
/**
 * create a cbops dc_offset operator
 *
 * \param nr_channels Number of channels at creation time.
 * \param io_idx Pointer to input/output channel indexes, operator
 *        will always be in-place.
 * \return pointer to created cbops dc offset operator
 */
extern cbops_op* create_dc_offset_op(unsigned nr_channels, unsigned* io_idx, unsigned step);

/**
 *  cbops_configure_ dc_offset_op
 *  Configure cbops dc offset individual channel
 *  op: cbops dc_offset op
 *  chan: channel index
 *   dc_offset_target: target added dc offset value (fixed-point fractional)
 */
extern void  cbops_configure_dc_offset_op(cbops_op *op, unsigned chan, int dc_offset_target);

/**
 * cbops_dc_offset_zero_all
 *    clears all dc offset configuration, dc offset will gradually
 *    return back to 0 based on the configured step size.
 * \param op Pointer to cbops_dc_offset operator structure
 */
extern void cbops_dc_offset_zero_all(cbops_op *op);


/**
 * find_cbops_dc_offset_op
 * Searches a graph to find a cbops dc_offset operator.
 * (in fact only looks at the end of graph as expected this op,
 * if present, always to be appended to end of graph)
 * \param graph cbops graph to search
 */
extern cbops_op *find_cbops_dc_offset_op(cbops_graph *graph);

/**
 * setup_cbops_dc_offset_compensation
 * \brief setup dc offset compensation for a cbops graph if needed
 * \param graph: cbops graph to setup the dc offset for
 * \gcbops_task_freq: cbops tasks per second (truncation is ok)
 * \returns the dc_offset cbops op that is appeneded to the end of
 *          graph, NULL otherwise.
 */
cbops_op *setup_cbops_dc_offset_compensation(cbops_graph *graph, unsigned cbops_task_freq);
