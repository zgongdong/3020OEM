...
#include "codec_library.h"
...
#ifdef DATAFORMAT_32
   .CONST        $sbc.DATA_WIDTH_PER_WORD         32;
   .CONST        $sbc.OCTETS_PER_WORD             4;
#else
   .CONST        $sbc.DATA_WIDTH_PER_WORD         16;
   .CONST        $sbc.OCTETS_PER_WORD             2;
#endif
   ...
   // these are the maximum values of the frame sizes
   .CONST $wbs.decoder.WBS_DECODED_FRAME_SIZE_W   120;
   .CONST $wbs.decoder.WBS_ENCODED_FRAME_SIZE_B   60;

#ifdef DATAFORMAT_32
   /***** NOTE: Keep this in sync with the values in sbc_c.h */
   .CONST       $sbc.mem.TMP_ENC_BUFF_SIZE        ($wbs.decoder.WBS_ENCODED_FRAME_SIZE_B >> LOG2_ADDR_PER_WORD);   // MAX ENCODED FRAME SIZE IN WORDS
   .CONST       TMP_ENC_BUFF_TOTAL_SIZE           $sbc.mem.TMP_ENC_BUFF_SIZE + $cbuffer.STRUC_SIZE;
   .CONST       TMP_ENC_DEC_BUFF_SIZE             TMP_ENC_BUFF_TOTAL_SIZE + $cbuffer.STRUC_SIZE;
   .CONST       TMP_ENC_DEC_BUFF_TOTAL_SIZE       TMP_ENC_DEC_BUFF_SIZE + $cbuffer.STRUC_SIZE;
#endif      // DATAFORMAT_32
...
// decoder
   ...
   /***** NOTE: Make sure the following structure size constant matches the one defined in sbc_c.h at all times...! ******/
   .CONST       $sbc.mem.STRUC_SIZE                               70;
#else
   .CONST       $sbc.mem.WBS_BITS_TO_COPY_FIELD                   70*ADDR_PER_WORD;          // N            For WBS only, internal
   .CONST       $sbc.mem.WBS_ENC_TMP_OP_BUFF_FIELD                71*ADDR_PER_WORD;          // N            For WBS only, internal
   .CONST       $sbc.mem.WBS_ENC_TMP_OP_CB_STRUCT_FIELD           (71 + $sbc.mem.TMP_ENC_BUFF_SIZE)*ADDR_PER_WORD;      // N            For WBS only, internal
   .CONST       $sbc.mem.WBS_FRAME_CB_STRUCT_FIELD                (71 + TMP_ENC_BUFF_TOTAL_SIZE)*ADDR_PER_WORD;         // N            For WBS only, internal
   .CONST       $sbc.mem.WBS_CLONED_IP_CB_STRUCT_FIELD            (71 + TMP_ENC_DEC_BUFF_SIZE)*ADDR_PER_WORD;           // N            For WBS only, internal
   /***** NOTE:  as above for SIZE */
   .CONST       $sbc.mem.STRUC_SIZE                               (71 + TMP_ENC_DEC_BUFF_TOTAL_SIZE);
#endif // DATAFORMAT_32
