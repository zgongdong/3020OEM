...
static inline void transform_metadata_header(int *data, CONNECTION_TYPE type, stream_sco_metadata *sco_metadata)
{
    ...
    {
        SCO_DBG_ERRORMSG("Error! Metadata reserved not zero. This probably means that metadata "
    ...
    sco_metadata->status = (metadata_status)MASK_16_BIT(status_val);
...
bool enough_data_to_run(SCO_COMMON_RCV_OP_DATA* sco_data, unsigned data_size)
{
    ...
    unsigned input_data = cbuffer_calc_amount_data_in_words(ip_buffer);

#ifdef SCO_RX_OP_GENERATE_METADATA
    if(buff_has_metadata(ip_buffer))
    {
        /* make sure we don't consume more than what metadata indicates */
        unsigned input_meta = buff_metadata_available_octets(ip_buffer)
                                        / cbuffer_get_usable_octets(ip_buffer);
...
stream_sco_metadata_status read_packet_metadata(SCO_COMMON_RCV_OP_DATA* sco_data, stream_sco_metadata *sco_metadata, CONNECTION_TYPE type)
{
    ...
    while(ip_buffer_data >= METADATA_HEADER_SIZE)
    {
        ...
        if (transform_metadata(data[0], type) == SYNC_WORD)
        {
            ...
            {
                SCO_DBG_ERRORMSG1("Error! Invalid metadata length  %d. ",
             ...
             */
            {
                ...
                if (wrapped_packet_len != sco_metadata->packet_length)
                {
                     ...
                     * wrap occurred. */
                    SCO_DBG_ERRORMSG3("SCO metadata record length (%d octets) "
                        ...
                        || sco_metadata->status == CRC_ERROR)
                    {
                        ...
                        SCO_DBG_ERRORMSG1("...that status was clearly bogus, "
                ...
                {
                    SCO_DBG_ERRORMSG2("Error! SCO metadata record implies "
...
stream_sco_metadata_status analyse_sco_metadata(SCO_COMMON_RCV_OP_DATA* sco_data, stream_sco_metadata *sco_metadata)
{
    ...
    if (sco_data->sco_rcv_parameters.t_esco == 0)
    {
        ...
        {
            SCO_DBG_ERRORMSG2("Warning! t_esco == 0. Packet time %d < Expected time %d. Packet late. ",
                                                time_stamp,
    ...
    /*
     * All other cases are complicated as ATn and ETn could be
     * at the edge of 16-bit number wrap. Check the difference.
     * (The difference A-B is what we need to add to B to get A:
     * if we need to add more than 0x8000, B is actually ahead.)
     */
    unsigned time_diff = (time_stamp - sco_data->sco_rcv_parameters.expected_time_stamp) & 0xFFFF;
    /*
     * If the difference is more than 0x8000, time_stamp is earlier.
     */
    if ( time_diff & 0x8000 )
    {
        /*
         * time_stamp is earlier than expected_time_stamp
         * Late if ATn < ETn - T(esco_in_ticks) /2
         * i.e. ATn - ETn + T(esco_in_ticks) /2 < 0
         */
        if ( (time_diff + sco_data->sco_rcv_parameters.t_esco) & 0x8000 )
        {
            SCO_DBG_ERRORMSG3("Warning! Packet time %d < Expected time %d - t_esco %d /2. Packet late. ",
                                                time_stamp,
                                                sco_data->sco_rcv_parameters.expected_time_stamp,
    ...
    {
        /*
         * Early if ATn > ETn + T(esco_in_ticks) /2
         * i.e. ATn - ETn > T(esco_in_ticks) /2
         */
        if ( time_diff > sco_data->sco_rcv_parameters.t_esco)
        {
            SCO_DBG_ERRORMSG3("Warning! Packet time %d > Expected time %d + t_esco %d /2. Packet early. ",
                                                time_stamp,
                                                sco_data->sco_rcv_parameters.expected_time_stamp,
                                                sco_data->sco_rcv_parameters.t_esco);
...
void discard_packet(SCO_COMMON_RCV_OP_DATA* sco_data, stream_sco_metadata* sco_metadata)
{
    ...
    SCO_DBG_ERRORMSG1("Error! Discard packet. Packet length %d.", sco_metadata->packet_length);
#ifdef SCO_RX_OP_GENERATE_METADATA
                                ...
                                CONNECTION_TYPE type)
{
    ...
    if(buff_has_metadata(ip_buffer) && input_processed != 0)
    {
        /* something has been consumed from input, remove the relevant tag */
        in_mtag = buff_metadata_remove(ip_buffer,
                        input_processed * cbuffer_get_usable_octets(ip_buffer),
                        &b4idx, &afteridx);
        if(in_mtag != NULL)
        {
            if( (IS_TIME_OF_ARRIVAL_TAG(in_mtag) || IS_TIMESTAMPED_TAG(in_mtag)) &&
               in_mtag->next == NULL &&
               b4idx == 0)
            {
                ...
                last_tag->valid = TRUE;
                last_tag->forward_ttp = IS_TIMESTAMPED_TAG(in_mtag);
    ...
    if(out_mtag != NULL)
    {
        TIME timestamp; /* timestamp for this tag */
        ...
        if(last_tag->valid)
        {
            if(last_tag->nrof_samples == 0)
            {
                ...
                SCO_DBG_MSG4("SCO_RCV_TRANSPORT_METADATA, Appending output tag, uses input tag info, toa=0x%08x(%dus in the past), sp_adjust=%d, len=%d",
                             timestamp, time_sub(current_time, timestamp), out_mtag->sp_adjust, out_mtag->length);
                /* store the the toa and current_time difference */
                last_tag->time_offset = time_sub(current_time, timestamp);
            }
            else
            {
                 ...
                 */
                timestamp = ttp_get_next_timestamp(last_tag->timestamp,
        ...
        else /* if(last_tag->valid) */
        {
             ...
             */
            timestamp = time_sub(current_time, last_tag->time_offset);
        ...
        if (last_tag->forward_ttp)
        {
            /* generate TTP Tag */
            METADATA_TIMESTAMP_SET(out_mtag, timestamp, METADATA_TIMESTAMP_LOCAL);
        }
        else if(sco_data->sco_rcv_parameters.generate_timestamp)
        {
            /* conver ToA to TTP by adding constant offset */
            TIME ts = time_add(timestamp, sco_data->sco_rcv_parameters.timestamp_latency);
        ...
        last_tag->timestamp = timestamp;
    }
    else /* if(out_mtag != NULL) */
    {
        SCO_DBG_MSG2("SCO_RCV_TRANSPORT_METADATA, Appending Null tag, len=%d, time=0x%08x", output_generated * OCTETS_PER_SAMPLE, time_get_time());
        last_tag->valid = FALSE;
    }
...
void sco_fw_check_bad_kick_threshold(SCO_COMMON_RCV_OP_DATA* sco_data)
{
    ...
    SCO_DBG_ERRORMSG1("Warning! Out of time counter incremented to %d.",
            sco_data->sco_rcv_parameters.out_of_time_pkt_cnt);
    ...
    if (sco_data->sco_rcv_parameters.out_of_time_pkt_cnt > OUT_OF_TIME_PKT_LIMIT)
    {
         ...
         * "packets_ok".
         */
        SCO_DBG_ERRORMSG("Error! Out of time reset.");
...
void sco_rcv_flush_input_buffer(SCO_COMMON_RCV_OP_DATA* sco_data)
{
    ...
    SCO_DBG_ERRORMSG1("sco rx, Flushing buffer with metadata, buf=0x%x", (unsigned)(uintptr_t)ip_buffer);
#ifdef SCO_RX_OP_GENERATE_METADATA
