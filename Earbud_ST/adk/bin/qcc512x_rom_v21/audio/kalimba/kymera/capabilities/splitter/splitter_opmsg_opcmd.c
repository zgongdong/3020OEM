...
/**
 * \brief Common part of setting running streams.
 *
 * \param op_data Pointer to the operator instance data
 * \param streams Streams to set:
    ...
    if (splitter->working_mode == CLONE_BUFFER)
    {
        ...
        if ((streams & (1<<new_out_index)) != 0)
        {
            while (channel)
            {
                ...
                if (align_buffs && new_out_buffer != NULL && cur_out_buffer != NULL)
                {
            ...
            set_current_output_state(splitter, new_out_index, ACTIVE);
        }
        else
        {
...
bool splitter_create(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    splitter->offload_enabled = opmgr_op_thread_offload(op_data) && audio_thread_offload_is_active();
#endif
