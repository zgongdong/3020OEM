...
MAP_INSTANCE_DATA(CAP_ID_DBE, DBE_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_DBE, DBE_OP_DATA)
...
#if !defined(CAPABILITY_DOWNLOAD_BUILD)
MAP_INSTANCE_DATA(CAP_ID_DBE_FULLBAND, DBE_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_DBE_FULLBAND, DBE_OP_DATA)
#endif /* CAPABILITY_DOWNLOAD_BUILD */
...
#if !defined(CAPABILITY_DOWNLOAD_BUILD)
MAP_INSTANCE_DATA(CAP_ID_DBE_FULLBAND_BASSOUT, DBE_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_DBE_FULLBAND_BASSOUT, DBE_OP_DATA)
#endif /* CAPABILITY_DOWNLOAD_BUILD */

/****************************************************************************
Private Function Declarations
*/
static inline DBE_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (DBE_OP_DATA *) base_op_get_instance_data(op_data);
}
...
{
    DBE_OP_DATA *p_ext_data = get_instance_data(op_data);
    ...
    switch (base_op_get_cap_id(op_data))
    {
       ...
       case DBE_FULLBAND_BASSOUTCAP_ID:
          ...
          break;
       default:
          /* Should never happen but required by the compiler. */
          return FALSE;
    }

    if(!cpsInitParameters(&p_ext_data->parms_def,(unsigned*)DBE_GetDefaults(base_op_get_cap_id(op_data)),(unsigned*)&p_ext_data->dbe_cap_params,sizeof(DBE_PARAMETERS)))
    {
       base_op_change_response_status(response_data, STATUS_CMD_FAILED);
       DBE_CAP_Destroy(op_data);
       return TRUE;
...
{
    DBE_OP_DATA        *op_extra_data = get_instance_data(op_data);
...
bool dbe_wrapper_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if(resp)
    {
        ...
        resp = cpsPack2Words(comp_config, op_state_stat, resp);
        cpsPack1Word(op_state_stat, resp);
    }
                 ...
                 uint16 length, unsigned* data, STATUS_KYMERA status,uint16 extra_status_info)
{
    ...
    DBE_OP_DATA *op_extra_data = get_instance_data((OPERATOR_DATA*)instance_data);
