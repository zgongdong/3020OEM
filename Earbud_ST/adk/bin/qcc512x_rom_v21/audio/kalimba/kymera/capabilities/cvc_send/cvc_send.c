...
void cvc_send_set_requested_features(CVC_SEND_OP_DATA *op_extra_data)
{
    ...
    for (i=1; i<=op_extra_data->num_mics; i++) {
        if(!op_extra_data->input_stream[i]->cbuffer) {
            return;
        }
    }
    ...
    if(num_va_outputs)
    {
        ...
        for (i=1; i<=num_va_outputs; i++) {
            if(!op_extra_data->output_stream[i]->cbuffer) {
                op_extra_data->op_feature_requested = 0;
                break;
            }
        }
    ...
    if(op_extra_data->output_stream[0]->cbuffer)
    {
    ...
    if(op_extra_data->input_stream[0]->cbuffer)
    {
...
bool cvc_send_connect(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    {
        op_extra_data->output_stream[terminal_id&CVC_SEND_NUM_OUTPUTS_MASK]->cbuffer = pterminal_buf;
    }
...
bool cvc_send_disconnect(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    if (terminal_id & TERMINAL_SINK_MASK)
    {
        ...
        {
            tCbuffer *this_buf = op_extra_data->input_stream[terminal_id&CVC_SEND_NUM_INPUTS_MASK]->cbuffer;
            if(this_buf == op_extra_data->mic_metadata_buffer)
            {
                ...
                {
                    tCbuffer *mic_buffer = op_extra_data->input_stream[mic_idx]->cbuffer;
    ...
    {
        op_extra_data->output_stream[terminal_id&CVC_SEND_NUM_OUTPUTS_MASK]->cbuffer = NULL;
    }
