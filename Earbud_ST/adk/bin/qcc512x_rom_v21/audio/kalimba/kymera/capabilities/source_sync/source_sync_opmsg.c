...
bool src_sync_opmsg_obpm_get_status(OPERATOR_DATA *op_data, void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (resp != NULL)
    {
        ...
        resp = cpsPack2Words(Cur_mode, comp_config, resp);
        resp = cpsPack2Words(op_extra_data->Dirty_flag, op_extra_data->stat_sink_stalled, resp);
        cpsPack1Word(op_extra_data->stat_sink_stall_occurred, resp);
        ...
        unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    else
    {
        ...
        {
            if (sink_grp->common.channel_mask == 0)
            {
            ...
            else
            {
                ...
                {
                    rate_adjust_enabled_mask |= sink_grp->common.channel_mask;
                }
                sink_grp->common.sample_rate = op_extra_data->default_sample_rate;
                ...
                SOSY_MSG4( SRC_SYNC_TRACE_ALWAYS, "0x%04x set_sink_groups sink_grp%d pr %d ra %d",
                           op_extra_data->id, sink_grp->common.idx, sink_grp->purge,
                           sink_grp->rate_adjust_enable);
            }
        }
