...
bool secure_basic_passthrough_create(OPERATOR_DATA *op_data, void *message_data, unsigned *response_id, void **response_data)
{
    ...
    if(!cpsInitParameters(&opx_data->parms_def,SECURE_BASIC_PASSTHROUGH_GetDefaults(cap_id),(unsigned*)(&opx_data->params),sizeof(SECURE_BASIC_PASSTHROUGH_PARAMETERS)))
    {
        base_op_change_response_status(response_data, STATUS_CMD_FAILED);
        return TRUE;
    }
    ...
    if (!basic_passthrough_retrieve_license_req(op_data,
...
bool secure_basic_passthrough_opmsg_obpm_get_status(OPERATOR_DATA *op_data,
                                                    void *message_data,
                                                    ...
                                                    OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (!common_obpm_status_helper(message_data, resp_length, resp_data,
                                   sizeof(SECURE_BASIC_PASSTHROUGH_STATISTICS),
                                   &resp))
    {
         return FALSE;
    }
                                            ...
                                            unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
     ...
     * Audio = apply a gain.
     */
    if ((SECURE_BASIC_PASS_CAP_ID == cap_id) || (SECURE_TTP_PASS_CAP_ID == cap_id))
    {
