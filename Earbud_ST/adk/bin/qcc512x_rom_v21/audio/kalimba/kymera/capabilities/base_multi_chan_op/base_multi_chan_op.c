...
bool multi_channel_stop_reset(OPERATOR_DATA *op_data,void *message_data, unsigned *response_id, void **response_data)
{
    ...
    if ((chan_def->config & MULTI_OFFLOAD_FLAG) != 0)
    {
        ...
        /* Stop any timer-based kicks */
        timer_cancel_event_atomic(&chan_def->self_kick_timer);
...
/* 
 * Handler for self-kick timer.
 * Kicks the operator so it runs again
 */
static void self_kick_timer_task(void *timer_data)
{
    OPERATOR_DATA *op_data = (OPERATOR_DATA*) timer_data;
    MULTI_CHANNEL_DEF *chan_def = get_class_data(op_data);

    chan_def->self_kick_timer = TIMER_ID_INVALID;
...
/* 
 * Set the self-kick timer.
 */
void multi_chan_set_self_kick(OPERATOR_DATA *op_data)
{
    MULTI_CHANNEL_DEF *chan_def = get_class_data(op_data);

    timer_schedule_event_in_atomic(SELF_KICK_TIME,
        self_kick_timer_task, (void*)op_data, &chan_def->self_kick_timer);
}

/* 
 * Cancel the self-kick timer.
 */
void multi_chan_cancel_self_kick(OPERATOR_DATA *op_data)
{
    MULTI_CHANNEL_DEF *chan_def = get_class_data(op_data);
    timer_cancel_event_atomic(&chan_def->self_kick_timer);
}
