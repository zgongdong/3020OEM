...
/* Configuration Flags */
#define MULTI_INPLACE_FLAG    0x01
#define MULTI_HOT_CONN_FLAG   0x02
#define MULTI_STREAM_FLAG     0x04
#define MULTI_METADATA_FLAG   0x08
#define MULTI_OFFLOAD_FLAG    0x10
...
typedef struct
{
    ...
    /** Input: Offset that will be added to
     *  output tag timestamp values (TS and TOA only)
     */
    TIME_INTERVAL timestamp_phase_correction;
} multi_chan_ttp_last_tag;

#if defined(BASE_MULTI_CHAN_OFFLOAD) && defined(CAPABILITY_DOWNLOAD_BUILD)
#define multi_chan_channel_struc offload_multi_chan_channel_struc
#endif
...
#ifdef BASE_MULTI_CHAN_OFFLOAD
    tCbuffer *connection_input;                      /* cloned input buffer*/
    tCbuffer *connection_output;                     /* cloned output buffer*/
#endif
...
#ifdef BASE_MULTI_CHAN_OFFLOAD
    tTimerId self_kick_timer;             /** Timer for scheduling self-kick */
    TOUCHED_TERMINALS offload_touched;    /** Cached terminal state */
#endif
...
#if defined(BASE_MULTI_CHAN_OFFLOAD) && defined(CAPABILITY_DOWNLOAD_BUILD)
/* Redirect function names so they don't conflict with existing versions */

#define multi_channel_create                        offload_multi_channel_create
#define multi_channel_detroy                        offload_multi_channel_detroy
#define multi_channel_connect                       offload_multi_channel_connect
#define multi_channel_disconnect                    offload_multi_channel_disconnect
#define multi_channel_sched_info                    offload_multi_channel_sched_info
#define multi_channel_buffer_details                offload_multi_channel_buffer_details
#define multi_channel_start                         offload_multi_channel_start
#define multi_channel_stop_reset                    offload_multi_channel_stop_reset
#define multi_channel_stream_based                  offload_multi_channel_stream_based
#define multi_channel_opmsg_set_buffer_size         offload_multi_channel_opmsg_set_buffer_size
#define multi_channel_check_buffers_adjusted        offload_multi_channel_check_buffers_adjusted
#define multi_channel_check_buffers                 offload_multi_channel_check_buffers
#define multi_channel_set_callbacks                 offload_multi_channel_set_callbacks
#define multi_channel_set_buffer_size               offload_multi_channel_set_buffer_size
#define multi_channel_set_block_size                offload_multi_channel_set_block_size
#define multi_channel_copy_mute                     offload_multi_channel_copy_mute
#define multi_channel_advance_buffers               offload_multi_channel_advance_buffers
#define multi_channel_get_channel_count             offload_multi_channel_get_channel_count
#define multi_channel_get_channel_def               offload_multi_channel_get_channel_def
#define multi_channel_active_channels               offload_multi_channel_active_channels
#define multi_channel_first_active_channel          offload_multi_channel_first_active_channel
#define multi_channel_metadata_propagate            offload_multi_channel_metadata_propagate
#define multi_channel_metadata_transfer             offload_multi_channel_metadata_transfer
#define multi_channel_metadata_limit_consumption    offload_multi_channel_metadata_limit_consumption
#endif
...
/**
 * \brief  Handle Stop / reset operator
 *
 * \param  op_data           Pointer to operator structure
 * \param  message_data      Pointer to message payload
 * \param  response_id       Pointer to response message ID
...
/**
 * \brief Synchronise clone buffers
 *
 * \param  op_data  Pointer to operator structure
 *
 * \return  number of samples of data processed
 */
extern unsigned multi_chan_sync_buffers(OPERATOR_DATA *op_data);

/**
 * \brief Update touched terminal info from pipelined run
 *
 * \param  op_data  Pointer to operator structure
 * \param  touched  Pointer to terminal info
 */
extern void multi_chan_update_touched_terminals(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched);

/**
 * \brief Set self-kick timer to reschedule operator
 *
 * \param  op_data  Pointer to operator structure
 */
extern void multi_chan_set_self_kick(OPERATOR_DATA *op_data);

/**
 * \brief Cancel self-kick timer to reschedule operator
 *
 * \param  op_data  Pointer to operator structure
 */
extern void multi_chan_cancel_self_kick(OPERATOR_DATA *op_data);

/**
 * \brief Query the offload status
 *
 * \param  op_data  Pointer to operator structure
 */
extern bool multi_chan_offload_active(OPERATOR_DATA *op_data);
