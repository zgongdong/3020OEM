...
static bool timestamped_close (ENDPOINT *ep, bool *pending);
...
#ifdef BAC32
    params->data_buf_flags = MMU_UNPACKED_16BIT_MASK;
#else
...
void stream_create_timestamped(const AUDIO_DATA_SERVICE_EP_CFG *cfg, AUDIO_DATA_SERVICE_EP_CREATE_RESULT* create_res)
{
    ...
    timestamped->usable_octets = NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD);
...
static bool timestamped_connect(ENDPOINT *ep, tCbuffer *cb,  ENDPOINT *ep_to_kick, bool* start_on_connect)
{
    ...
    if((timestamped->data_format == AUDIO_DATA_FORMAT_16_BIT) ||
        (timestamped->data_format == AUDIO_DATA_FORMAT_16_BIT_WITH_METADATA))
    {
        cbuffer_set_usable_octets(cb, NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD));
    }
    else if(timestamped->data_format == AUDIO_DATA_FORMAT_ENCODED_DATA)
    {
        cbuffer_set_usable_octets(cb, ENCODED_DATA_OCTETS_IN_WORD);
    }
...
static void timestamped_internal_kick(void *priv)
{
    ...
    audio_data_service_timestamped_kick(priv, STREAM_KICK_INTERNAL);
}
...
static bool timestamped_source_buffer_align(ENDPOINT *ep)
{
    ...
    data_in_octets = cbuffer_calc_amount_data_ex(cb);
    ...
    if (BUFF_METADATA(cb))
    {
        ...
        if (tag != NULL)
        {
            ...
            if (octet_advance <= data_in_octets)
            {
                ...
                cbuffer_advance_read_ptr_ex(cb, octet_advance);
...
static void timestamped_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    if (kick_dir != STREAM_KICK_INTERNAL)
    {
        if (ep->deferred.kick_is_deferred)
        {
            if (data_available > 0)
            {
                ep->deferred.kick_dir = STREAM_KICK_INTERNAL;
                raise_bg_int(ep->deferred.bg_task);
            }
        }
        else
    ...
    if (SOURCE == ep->direction)
    {
        if (BUFF_METADATA(timestamped->source_buf))
        {
            ...
            /* audio_data_service_queue_meta updates the slave buffer pointers
             * so recalculate the amount of data available after that's happened
             */
            data_available = cbuffer_calc_amount_data_ex(timestamped->source_buf);
        }
    }
    ...
    if (BUFF_METADATA(timestamped->source_buf))
    {
        ...
        if (SINK == ep->direction)
        {
            if (!timestamped->aligned)
            {
                ...
                data_available = cbuffer_calc_amount_data_ex(timestamped->source_buf);
                /* metadata available octets is limited within meta_complete_tag_octets 
                 * anyway, so do not recompute */
            }
    ...
    if ((ep->connected_to != NULL) && (copy_octets > 0))
    {
        ...
        copy_octets = cbuffer_copy_ex(timestamped->sink_buf, timestamped->source_buf, copy_octets);
    }
    ...
    if (copy_octets != 0)
    {
        ...
        if (SINK == ep->direction)
        {
            ...
            {
                audio_data_service_dequeue_meta(timestamped->service_priv, copy_octets, FALSE);
            }
        }
...
static bool timestamped_buffer_details(ENDPOINT *ep, BUFFER_DETAILS *details)
{
    ...
    /* This buffer size will end up double the value if the endpoint is resolved
     * to be dealing with 32-bit data because ep->state.file.usable_octets is
     * initialised to 2. The value is changed to the correct one in configure().*/
    details->b.buff_params.size = DATA_BUFFER_SIZE_BYTES / \
                ep->state.timestamped.usable_octets;
...
static bool timestamped_configure(ENDPOINT *ep, unsigned int key, uint32 value)
{
    switch(key)
    {
        case EP_DATA_FORMAT:
        {
            ...
            {
                ep->state.timestamped.data_format = format;
                ep->state.timestamped.usable_octets = \
                        NR_OF_OCTETS_IN_WORD(USE_16BIT_PER_WORD);
                return TRUE;
            }
            else if(format == AUDIO_DATA_FORMAT_ENCODED_DATA)
            {
                ep->state.timestamped.data_format = format;
                ep->state.timestamped.usable_octets = \
                        ENCODED_DATA_OCTETS_IN_WORD;
                return TRUE;
            }
...
static bool timestamped_get_config(ENDPOINT *ep, unsigned int key, ENDPOINT_GET_CONFIG_RESULT* result)
{
    ...
    {
        case EP_DATA_FORMAT:
            result->u.value = ep->state.timestamped.data_format;
            return TRUE;
        case EP_RATEMATCH_ABILITY:
            if(SOURCE == ep->direction)
            {
                /* Source rate is driven by incoming data, no rate adjustment
                 * possible. This would be RATEMATCHING_SUPPORT_NONE, but there
                 * are scenarios where ratemathcing is done elsewhere.
                 * SCO+MIC forwarding (earbuds) is an example of this scenario,
                 * because SCO and MIC data are rate adjusted on the peer.
                 */
                result->u.value = (uint32)RATEMATCHING_SUPPORT_AUTO;
            }
            else
            {
                /* Sink can transmit at the rate it receives data from connected
                 * graph (assuming it has enough bandwidth) so no rate matching
                 * action is required.
                 */
                result->u.value = (uint32)RATEMATCHING_SUPPORT_AUTO;
            }
...
static void timestamped_get_timing(ENDPOINT *ep, ENDPOINT_TIMING_INFORMATION *time_info)
{
    ...
    /* Naturally this EP would be volatile, we deliberately chose to say it's not
       so that operators connecting here that can work in_place
       are allowed to do so, avoiding wasting memory for large buffers;
       a known use case: TWS slave / Broadcast receiver on Stre where
       we connect a passthrough immediately after the timestamped source endpoint,
       to provide somewhere for the app to configure a latency buffer size.
     */
