 ...
 */
void cbuffer_buffer_configure(tCbuffer *cbuffer, int *buffer,
                              ...
                              unsigned int descriptor)
{
    ...
    /* buffer module internal sizes in memory allocation units */
    cbuffer->size = buffer_size*sizeof(int);
    ...
    if (BUFF_METADATA(cbuffer))
    {
        ...
        usable_octets = cbuffer_get_usable_octets( cbuffer);
        /* up to 4 octets (32-bit words) */
        PL_ASSERT(usable_octets <= 4);
        /* adjust the cached buffer_size */
        buff_metadata_set_buffer_size(cbuffer, usable_octets);
    }
...
void cbuffer_set_usable_octets(tCbuffer *buff, unsigned usable_octets)
{
    ...
    if (usable_octets >= 4)
    {
        ...
        {
            fault_diatribe(FAULT_AUDIO_USABLE_OCTETS_INVALID, usable_octets);
        }
        /* usable_octets is a 2-bit value in buff->descriptor, allowing up to 4 octets (32-bit words) */
        usable_octets = 4;          /* for 32-bit platforms */
    }
    buff->descriptor = BUF_DESC_USABLE_OCTETS_CLEAR(buff->descriptor);
    buff->descriptor |= (usable_octets>=4 ? 0 : usable_octets) << BUF_DESC_USABLE_OCTETS_SHIFT;

#ifdef INSTALL_METADATA
    /* need to adjust the metadata cached buffer_size */
    buff_metadata_set_buffer_size( buff, usable_octets);
#endif
}

unsigned cbuffer_get_usable_octets(tCbuffer *buff)
{
    /* silently return the default for invalid arguments */
    unsigned usable_octets = 4;

    if (buff != NULL)
    {
        usable_octets = ((buff->descriptor) & BUF_DESC_USABLE_OCTETS_MASK)>> BUF_DESC_USABLE_OCTETS_SHIFT;
        usable_octets = (usable_octets == 0) ? 4 : usable_octets;
    }
...
/**
 * Get read offset of a software buffer.
 *
...
/**
 * Get write offset of a software buffer.
 *
