...
#endif /* defined(INSTALL_DUAL_CORE_SUPPORT) */
...
#if defined(CHIP_STRE)
#define PM_P1_HEAP_START_EXT_ADDR       (void_func_ptr)0x04012800
/*
 * With Direct Cache, cache size is 0x800 on PM BANK9. This
 * means that (0x2000-0x800=)0x1800 is unused PM memory, as
 * it is non-contiguous with the other 9 PM BANKs 0..8.
 */
#define PM_P1_RAM_SIZE_EXTRA            0x1800
#endif
...
void init_heap_pm(void)
{
    ...
    if (audio_thread_offload_is_configured())
    {
         ...
         */
        heap_pm_end = HEAP_NODE(PM_RAM_END_ADDRESS-PM_BANK_SIZE);
        (void)init_and_adjust_pm_heap(PM_P1_RAM_SIZE_EXTRA, PM_P1_HEAP_START_EXT_ADDR, NULL);
        heap_pm_start = init_and_adjust_pm_heap(PM_HEAP_SIZE, heap_pm_start, PM_P1_HEAP_START_EXT_ADDR);
        freelist_pm = heap_pm_start;
        L2_DBG_MSG4("Direct cache: PM heap: start 0x%x end 0x%x, and 0x%x to 0x%x", 
                    heap_pm_start, heap_pm_end, PM_P1_RAM_SIZE_EXTRA, HEAP_NODE(PM_RAM_END_ADDRESS));
...
#else /* not CHIP_STRE */
    #error Thread Offload only implemented for CHIP_STRE at time of writing
#endif /* CHIP_STRE */
