...
{
    return (IPC_PRIMARY_CONTEXT() ?
             opmgr_get_ops_count_in_oplist(capid, remote_oplist_head) : 0);
}


unsigned int opmgr_get_list_remote_ops_count(unsigned int num_ops, unsigned int *op_list, OP_PROCESSOR_ID proc_id)
{
    unsigned int i, id;
    unsigned int n = 0;
    OPERATOR_DATA* entry;
 ...
 *
 */
bool opmgr_get_processor_id_from_opid(unsigned ext_opid, OP_PROCESSOR_ID *processor_id)
{
...
bool opmgr_get_processor_id(unsigned ep_id, OP_PROCESSOR_ID *processor_id)
{
...
bool opmgr_destroy_op_endpoints(OPERATOR_DATA* op_data)
{
    ...
    if (OPMGR_ON_SAME_CORE(op_data->processor_id))
    {
#endif
...
OPERATOR_DATA* get_anycore_op_data_from_id(unsigned int id)
{
    ...
    if (IPC_PRIMARY_CONTEXT() && (entry == NULL))
    {
        entry = find_op_data_in_list(id, remote_oplist_head);
    }
#endif /* INSTALL_DUAL_CORE_SUPPORT */
...
{
    return (IPC_PRIMARY_CONTEXT() ? find_op_data_in_list(id, remote_oplist_head) : NULL);
}
#endif /* INSTALL_DUAL_CORE_SUPPORT */
