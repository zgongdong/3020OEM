...
static void record_fault(faultid f, DIATRIBE_TYPE arg, bool freshen)
{
    ...
    this_preserved->fault.last_arg = arg;
#endif
...
#define ARGS_ARRAY_LENGTH ((DIATRIBE_TYPE_BIT+15)/16)
    uint16 args16[ARGS_ARRAY_LENGTH];
    uint16f i;
    DIATRIBE_TYPE arg;
#if FAULT_TIMESTAMP_WIDTH == 32
    t = fi->st;
    ...
    if (!IPC_PRIMARY_CONTEXT())
    {
         ...
         * the fault.
         */
        IPC_PROCESSOR_ID_NUM local_proc;
        uint16 con_id;
        ADAPTOR_MSGID msg_id;
        
        local_proc = ipc_get_processor_id();
        con_id = PACK_CON_ID(PACK_SEND_RECV_ID(IPC_PROCESSOR_0,0),
                             PACK_SEND_RECV_ID(local_proc,0));

        KIP_MSG_PUBLISH_FAULT_REQ* req_msg = xpmalloc(sizeof(KIP_MSG_PUBLISH_FAULT_REQ)
                                                      + (ARGS_ARRAY_LENGTH * sizeof(uint16)));

        KIP_MSG_PUBLISH_FAULT_REQ_PACK(req_msg, con_id, (uint16)fi->f, local_proc,
                                       (uint16)fi->n, t, ARGS_ARRAY_LENGTH, 0);

        memcpy(req_msg->_data + KIP_MSG_PUBLISH_FAULT_REQ_ARGS_WORD_OFFSET,
               args16, ARGS_ARRAY_LENGTH * sizeof(uint16));


        msg_id = KIP_MSG_ID_TO_ADAPTOR_MSGID(KIP_MSG_ID_PUBLISH_FAULT_REQ);
        adaptor_send_message(con_id, msg_id,
                             KIP_MSG_PUBLISH_FAULT_REQ_WORD_SIZE,
...
#endif
    {
        subreport_event(CCP_EVENT_LEVEL_FAULT, (uint16)fi->f, P0,
                        (uint16)fi->n, t, args16, ARGS_ARRAY_LENGTH);
    }
