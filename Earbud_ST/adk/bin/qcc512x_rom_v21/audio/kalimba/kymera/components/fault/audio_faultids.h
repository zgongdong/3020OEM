 ...
 */
    ...
    /**
     * Unused fault id 0x0046. Please don't reuse this id.
     *
     * Previously used for:
     ...
     */
    FAULT_AUDIO_USABLE_OCTETS_INVALID = 0x0051,
    ...
    /**
     *  0x005e is reserved. It was FAULT_AUDIO_METADATA_KIP_BUFFER_NOT_ENOUGH_SPACE
     *  but is now unused. However, FAULT_AUDIO_METADATA_KIP_BUFFER_NOT_ENOUGH_SPACE
     *  is present in previous kymera ROMs so it's not a good idea to recycle it.
     */
    ...
    /**
     * Invalid PM heap allocation specified
     */
     FAULT_AUDIO_INVALID_PM_HEAP_ALLOCATION = 0x0061,

    /**
     * Change in SCO wallclock looks implausible
     */
     FAULT_AUDIO_SCO_WALLCLOCK_UNEXPECTED_CHANGE = 0x0062,

    /**
     * Generic Apps Direct Flash access fault, the diatribe will give more detail
     */
     FAULT_AUDIO_APPS_DIRECT_FLASH_ACCESS = 0x0063,

    /**
     * Switched Passthrough Consumer is handling only complete tags of data.
     * It found a tag that is bigger than either the input or the output buffer.
     * Argument is the tag size. The user should increase buffer sizes.
     */
     FAULT_AUDIO_SPC_TAG_BIGGER_THAN_BUFFER = 0x0064,

     /**
      * VA Graph Manager received a message from an unrecognized operator.
      * Operator id in argument
      */
     FAULT_AUDIO_VAGM_UNRECOGNIZED_OPERATOR = 0x0065,

     /**
      * VA Graph Manager received an error as response to an operator configuration.
      * Operator id in argument
      */
     FAULT_AUDIO_VAGM_ERROR_MESSAGE = 0x0066,

     /**
      * VA Graph Manager sent a request to the AOV Client but this was not present.
      * Requested clock in argument
      */
     FAULT_AUDIO_VAGM_AOV_NOT_PRESENT = 0x0067,

    /**
     * A timeout occured while trying to change the CPU clock frequency.
     * The performance of the subsystem will be degraded.
     * The requested clock frequency is specified in argument.
     */
    FAULT_AUDIO_CLOCK_MANAGER_TIMEOUT = 0x0068,

    /**
     * The clock manager is not able to monitor the temperature
     * of the chip anymore and can therefore not compensate for it anymore.
     * This might cause the subsystem's clocks to drift leading to measurable
     * audio quality issues.
     */
    FAULT_AUDIO_CLOCK_MANAGER_MONITORING_FAILED = 0x0069,
