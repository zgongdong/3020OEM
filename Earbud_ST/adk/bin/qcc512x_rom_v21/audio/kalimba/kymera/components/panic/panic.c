...
#define report_panic(deathbed_confession, last_arg) \
            do { comms_send_panic(); UNUSED(last_arg); } while(0)
#else
...
void panic_diatribe(panicid deathbed_confession, DIATRIBE_TYPE diatribe)
{
    ...
    if (ipc_get_processor_id() == IPC_PROCESSOR_0)
    {
        ...
        for (proc = IPC_PROCESSOR_1; proc < IPC_PROCESSOR_MAX; proc++)
        {
...
void panic_remote_proc(uint16 remote_proc_id, uint16 data_length, void * panic_data)
{
#ifdef INSTALL_HYDRA
    volatile panic_preserved * preserved_data = (volatile panic_preserved *)panic_data;
    uint16 arg = (uint16)preserved_data->last_arg;
    if (ipc_get_processor_id() == IPC_PROCESSOR_0)
    {
...
#endif
        panic_diatribe(PANIC_AUDIO_PX_PANICKED, remote_proc_id);
}
#endif
