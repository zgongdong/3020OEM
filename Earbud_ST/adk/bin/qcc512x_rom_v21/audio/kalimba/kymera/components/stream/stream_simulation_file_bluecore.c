...
****************************************************************************/
/**
 * \addtogroup File Simulation File endpoint
 * \ingroup endpoints
 * \file  stream_simulation_bluecore.c
 *
 * stream file type file. <br>
 * This file contains stream functions for bluecore simulation file endpoints. <br>
 *
 */
...
#define PARAM_BIT_MODE    1

enum endpoint_bit_mode {
BIT16_MODE = 0,
BIT24_MODE };

#define SIM_FILE_BUFFER_MIN_SIZE 256

#define SIM_FILE_KICK_PERIOD    (2 * MILLISECOND)
...
*/
static bool sim_file_close (ENDPOINT *ep, bool *pending);
static bool sim_file_connect (ENDPOINT *ep, tCbuffer *Cbuffer_ptr, ENDPOINT *ep_to_kick, bool* start_on_connect);
static bool sim_file_disconnect (ENDPOINT *ep);
static bool sim_file_buffer_details (ENDPOINT *ep, BUFFER_DETAILS *details);
static void sim_file_sched_kick (ENDPOINT *ep, KICK_OBJECT *ko);
static void sim_file_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir);
static bool sim_file_start (ENDPOINT *ep, KICK_OBJECT *ko);
static bool sim_file_stop (ENDPOINT *ep);
static bool sim_file_configure (ENDPOINT *ep, unsigned int key, uint32 value);
static bool sim_file_get_config (ENDPOINT *ep, unsigned int key, ENDPOINT_GET_CONFIG_RESULT* result);
static void sim_file_get_timing (ENDPOINT *ep, ENDPOINT_TIMING_INFORMATION *time_info);
static bool sim_file_set_data_format (ENDPOINT *ep, AUDIO_DATA_FORMAT format);

DEFINE_ENDPOINT_FUNCTIONS(sim_file_functions, sim_file_close, sim_file_connect,
                          sim_file_disconnect, sim_file_buffer_details,
                          sim_file_kick, sim_file_sched_kick,
                          sim_file_start, sim_file_stop,
                          sim_file_configure, sim_file_get_config,
                          sim_file_get_timing, stream_sync_sids_dummy,
                                    ...
                                    unsigned *params)
{
    ...
    unsigned port, key;
    //unsigned bit_mode;
    enum endpoint_bit_mode bit_mode;
    ENDPOINT *ep;
    ...
    bit_mode = params[PARAM_BIT_MODE];

    ep = stream_get_endpoint_from_key_and_functions(key, dir,
                                                  &endpoint_sim_file_functions);
    if (!ep)
    {
        if ((ep = STREAM_NEW_ENDPOINT(sim_file/*file*/, key, dir, con_id)) == NULL)
        {
            return NULL;
        }
        ...
        if(SOURCE == dir)
        {
            ...
            if (bit_mode == BIT16_MODE)
            {
            	cbuffer_reconfigure_read_port(port, BITMODE_16BIT);   // (sign extension), 16-bit

            }
            else if (bit_mode == BIT24_MODE)
            {
                cbuffer_reconfigure_read_port(port, BITMODE_24BIT);           // test for 24 bit mode
            }
            else
            {
            	return NULL;
        ...
        else
        {
            ...
            if (bit_mode == BIT16_MODE)
            {
                cbuffer_reconfigure_write_port(port, BITMODE_16BIT);   // (sign extension), 16-bit

             }
            else if (bit_mode == BIT24_MODE)
            {
                cbuffer_reconfigure_write_port(port, BITMODE_24BIT);           // test for 24 bit mode
             }
            else
            {
                return NULL;
        ...
        ep->state.file.kick_period = SIM_FILE_KICK_PERIOD;
        ...
        if (bit_mode == BIT16_MODE)
            ...
            {
               	if ((ep->cbops = cbops_mgr_create(ep->direction, CBOPS_COPY_ONLY)) == NULL)
               	{
                   stream_destroy_endpoint(ep);
                   return NULL;
                }
...
static bool sim_file_buffer_details(ENDPOINT *ep, BUFFER_DETAILS *details)
{
    ...
    details->b.buff_params.size = SIM_FILE_BUFFER_MIN_SIZE;
...
static bool sim_file_configure(ENDPOINT *ep, unsigned int key, uint32 value)
{
    ...
    {
    case EP_DATA_FORMAT:
        return sim_file_set_data_format(ep, (AUDIO_DATA_FORMAT)value);
 ...
 */
static AUDIO_DATA_FORMAT sim_file_get_data_format (ENDPOINT *endpoint)
{
...
static bool sim_file_get_config(ENDPOINT *ep, unsigned int key, ENDPOINT_GET_CONFIG_RESULT* result)
{
    ...
    {
    case EP_DATA_FORMAT:
        result->u.value = (uint32)sim_file_get_data_format(ep);
        return TRUE;
