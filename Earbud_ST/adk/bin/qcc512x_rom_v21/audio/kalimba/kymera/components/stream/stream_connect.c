                                ...
                                STREAM_CONNECT_INFO* state_info)
{
    ...
    {
#ifdef STREAM_CONNECT_FAULT_CODE
        L1_DBG_MSG("stream_connect_get_buffer: buffer_details() failed.");
#endif /* STREAM_CONNECT_FAULT_CODE */
        return FALSE;
    }
...
bool stream_disconnect_endpoint_transform( ENDPOINT *endpoint)
{
    ...
    if (endpoint->connected_to)
    {
        ...
        if(STREAM_EP_IS_SHADOW(endpoint->connected_to))
        {
             ...
             * the processor id
             */
            proc_id = IPC_PRIMARY_CONTEXT()? IPC_PROCESSOR_1:
                                             IPC_PROCESSOR_0;
        ...
        if ((IPC_SECONDARY_CONTEXT()) && (tr_id_found))
        {
...
bool stream_transform_disconnect(TRANSFORM *transform)
{
    ...
    if (!transform->sink->functions->disconnect(transform->sink))
    {
        ...
        if (transform->source->is_real)
            {
                ...
                if (restart_src)
                {
                    ko = kick_obj_from_sched_endpoint(stream_get_head_of_sync(transform->source));
                    if (ko != NULL)
                    {
        ...
        if (transform->sink->is_real)
        {
            ...
            if(restart_sink)
            {
                ko = kick_obj_from_sched_endpoint(stream_get_head_of_sync(transform->sink));
                if (ko != NULL)
                {
...
#ifdef AVOID_DEEP_CALL_STACK
    /* Try to mitigate a dangerously deep call stack situation by blocking interrupts while cease_ratematching (B-270105) */
    interrupt_block();
#endif
                                     ...
                                     unsigned transform_id)
{
    ...
    {
#ifdef STREAM_CONNECT_FAULT_CODE
        L1_DBG_MSG("connect_transform: buffer_details() failed.");
#endif /* STREAM_CONNECT_FAULT_CODE */
