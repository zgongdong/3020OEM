                                              ...
                                              tCbuffer **sink_cbuffer)
{
    ...
    *sink_cbuffer = NULL;
    ...
    else
    {
        ...
        shift = 16;
#else
    /* Amber had BAC24 and flags = 0; shift = 8; but it's no longer supported */
#error "SCO buffer flags and shift undefined for this BAC"
    ...
    else
    {
        ...
        shift = 16;
#else
    /* Amber had BAC24 and flags = 0; shift = 8;
        TODO although that was probably wrong, see B-268112
...
static void sco_rx_generate_metadata(ENDPOINT *endpoint)
{
    ...
    if(new_words_received != 0)
    {
        ...
        while(new_words_received > 0)
        {
            ...
            b4idx = 0;
            afteridx = tag_size*cbuffer_get_usable_octets(sco->cbuffer);
            ...
            {
                STREAM_METADATA_DBG_MSG1("SCO_RX_GENERATE_METADATA, NULL TAG ADDED, time=0x%08x", time_get_time());
            }
    ...
    {
        /* no new packets received, log some debug info */
        sco->late_arrival_counter++;
        sco->last_late_arrival_time = time_get_time();
        STREAM_METADATA_DBG_MSG2("SCO_RX_GENERATE_METADATA, No new packet observed: packet_offset=%d, time=0x%08x",
                                 sco->packet_offset, time_get_time());
    }
