...
#ifdef INSTALL_DUAL_CORE_SUPPORT
#define INVALID_PROCESSOR_ID(pid) ipc_invalid_processor_id((IPC_PROCESSOR_ID_NUM)pid)
#else
        ...
        unsigned int *op_list, OP_STD_LIST_CBACK callback, OPCMD_ID msg_id, preproc_function preproc_func)
{
    ...
    else
    {
        ...
        if ((GET_CON_ID_SEND_ID(cur_op->con_id) != GET_CON_ID_SEND_ID(con_id)) &&
            (GET_SEND_RECV_ID_CLIENT_ID(GET_CON_ID_SEND_ID(con_id)) != GET_SEND_RECV_ID_CLIENT_ID(OPMGR_ADAPTOR_OBPM)))
#else
        if ((cur_op->con_id != con_id) && (con_id != OPMGR_ADAPTOR_OBPM))
...
static void opmgr_create_operator_post_dnld(unsigned int con_id, CAP_ID cap_id,
        ...
        OP_PROCESSOR_ID processor_id, bool offload_enabled,
        OP_CREATE_CBACK callback)
{
    tRoutingInfo new_rinfo;
     ...
     */
    if ((IPC_PRIMARY_CONTEXT() && INVALID_PROCESSOR_ID(processor_id)) ||
        (IPC_SECONDARY_CONTEXT() && ((!OPMGR_ON_SAME_CORE(processor_id)) || (op_id == 0))))
    {
    ...
    new_op->thread_offload_enabled = offload_enabled;
#endif
...
static bool opmgr_create_operator_cback(unsigned int status)
{
    ...
    else
    {
        ...
        opmgr_create_operator_post_dnld(create_req_params->con_id, create_req_params->cap_id,
                ...
                create_req_params->use_thread_offload, create_req_params->callback);
    }
                              ...
                              OP_CREATE_CBACK callback)
{
    ...
    for(i=0; i<num_keys; i++)
    {
        switch(info[i].key)
        {
            ...
            case OPERATOR_CREATE_PROCESSOR_ID:
            {
                ...
                processor_id = (OP_PROCESSOR_ID)info[i].value;

#ifdef INSTALL_DUAL_CORE_SUPPORT
                /* The primary processor is always available while auxiliary processors might not be. */
                if ((info[i].value != 0) && (!kip_aux_processor_has_started((IPC_PROCESSOR_ID_NUM)info[i].value)))
                {
#ifdef INSTALL_THREAD_OFFLOAD
                    if (audio_thread_offload_is_enabled())
                    {
...
#ifdef INSTALL_DUAL_CORE_SUPPORT
    if (IPC_PRIMARY_CONTEXT() && (processor_id != 0))
    {
    ...
    if(OPMGR_ON_SAME_CORE(processor_id))
    {
        ...
        if (is_downloadable && (status == CAP_INSTALLED))
        {
            ...
            create_req_params->use_thread_offload = offload_enabled;
    ...
    opmgr_create_operator_post_dnld(con_id, cap_id, op_id, priority, processor_id, offload_enabled, callback);
}
...
void opmgr_destroy_ops_by_con_id(unsigned con_id, OP_CON_ID_CBACK callback, void *data)
{
    ...
    for (proc_id_index=IPC_PROCESSOR_1; proc_id_index<=kip_get_max_processor_id(); proc_id_index++)
    {
    ...
    for (proc_id_index=IPC_PROCESSOR_1; proc_id_index<=kip_get_max_processor_id(); proc_id_index++)
    {
        cnt += opmgr_list_ops(PACK_CONID_PROCID(con_id, proc_id_index), remote_oplist_head, cnt, destroy_list);
    }
#endif
...
static bool opmgr_validate_message_source(unsigned sender_con_id, unsigned owner_con_id)
{
...
#if defined(INSTALL_DUAL_CORE_SUPPORT) || defined(AUDIO_SECOND_CORE)
    source_valid = (sender_con_id == owner_con_id) || (GET_CON_ID_SEND_ID(GET_SEND_RECV_ID_CLIENT_ID(sender_con_id)) == OPMGR_ADAPTOR_OBPM);
#ifdef INSTALL_AOV
    ...
    source_valid = (sender_con_id == owner_con_id) || (sender_con_id == OPMGR_ADAPTOR_OBPM);
#ifdef INSTALL_AOV
    source_valid = source_valid ||
 ...
 *
 */
void opmgr_operator_message(unsigned int con_id, EXT_OP_ID op_id,
...
#ifdef CHIP_BASE_BC7
        COMPILE_TIME_ASSERT(OPMGR_ADAPTOR_STIBBONS == ADAPTOR_STIBBONS,
            Assumption_about_OPMGR_ADAPTOR_STIBBONS_in_opmgr_violated);
#endif /* CHIP_BASE_BC7 */
 ...
 */
bool opmgr_unsolicited_message(unsigned con_id, OP_PROCESSOR_ID processor_id, OP_UNSOLICITED_MSG *msg_from_op)
{
