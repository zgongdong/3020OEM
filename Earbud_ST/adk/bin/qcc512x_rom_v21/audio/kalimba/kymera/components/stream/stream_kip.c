 ...
 * \return  TRUE on success
 */
static bool stream_kip_transform_list_remove_entry(unsigned packed_con_id,
                                                   unsigned count,
                                                   unsigned *tr_list,
                                                   void     *state)
{
 ...
 * \return TRUE on success
 */
static bool stream_kip_send_transform_disconnect_resp(unsigned con_id,
                                                      unsigned status,
                                                      unsigned count)
{
 ...
 * \return TRUE on success
 */
static bool stream_kip_transform_list_remove_entry_resp(unsigned con_id,
                                                        unsigned status,
                                                        unsigned count)
{
 ...
 */
STREAM_KIP_CONNECT_INFO *stream_kip_create_connect_info_record_ex(
                            unsigned con_id, unsigned source_id, unsigned sink_id,
                            STREAM_EP_LOCATION ep_location,
                            unsigned block_size, unsigned period,
    ...
    else
    {
        ...
        if (state->ep_location != STREAM_EP_REMOTE_ALL)
        {
                ...
                STREAM_EP_IS_SHADOW_ID(state->source_id))
            {
                ...
                KIP_MSG_STREAM_CONNECT_CONFIRM_REQ_PACK(&req,
                                                        UNPACK_REVERSE_CONID(con_id),
                                                        stream_external_id_from_endpoint(op_term));
 ...
 */
void stream_kip_connect_confirm_handler(unsigned con_id, unsigned conn_to)
{
    unsigned shadow_id;
    ENDPOINT *shadow_ep;
    endpoint_shadow_state *state;
                                                       ...
                                                       uint16 meta_channel_id)
{
    ...
    IPC_PROCESSOR_ID_NUM remote_proc_id = KIP_PRIMARY_CONTEXT () ?
             GET_RECV_PROC_ID(packed_con_id) : GET_SEND_PROC_ID(packed_con_id);
